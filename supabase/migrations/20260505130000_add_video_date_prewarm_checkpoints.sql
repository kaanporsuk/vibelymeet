-- Additive expansion of the Video Date launch-latency checkpoint allowlist.
--
-- Adds the six client-facing checkpoints that the upcoming Daily prewarm
-- slice will emit:
--   * daily_prewarm_started         — call object created, startCamera() begins
--   * daily_prewarm_camera_ready    — startCamera() resolved, devices acquired
--   * daily_prewarm_preauth_success — preAuth() returned (token validated)
--   * daily_prewarm_consumed        — date screen adopted the prewarmed object
--   * daily_prewarm_fallback        — date screen rejected handoff and rebuilt
--   * daily_prewarm_destroyed       — handoff cleaned up before consumption
--
-- Pure additive change. Safety gates, payload sanitization, ACLs, and the
-- emitted `client_event_name` payload key all remain identical to
-- 20260505120000_video_date_launch_latency_checkpoint_observability.sql.
-- No new index is required; the existing
-- `event_loop_obs_video_date_launch_latency_idx` already covers reads of the
-- new rows because it indexes by (session_id, actor_id, created_at) where
...
COMMENT ON FUNCTION public.record_video_date_launch_latency_checkpoint(uuid, text, jsonb, integer) IS
  ...
  'Allowlist now includes daily_prewarm_* checkpoints emitted by the Daily prewarm slice. '
  'Payload is sanitized; no state transitions are changed.';
