/**
 * Backend RPC: detect_ghost_bootstrap_accounts
 * 
 * Identifies likely ghost/abandoned bootstrap-fresh duplicate accounts for admin review.
 * Read-only observability only; no destructive actions.
 * 
 * Returns candidates with:
 *   - bootstrap_fresh profile state (incomplete onboarding)
 *   - stale creation date (older than threshold)
 *   - zero/minimal activity
 *   - optional: identity collision hints
 */
...
AS $$
  WITH profile_completeness AS (
    SELECT
      ...
      p.about_me,
      p.last_seen_at,
      -- Determine if profile is bootstrap_fresh
      (
  ...
  activity_counts AS (
    SELECT
      ...
      -- Activity score: sum of normalized engagement metrics
...
COMMENT ON FUNCTION public.detect_ghost_bootstrap_accounts(int, int) IS
  'Identifies likely ghost/abandoned bootstrap-fresh profiles for admin review. '
  'Returns masked contact info and activity metrics. Read-only observability. '
  'days_old_threshold: minimum days old (default 7); min_activity_threshold: max activity score (default 0).';

-- Grant admin-only access
REVOKE ALL ON FUNCTION public.detect_ghost_bootstrap_accounts(int, int) FROM public;
GRANT EXECUTE ON FUNCTION public.detect_ghost_bootstrap_accounts(int, int) TO authenticated;
-- Full access only via dashboard (auth.uid() check in admin UI layer)
