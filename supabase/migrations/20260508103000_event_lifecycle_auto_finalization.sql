-- Event lifecycle auto-finalization.
--
-- Scheduled event completion is automatic after a 10 minute operator grace.
-- User-facing access closes at scheduled_end; ended_at is written shortly after
-- as durable lifecycle truth and to fire existing downstream cleanup triggers.
...
BEGIN
  WITH candidates AS (
    ...
    WHERE e.archived_at IS NULL
      ...
      AND lower(COALESCE(e.status, 'upcoming')) NOT IN ('draft', 'cancelled')
...
BEGIN
  ...
  IF v_before.archived_at IS NOT NULL OR v_before.ended_at IS NOT NULL OR lower(COALESCE(v_before.status, '')) IN ('draft', 'cancelled') THEN
...
BEGIN
  ...
  SELECT * INTO v_before FROM public.events WHERE id = p_event_id FOR UPDATE;
  IF NOT FOUND THEN RETURN public.admin_json_error('NOT_FOUND', 'Event was not found.'); END IF;
  IF v_before.archived_at IS NOT NULL OR v_before.ended_at IS NOT NULL OR lower(COALESCE(v_before.status, '')) IN ('draft', 'completed', 'cancelled') THEN
...
BEGIN
  ...
  IF NOT FOUND THEN RETURN public.admin_json_error('NOT_FOUND', 'Event was not found.'); END IF;
  IF v_event.archived_at IS NOT NULL OR v_event.ended_at IS NOT NULL OR lower(COALESCE(v_event.status, '')) IN ('draft', 'cancelled') THEN
...
BEGIN
  ...
  IF NOT FOUND
     ...
     OR lower(COALESCE(v_status, '')) IN ('draft', 'cancelled')
...
BEGIN
  ...
  SELECT * INTO v_before FROM public.events WHERE id = p_event_id FOR UPDATE;
  IF NOT FOUND THEN RETURN public.admin_json_error('NOT_FOUND', 'Event was not found.'); END IF;
  IF v_before.archived_at IS NOT NULL OR v_before.ended_at IS NOT NULL OR lower(COALESCE(v_before.status, '')) IN ('draft', 'cancelled', 'completed') THEN
...
BEGIN
  ...
  SELECT * INTO v_before FROM public.events WHERE id = p_event_id FOR UPDATE;
  IF NOT FOUND THEN RETURN public.admin_json_error('NOT_FOUND', 'Event was not found.'); END IF;
  IF v_before.archived_at IS NOT NULL OR v_before.ended_at IS NOT NULL OR lower(COALESCE(v_before.status, '')) IN ('cancelled', 'completed') THEN
    RETURN public.admin_json_error('INVALID_TRANSITION', 'Event is archived or already terminal.', jsonb_build_object('status', v_before.status));
  END IF;
    ...
    'events', jsonb_build_object(
      ...
      'live', count(*) FILTER (
        WHERE archived_at IS NULL
          ...
          AND lower(COALESCE(status, 'upcoming')) NOT IN ('draft', 'cancelled')
      ...
      'archived', count(*) FILTER (WHERE archived_at IS NOT NULL)::integer,
      'ended', count(*) FILTER (
...
VALUES (
  ...
  'Event lifecycle auto-finalization',
  'schema+policy',
  'Adds cron-safe event finalization after a 10 minute grace, makes admin_activity_logs support system actor rows, and tightens registration/payment/reminder/extension cutoffs at scheduled event end. No event rows are rewritten by the migration itself.',
  false
)
