-- Align Mystery Match-created video_sessions with canonical ready-gate statuses.
-- Legacy behavior created ready_gate_status='waiting', which is outside the current
-- promotion/cleanup semantics centered on queued/ready/ready_a/ready_b/both_ready/expired.

-- 1) Normalize existing live legacy rows first.
-- If participants are already in_ready_gate for this session, map waiting -> ready.
...
WHERE vs.ended_at IS NULL
  AND vs.ready_gate_status = 'waiting'
  AND EXISTS (
    SELECT 1
    FROM public.event_registrations er1
    WHERE er1.event_id = vs.event_id
      AND er1.profile_id = vs.participant_1_id
      AND er1.queue_status = 'in_ready_gate'
      AND er1.current_room_id = vs.id
  )
  AND EXISTS (
    SELECT 1
    FROM public.event_registrations er2
    WHERE er2.event_id = vs.event_id
      AND er2.profile_id = vs.participant_2_id
      AND er2.queue_status = 'in_ready_gate'
      AND er2.current_room_id = vs.id
  );

-- Any other still-live waiting row is legacy pending; map to canonical queued TTL path.
UPDATE public.video_sessions vs
SET
  ready_gate_status = 'queued',
  ready_gate_expires_at = NULL,
  queued_expires_at = COALESCE(vs.queued_expires_at, COALESCE(vs.started_at, now()) + interval '10 minutes')
WHERE vs.ended_at IS NULL
  AND vs.ready_gate_status = 'waiting';

-- 2) Recreate mystery-match function so new sessions use canonical status.
