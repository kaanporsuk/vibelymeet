-- VD acceptance follow-up round 2 (item 2b): per-participant in_survey stamping.
--
-- The PR-#1314 guard made the terminal in_survey stamp pair-feedback-aware
-- (skip when BOTH verdicts exist). This round refines stamping to be
-- per-row: a participant whose own date_feedback row already exists is never
-- re-stamped in_survey, even while the partner is still incomplete. Removes
-- the one residual survey-route bounce a completed user could take when the
-- pair was re-stamped on their behalf. The pair-level branch guard (stamp vs
-- release) is unchanged; mid-flow writers (reconnect expiry, deadline
-- finalizer, absence review) are untouched (they run where feedback cannot
-- exist); submit_video_date_safety_report_v2's stamp stays intentional.
--
-- Both bodies are full live recreates dumped 2026-06-12 post-#1314 and
-- patched only at the stamp predicates (transition: deep-block + result-ended
-- stamps; remote_seen: its terminal stamp).
