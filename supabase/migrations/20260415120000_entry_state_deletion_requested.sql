-- Entry-state resolver: distinguish scheduled deletion recovery from moderation suspension.
--
-- Pending account deletion inside the grace window should route to entry recovery with
-- state `deletion_requested`, including the scheduled deletion date. Real moderation
-- suspensions keep routing to `account_suspended`.
...
DECLARE
  ...
  v_pending_deletion_scheduled_at timestamptz := NULL;
  v_has_active_moderation_suspension boolean := false;
  v_has_legacy_deletion_hold boolean := false;
BEGIN
  ...
  -- Normalise to no-'+' form: auth.users.phone never has '+', but phone-verify stores
  -- profiles.phone_number with '+'. Strip here once so all comparisons are consistent.
  v_current_phone := ltrim(NULLIF(trim(COALESCE(v_auth_user.phone, '')), ''), '+');
  ...
  SELECT adr.scheduled_deletion_at
  INTO v_pending_deletion_scheduled_at
  FROM public.account_deletion_requests adr
  WHERE adr.user_id = v_uid
    AND adr.status = 'pending'
    AND adr.scheduled_deletion_at > now()
  ORDER BY adr.scheduled_deletion_at ASC, adr.requested_at DESC
  LIMIT 1;
  ...
  INTO v_has_active_moderation_suspension;

  v_has_legacy_deletion_hold :=
    v_pending_deletion_scheduled_at IS NOT NULL
    AND v_has_profile
    AND COALESCE(v_profile.is_suspended, false) = true
    AND COALESCE(v_profile.suspension_reason, '') = 'Account deletion requested'
    AND NOT v_has_active_moderation_suspension;

  IF v_pending_deletion_scheduled_at IS NOT NULL
     AND NOT v_has_active_moderation_suspension
     AND (
       NOT v_has_profile
       OR COALESCE(v_profile.is_suspended, false) = false
       OR v_has_legacy_deletion_hold
     )
  THEN
    RETURN jsonb_build_object(
      ...
      'candidate_fragment', NULL,
      'scheduled_deletion_at', v_pending_deletion_scheduled_at,
      'retryable', false,
      'evaluation_version', 1
    );
  END IF;

  -- Moderation: suspended accounts cannot use the normal app shell at any onboarding stage.
  IF v_has_active_moderation_suspension
     OR (v_has_profile AND COALESCE(v_profile.is_suspended, false) = true)
  THEN
    RETURN jsonb_build_object(
  ...
  -- Completed users: early-return before candidate analysis.
    ...
    SELECT
      ...
      CASE
        WHEN v_current_phone IS NOT NULL
          ...
          -- Normalise profile phone to no-'+' form to match v_current_phone
          AND ltrim(NULLIF(trim(COALESCE(p.phone_number, '')), ''), '+') = v_current_phone
        THEN 'verified_phone'
...
COMMENT ON FUNCTION public.resolve_entry_state IS
  'Shared entry-state resolver for authenticated users. Routes to app, onboarding, or entry recovery; returns deletion_requested for live scheduled deletion holds, and account_suspended for real moderation restrictions. Does not expose foreign account identifiers or raw contact data.';
