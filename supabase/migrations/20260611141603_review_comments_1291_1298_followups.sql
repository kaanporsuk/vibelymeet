-- Follow-ups for Codex review comments across PRs 1291-1298.
--
-- The queued-residue purge accidentally replaced Sprint 7 ops health with a
-- zero-only payload. Restore the real safety/ops rollups in a forward
-- migration while keeping the removed queue/drain counters out of the contract.
...
BEGIN
  ...
  session_rollup AS (
    SELECT
      ...
      count(*) FILTER (
        WHERE sw.ended_at IS NULL
          AND (
            COALESCE(sw.phase, '') IN ('entry', 'handshake', 'warmup')
            OR sw.state::text IN ('entry', 'handshake')
          )
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'stuck_handshake_count', COALESCE(sr.stuck_entry_count, 0),
...
COMMENT ON FUNCTION public.get_video_date_sprint7_ops_health(uuid) IS
  'Service-role-only Sprint 7 Video Date safety/privacy/ops dashboard payload. Restores real stuck-session, survey, safety, webhook DLQ, and orphan-room counts after queued-state removal; queue/drain counters remain removed.';
...
VALUES (
  ...
  'schema+policy',
  'Forward repair for Codex review comments. Replaces one service-role-only aggregate RPC body without changing its signature, grants, or client surface.',
  false
)
