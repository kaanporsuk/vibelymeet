-- Follow-up fixes for unresolved Codex review comments across PR #1199,
-- #1200, #1202, and #1204.
--
-- This migration updates live functions instead of rewriting already-applied
-- migration history.

BEGIN;

DROP FUNCTION IF EXISTS public.video_date_promote_confirmed_encounter_v1_20260605221535_participant_auth_base(
  uuid,
  uuid,
  text,
  text,
...
ALTER FUNCTION public.video_date_promote_confirmed_encounter_v1(uuid, uuid, text, text, boolean)
  RENAME TO video_date_promote_confirmed_encounter_v1_20260605221535_participant_auth_base;

REVOKE ALL ON FUNCTION public.video_date_promote_confirmed_encounter_v1_20260605221535_participant_auth_base(
  uuid,
  uuid,
  text,
  text,
...
GRANT EXECUTE ON FUNCTION public.video_date_promote_confirmed_encounter_v1_20260605221535_participant_auth_base(
  uuid,
  uuid,
  text,
  text,
...
BEGIN
  ...
  RETURN public.video_date_promote_confirmed_encounter_v1_20260605221535_participant_auth_base(
    p_session_id,
    p_actor,
    p_source,
    p_reason,
...
COMMENT ON FUNCTION public.video_date_promote_confirmed_encounter_v1(uuid, uuid, text, text, boolean) IS
  'Participant-authorized confirmed-encounter promotion wrapper. Authenticated callers are checked before delegating to the base function that may repair room metadata.';
...
COMMENT ON FUNCTION public.expire_video_date_reconnect_graces() IS
  'Ends expired Video Date reconnect graces only after rechecking latest joined, remote-media, lifecycle reason, and current unexpired video-date surface evidence. Rejoin suppression requires the participant with the latest away marker to have rejoined after their own away timestamp.';
