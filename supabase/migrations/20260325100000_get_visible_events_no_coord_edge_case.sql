-- Fix: local-scope events without coordinates should not silently appear in
-- radius-filtered views. Global/regional events intentionally have no coordinates
-- and should still pass through. Only scope='local' with NULL lat/lng is tightened.
...
BEGIN
  ...
  v_can_premium_browse := COALESCE(v_sub_active, false) OR COALESCE(v_is_admin, false);
  ...
  WHERE e.archived_at IS NULL
    ...
    -- Scope visibility: who can see this event?
    AND (
      e.scope = 'global'
      OR e.scope IS NULL
    ...
    -- User radius filter: narrow results to selected distance
    AND (
      p_filter_radius_km IS NULL
      OR v_effective_lat IS NULL
      -- Global/regional/null-scope events are not coordinate-filtered
      OR COALESCE(e.scope, 'global') IN ('global', 'regional')
      -- Local events must have coordinates to pass a radius filter
