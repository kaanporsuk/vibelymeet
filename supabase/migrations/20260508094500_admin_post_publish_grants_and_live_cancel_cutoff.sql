-- Post-publish guardrails for Daily Drop worker writes and live event self-cancel truth.
-- The preceding migrations were already applied to cloud; keep these changes replayable.
...
BEGIN
  ...
  IF v_event.event_date IS NULL
     ...
     OR btrim(lower(COALESCE(v_event.status, ''))) IN ('draft', 'cancelled', 'live', 'ended', 'completed')
...
VALUES (
  '20260508094500',
  'Admin post-publish grants and live cancel cutoff',
  'schema+policy',
  'Grants Daily Drop generation run write authority to service_role and closes authenticated self-cancel for events already marked live. No data rewrite.',
  false
)
...
COMMENT ON FUNCTION public.cancel_event_registration(uuid) IS
  'Authenticated self-cancel for event registrations. Closes when the event start is missing, once the event starts or is marked live, or once terminal/archived truth is present; admin removal remains available through governed admin RPCs.';
