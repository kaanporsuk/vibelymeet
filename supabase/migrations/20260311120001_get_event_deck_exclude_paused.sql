-- Stream 1B: Exclude paused profiles from event deck (discoverability).
-- A profile is effectively paused when is_paused = true and (paused_until IS NULL OR paused_until > now()).
...
BEGIN
  ...
  WHERE er.event_id = p_event_id
    ...
    AND (p.is_paused = false OR (p.paused_until IS NOT NULL AND p.paused_until <= now()))
    AND EXISTS (
