-- Cloud sync follow-up for schedule-share accept hardening.
--
-- Replay-safe: pure CREATE OR REPLACE FUNCTION.
-- Non-destructive: preserves the existing v2 action surface while guarding
-- malformed payloads, enforcing exact-time schedule-share accepts, and binding
-- chosen slots to the grant attached to this suggestion.
...
BEGIN
  ...
  ELSIF p_action = 'counter' THEN
    ...
    IF v_payload->'revision'->'selected_slot_keys' IS NULL
       OR jsonb_typeof(v_payload->'revision'->'selected_slot_keys') = 'null' THEN
      r_slot_keys := NULL;
    ELSIF jsonb_typeof(v_payload->'revision'->'selected_slot_keys') <> 'array' THEN
      RETURN jsonb_build_object('ok', false, 'error', 'invalid_selected_slot_keys');
    ELSE
      r_slot_keys := ARRAY(SELECT jsonb_array_elements_text(v_payload->'revision'->'selected_slot_keys'));
    END IF;
