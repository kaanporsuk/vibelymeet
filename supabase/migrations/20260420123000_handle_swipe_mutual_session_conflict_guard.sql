-- Stream 1C: mutual handle_swipe — align with promote_ready_gate_if_eligible conflict semantics.
--
-- Before inserting into video_sessions, reject when either participant already has a
-- *different* non-ended session row in this event (same pair is still handled by
-- INSERT ... ON CONFLICT → already_matched). Mirrors the EXISTS guard in
-- promote_ready_gate_if_eligible (step 6), adapted for a net-new pair insert.
...
BEGIN
  ...
  IF v_mutual THEN
    ...
    -- Same logical guard as promote_ready_gate_if_eligible step 6: block a *new* session
    -- row when either participant is already in another non-ended session for this
    -- event with a *different* pair. Same pair remains INSERT + ON CONFLICT DO NOTHING.
    IF EXISTS (
      SELECT 1
