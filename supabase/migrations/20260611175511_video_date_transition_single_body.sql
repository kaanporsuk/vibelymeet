...
BEGIN
  ...
  -- physically backed by the legacy handshake state machine (flips in PR 5).
    ...
    IF v_result IS NULL THEN
      BEGIN
        ...
        IF v_result IS NULL
          ...
          IF FOUND
            ...
            v_recent_handshake :=
              ...
              AND v_session.handshake_started_at >= v_clock_now - v_warmup_window;
        ...
        IF v_result IS NULL THEN
          <<deep>>
          LOOP
            -- complete_handshake delegates to the deadline finalizer
            -- (formerly vdt_survey_continuity).
            IF v_delegate_action = 'complete_handshake' THEN
            ...
            IF v_delegate_action IN ('vibe', 'pass') AND v_actor IS NOT NULL THEN
              ...
              IF FOUND
                 ...
                 AND v_session.handshake_started_at + interval '60 seconds' <= now() THEN
                v_result := public.finalize_video_date_handshake_deadline(
                  p_session_id,
                  v_actor,
                  'late_' || v_delegate_action || '_after_handshake_deadline',
                  p_reason
                );
                EXIT deep;
              END IF;
