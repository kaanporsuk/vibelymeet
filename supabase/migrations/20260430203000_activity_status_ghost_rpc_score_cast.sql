-- Keep the admin-guarded ghost diagnostic RPC's declared return contract stable.
-- COUNT aggregates are bigint in Postgres, so the derived activity score must be
-- cast back to int to match the existing RPC result shape.
