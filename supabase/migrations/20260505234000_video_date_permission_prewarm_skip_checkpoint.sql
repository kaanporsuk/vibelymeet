-- Add a launch-latency checkpoint for browsers that cannot report camera
-- permission state through the Permissions API. This is observability-only:
-- payload sanitization, participant auth, and timeline visibility are unchanged.
...
COMMENT ON FUNCTION public.record_video_date_launch_latency_checkpoint(uuid, text, jsonb, integer) IS
  ...
  'Allowlist includes Daily prewarm, route preload, and permission-prewarm skip checkpoints. '
