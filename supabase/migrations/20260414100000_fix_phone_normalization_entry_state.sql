-- Fix phone normalization in resolve_entry_state() fragmentation detection.
--
-- Root cause: auth.users.phone stores phone numbers WITHOUT a leading '+' (e.g. '905072054954'),
-- but profiles.phone_number set via the phone-verify Edge Function stores the client-supplied
-- E.164 value WITH a leading '+' (e.g. '+905072054954').  The bootstrap_profile_from_auth_user
-- trigger also copies auth.users.phone as-is (no '+'), so the same profile can have either form.
-- Raw string equality therefore produces false negatives and suppresses fragmented-identity
-- detection for phone-verified users.
--
-- Fix: normalise both sides to the no-'+' form at comparison time using ltrim(..., '+').
-- v_current_phone is normalised once at assignment; profile phone is normalised inline in
-- the two comparison sites inside candidate_matches (CASE expression and WHERE clause).
--
-- Scope: pure function replacement — no schema changes, no new tables, no env vars.
-- Completed-user path is unaffected: onboarding_complete = true still early-returns at
-- line 113 before candidate_matches is evaluated.
...
BEGIN
  ...
  -- This path is unchanged by this migration.
