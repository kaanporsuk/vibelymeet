...
-- Advance sequence past any existing VB-##### reference IDs to prevent
-- UNIQUE constraint collisions. Safe to run even if no tickets exist yet.
SELECT setval(
  'public.support_ticket_ref_seq',
  GREATEST(
    10001,
    COALESCE(
      (SELECT MAX(
        CAST(NULLIF(regexp_replace(reference_id, '[^0-9]', '', 'g'), '') AS integer)
       ) FROM public.support_tickets
       WHERE reference_id ~ '^VB-[0-9]+$'),
      10000
    ) + 1
  ),
  false
