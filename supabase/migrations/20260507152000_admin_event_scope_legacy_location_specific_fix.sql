-- Allow admins to move legacy location-specific events to regional/global scopes.
--
-- Older event rows may have is_location_specific=true even when the admin form is
-- changing scope away from local and clearing coordinates. Normalize the effective
-- update payload before validation so the legacy flag does not keep requiring
-- latitude/longitude after the event is no longer local.
...
BEGIN
  ...
  IF p_payload ? 'scope'
     AND COALESCE(NULLIF(lower(p_payload ->> 'scope'), ''), 'global') <> 'local'
     AND NOT (p_payload ? 'is_location_specific') THEN
...
COMMENT ON FUNCTION public.admin_update_event(uuid, jsonb, text) IS
  'Admin-owned event update mutation. Clears legacy location-specific state when scope moves away from local.';
