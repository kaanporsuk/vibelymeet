-- Admin event publish/unpublish lifecycle actions.
...
BEGIN
  ...
  IF p_event_id IS NULL THEN RETURN public.admin_json_error('VALIDATION_ERROR', 'Event id is required.'); END IF;

  v_cached := public.admin_idempotency_begin(v_admin_id, 'admin_publish_event', p_idempotency_key, jsonb_build_object('event_id', p_event_id, 'reason', p_reason));
  IF v_cached IS NOT NULL THEN RETURN v_cached; END IF;

  SELECT * INTO v_before FROM public.events WHERE id = p_event_id FOR UPDATE;
  IF NOT FOUND THEN
    v_response := public.admin_json_error('NOT_FOUND', 'Event was not found.');
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event', p_idempotency_key, v_response);
  END IF;

  v_status := lower(COALESCE(v_before.status, ''));
  IF v_before.archived_at IS NOT NULL OR v_status = 'archived' THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Archived events cannot be published.', jsonb_build_object('status', v_before.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event', p_idempotency_key, v_response);
  END IF;
  IF v_before.ended_at IS NOT NULL OR v_status IN ('live', 'cancelled', 'ended', 'completed') THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Event cannot be published from its current lifecycle state.', jsonb_build_object('status', v_before.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event', p_idempotency_key, v_response);
  END IF;
  IF v_status NOT IN ('draft', 'upcoming') THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Only draft events can be published.', jsonb_build_object('status', v_before.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event', p_idempotency_key, v_response);
  END IF;
  IF v_before.event_date IS NULL OR v_now >= v_before.event_date THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Only future events can be published.', jsonb_build_object('event_date', v_before.event_date));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event', p_idempotency_key, v_response);
  END IF;

  IF v_status = 'upcoming' THEN
    v_response := public.admin_json_success(jsonb_build_object('event_id', p_event_id, 'event', to_jsonb(v_before), 'changed', false));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event', p_idempotency_key, v_response);
  END IF;

  v_validation := public.admin_validate_event_payload(to_jsonb(v_before) || jsonb_build_object('status', 'upcoming'), false);
  IF v_validation IS NOT NULL THEN
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event', p_idempotency_key, v_validation);
  END IF;
  ...
  v_audit_id := public.log_admin_action('event.publish', 'event', p_event_id, jsonb_build_object('reason', p_reason, 'before', to_jsonb(v_before), 'after', to_jsonb(v_after)));
  v_response := public.admin_json_success(jsonb_build_object('event_id', p_event_id, 'event', to_jsonb(v_after), 'audit_log_id', v_audit_id, 'changed', true));
  RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event', p_idempotency_key, v_response);
END;
$function$;

CREATE OR REPLACE FUNCTION public.admin_unpublish_event(
  p_event_id uuid,
  p_reason text DEFAULT NULL,
  p_idempotency_key text DEFAULT NULL
)
...
BEGIN
  ...
  v_cached := public.admin_idempotency_begin(v_admin_id, 'admin_unpublish_event', p_idempotency_key, jsonb_build_object('event_id', p_event_id, 'reason', p_reason));
  IF v_cached IS NOT NULL THEN RETURN v_cached; END IF;

  SELECT * INTO v_before FROM public.events WHERE id = p_event_id FOR UPDATE;
  IF NOT FOUND THEN
    v_response := public.admin_json_error('NOT_FOUND', 'Event was not found.');
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event', p_idempotency_key, v_response);
  END IF;

  v_status := lower(COALESCE(v_before.status, ''));
  IF v_before.archived_at IS NOT NULL OR v_status = 'archived' THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Archived events cannot be unpublished.', jsonb_build_object('status', v_before.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event', p_idempotency_key, v_response);
  END IF;
  IF v_before.ended_at IS NOT NULL OR v_status IN ('live', 'cancelled', 'ended', 'completed') THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Event cannot be unpublished from its current lifecycle state.', jsonb_build_object('status', v_before.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event', p_idempotency_key, v_response);
  END IF;
  IF v_status NOT IN ('upcoming', 'draft') THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Only upcoming events can be unpublished.', jsonb_build_object('status', v_before.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event', p_idempotency_key, v_response);
  END IF;
  IF v_before.event_date IS NULL OR v_now >= v_before.event_date THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Only future events can be unpublished.', jsonb_build_object('event_date', v_before.event_date));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event', p_idempotency_key, v_response);
  END IF;

  SELECT
    count(*) FILTER (WHERE admission_status = 'confirmed')::integer,
    count(*) FILTER (WHERE admission_status = 'waitlisted')::integer
  INTO v_confirmed_count, v_waitlisted_count
  FROM public.event_registrations
  WHERE event_id = p_event_id
    AND admission_status IN ('confirmed', 'waitlisted');
  v_active_count := COALESCE(v_confirmed_count, 0) + COALESCE(v_waitlisted_count, 0);

  IF v_active_count > 0 THEN
    v_response := public.admin_json_error(
      'INVALID_TRANSITION',
      'Events with confirmed or waitlisted registrations cannot be unpublished.',
      jsonb_build_object('active_registration_count', v_active_count, 'confirmed_count', v_confirmed_count, 'waitlisted_count', v_waitlisted_count)
    );
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event', p_idempotency_key, v_response);
  END IF;
  ...
  v_audit_id := public.log_admin_action('event.unpublish', 'event', p_event_id, jsonb_build_object('reason', p_reason, 'before', to_jsonb(v_before), 'after', to_jsonb(v_after)));
  v_response := public.admin_json_success(jsonb_build_object('event_id', p_event_id, 'event', to_jsonb(v_after), 'audit_log_id', v_audit_id, 'changed', true));
  RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event', p_idempotency_key, v_response);
END;
$function$;

CREATE OR REPLACE FUNCTION public.admin_publish_event_series(
...
BEGIN
  ...
  v_cached := public.admin_idempotency_begin(v_admin_id, 'admin_publish_event_series', p_idempotency_key, jsonb_build_object('parent_event_id', p_parent_event_id, 'reason', p_reason));
  IF v_cached IS NOT NULL THEN RETURN v_cached; END IF;

  SELECT * INTO v_parent FROM public.events WHERE id = p_parent_event_id FOR UPDATE;
  IF NOT FOUND THEN
    v_response := public.admin_json_error('NOT_FOUND', 'Recurring event series was not found.');
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event_series', p_idempotency_key, v_response);
  END IF;
  IF COALESCE(v_parent.is_recurring, false) IS NOT TRUE THEN
    v_response := public.admin_json_error('VALIDATION_ERROR', 'Parent event is not recurring.');
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event_series', p_idempotency_key, v_response);
  END IF;

  v_parent_status := lower(COALESCE(v_parent.status, ''));
  IF v_parent.archived_at IS NOT NULL OR v_parent_status IN ('archived', 'cancelled') THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Archived or cancelled series cannot be published.', jsonb_build_object('status', v_parent.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event_series', p_idempotency_key, v_response);
  END IF;
  ...
  SELECT COALESCE(array_agg(id ORDER BY event_date NULLS LAST, id), ARRAY[]::uuid[])
  INTO v_candidate_ids
  FROM public.events
  WHERE (id = p_parent_event_id OR parent_event_id = p_parent_event_id)
    AND event_date IS NOT NULL
    AND event_date > v_now
    AND archived_at IS NULL
    AND ended_at IS NULL
    AND lower(COALESCE(status, '')) IN ('draft', 'upcoming');
  ...
  LOOP
    v_validation := public.admin_validate_event_payload(to_jsonb(v_event) || jsonb_build_object('status', 'upcoming'), false);
    IF v_validation IS NOT NULL THEN
      RETURN public.admin_idempotency_complete(
        v_admin_id,
        'admin_publish_event_series',
        p_idempotency_key,
        v_validation || jsonb_build_object('event_id', v_event.id)
      );
    END IF;
  END LOOP;
  ...
  SELECT count(*)::integer, COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[])
  INTO v_changed_count, v_changed_ids
  FROM updated;

  v_audit_id := public.log_admin_action(
    'event.publish_series',
    'event',
    p_parent_event_id,
    jsonb_build_object('reason', p_reason, 'candidate_ids', v_candidate_ids, 'published_count', v_changed_count, 'event_ids', v_changed_ids)
  );
  v_response := public.admin_json_success(jsonb_build_object('parent_event_id', p_parent_event_id, 'candidate_ids', v_candidate_ids, 'published_count', v_changed_count, 'event_ids', v_changed_ids, 'audit_log_id', v_audit_id));
  RETURN public.admin_idempotency_complete(v_admin_id, 'admin_publish_event_series', p_idempotency_key, v_response);
END;
$function$;

CREATE OR REPLACE FUNCTION public.admin_unpublish_event_series(
  p_parent_event_id uuid,
  p_reason text DEFAULT NULL,
  p_idempotency_key text DEFAULT NULL
)
...
BEGIN
  ...
  v_cached := public.admin_idempotency_begin(v_admin_id, 'admin_unpublish_event_series', p_idempotency_key, jsonb_build_object('parent_event_id', p_parent_event_id, 'reason', p_reason));
  IF v_cached IS NOT NULL THEN RETURN v_cached; END IF;

  SELECT * INTO v_parent FROM public.events WHERE id = p_parent_event_id FOR UPDATE;
  IF NOT FOUND THEN
    v_response := public.admin_json_error('NOT_FOUND', 'Recurring event series was not found.');
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event_series', p_idempotency_key, v_response);
  END IF;
  IF COALESCE(v_parent.is_recurring, false) IS NOT TRUE THEN
    v_response := public.admin_json_error('VALIDATION_ERROR', 'Parent event is not recurring.');
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event_series', p_idempotency_key, v_response);
  ...
  v_parent_status := lower(COALESCE(v_parent.status, ''));
  IF v_parent.archived_at IS NOT NULL OR v_parent_status IN ('archived', 'cancelled') THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Archived or cancelled series cannot be unpublished.', jsonb_build_object('status', v_parent.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event_series', p_idempotency_key, v_response);
  END IF;
  ...
  INTO v_confirmed_count, v_waitlisted_count
  FROM public.event_registrations
  WHERE event_id = ANY(v_candidate_ids)
    AND admission_status IN ('confirmed', 'waitlisted');
  v_active_count := COALESCE(v_confirmed_count, 0) + COALESCE(v_waitlisted_count, 0);
  ...
  IF v_active_count > 0 THEN
    v_response := public.admin_json_error(
      'INVALID_TRANSITION',
      'Series with confirmed or waitlisted registrations cannot be unpublished.',
      jsonb_build_object('active_registration_count', v_active_count, 'confirmed_count', v_confirmed_count, 'waitlisted_count', v_waitlisted_count, 'candidate_ids', v_candidate_ids)
    );
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event_series', p_idempotency_key, v_response);
  END IF;
  ...
  v_audit_id := public.log_admin_action(
    'event.unpublish_series',
    'event',
    p_parent_event_id,
    jsonb_build_object('reason', p_reason, 'candidate_ids', v_candidate_ids, 'unpublished_count', v_changed_count, 'event_ids', v_changed_ids)
  );
  v_response := public.admin_json_success(jsonb_build_object('parent_event_id', p_parent_event_id, 'candidate_ids', v_candidate_ids, 'unpublished_count', v_changed_count, 'event_ids', v_changed_ids, 'audit_log_id', v_audit_id));
  RETURN public.admin_idempotency_complete(v_admin_id, 'admin_unpublish_event_series', p_idempotency_key, v_response);
END;
$function$;
...
BEGIN
  ...
  SELECT * INTO v_parent FROM public.events WHERE id = p_parent_event_id FOR UPDATE;
  IF NOT FOUND THEN
    v_response := public.admin_json_error('NOT_FOUND', 'Parent event was not found.');
  ...
  v_parent_status := lower(COALESCE(v_parent.status, ''));
  IF v_parent.archived_at IS NOT NULL OR v_parent_status IN ('archived', 'cancelled') THEN
    v_response := public.admin_json_error('INVALID_TRANSITION', 'Archived or cancelled recurring parents cannot generate occurrences.', jsonb_build_object('status', v_parent.status));
    RETURN public.admin_idempotency_complete(v_admin_id, 'admin_generate_recurring_events', p_idempotency_key, v_response);
  END IF;
...
REVOKE ALL ON FUNCTION public.admin_publish_event(uuid, text, text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.admin_unpublish_event(uuid, text, text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.admin_publish_event_series(uuid, text, text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.admin_unpublish_event_series(uuid, text, text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.admin_generate_recurring_events(uuid, integer, text) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.admin_publish_event(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_unpublish_event(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_publish_event_series(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_unpublish_event_series(uuid, text, text) TO authenticated;
...
COMMENT ON FUNCTION public.admin_publish_event(uuid, text, text) IS
  'Admin lifecycle action that publishes a future draft event by setting status to upcoming with audit and idempotency.';
COMMENT ON FUNCTION public.admin_unpublish_event(uuid, text, text) IS
  'Admin lifecycle action that returns a future upcoming event to draft when no confirmed or waitlisted registrations exist.';
COMMENT ON FUNCTION public.admin_publish_event_series(uuid, text, text) IS
  'Admin lifecycle action that publishes future draft occurrences in a recurring event series.';
COMMENT ON FUNCTION public.admin_unpublish_event_series(uuid, text, text) IS
  'Admin lifecycle action that returns future upcoming occurrences in a recurring event series to draft when no active registrations exist.';
COMMENT ON FUNCTION public.generate_recurring_events(uuid, integer) IS
  'Generates recurring child events; draft parents produce draft children and published parents produce upcoming children.';
COMMENT ON FUNCTION public.admin_generate_recurring_events(uuid, integer, text) IS
  'Admin wrapper for recurring occurrence generation with archive/cancel guards, audit logging, and idempotency.';
COMMENT ON FUNCTION public.admin_update_event(uuid, jsonb, text) IS
  'Admin event detail update RPC. Lifecycle fields are rejected and must be changed through explicit lifecycle admin actions.';
...
VALUES (
  '20260601120000',
  'Admin event publish and unpublish lifecycle',
  'schema+policy',
  'Adds explicit admin publish/unpublish lifecycle RPCs, preserves draft discovery suppression, hardens generic event updates against lifecycle mutation, and makes draft recurring parents generate draft children. No event rows are rewritten by the migration itself.',
  false
)
