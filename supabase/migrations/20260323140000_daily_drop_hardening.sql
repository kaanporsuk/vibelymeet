-- Daily Drop hardening: lock client UPDATE on daily_drops; disallow pass after opener; cap reply length.
-- See repo audit / daily_drop_transition prior revision in 20260311160000_daily_drop_transition.sql

DROP POLICY IF EXISTS "Users can update own drop interactions" ON public.daily_drops;

COMMENT ON COLUMN public.daily_drops.status IS
  'Lifecycle: active_* → matched|passed|expired_*|invalidated. invalidated is reserved for admin/ops; viewed_no_opener on daily_drop_cooldowns is unused by current generator.';
...
BEGIN
  ...
  -- Action: pass (not allowed after an opener was sent)
