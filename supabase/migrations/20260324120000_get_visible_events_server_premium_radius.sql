-- get_visible_events: server-side premium (subscriptions + admin), optional user radius filter,
-- remove local-scope bypass that returned all local events for premium browse coords.

DROP FUNCTION IF EXISTS public.get_visible_events(uuid, double precision, double precision, boolean, double precision, double precision);
...
BEGIN
  -- Canonical premium for browse / regional: active or trialing subscription, or admin.
  -- p_is_premium from clients is ignored (prevents spoofing).
  SELECT EXISTS (
