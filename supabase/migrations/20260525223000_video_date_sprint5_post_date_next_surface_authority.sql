-- Sprint 5: post-date survey and next-surface authority hardening.
--
-- Goals:
-- - A terminal safety-blocked/reported pair must not reopen post-date survey.
-- - A submitted verdict is immutable; retries cannot flip pass/vibe.
-- - Idempotent verdict retries must not emit duplicate session verdict events.
-- - resolve_post_date_next_surface remains the final authority and never routes
--   a reported/blocked pair into chat or another same-pair active surface.
...
DECLARE
  ...
  v_effective_liked boolean := p_liked;
...
BEGIN
  ...
  IF COALESCE(v_session.ended_reason, '') IN ('blocked_pair', 'blocked_or_reported_pair')
  ...
  SELECT EXISTS (
    ...
    WHERE (ur.reporter_id = v_uid AND ur.reported_id = v_target)
       OR (ur.reporter_id = v_target AND ur.reported_id = v_uid)
  ...
  IF NOT v_already_submitted THEN
    ...
    VALUES (p_session_id, v_uid, v_target, v_effective_liked);
  END IF;
...
COMMENT ON FUNCTION public.submit_post_date_verdict_v3(uuid, boolean, text, jsonb, text) IS
  'Post-date verdict wrapper with v4 command idempotency, immutable safety-pass coercion, safety-aware mutual confirmation, and best-effort next_surface.';
...
COMMENT ON FUNCTION public.resolve_post_date_next_surface(uuid) IS
  'Participant-only authoritative post-date router. Returns survey, ready_gate, video_date, lobby, chat, or wrap_up from backend truth while suppressing unsafe reported/blocked pair surfaces.';
