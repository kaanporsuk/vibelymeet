-- Surgical add-on: `edit_schedule_share_slots` action lets the current
-- revision author update which selected blocks they shared, without creating
-- a new revision and without touching the partner's separate grant row.
--
-- Builds on top of 20260511130000_chat_schedule_selective_share_and_event_lock
-- which introduced `schedule_share_grant_slots` and `_date_suggestion_upsert_share_grant`.
--
-- Replay-safe: pure CREATE OR REPLACE FUNCTION.
-- Non-destructive: does not change Accept, Counter, Decline, Not now, Cancel,
-- send_proposal, or send_proposal grant write paths.
...
BEGIN
  ...
  ELSIF p_action = 'edit_schedule_share_slots' THEN
    ...
    r_slot_keys := CASE
      WHEN v_payload->'selected_slot_keys' IS NULL THEN NULL
      ELSE ARRAY(SELECT jsonb_array_elements_text(v_payload->'selected_slot_keys'))
    END;
    ...
    -- Grant-owner gate: caller must already have a grant on THIS suggestion.
