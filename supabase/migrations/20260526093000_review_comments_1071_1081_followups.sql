-- Follow-ups for Copilot/Codex review comments across PRs 1071-1081.
--
-- Applied after the original admin Sprint 2 migration because that version may
-- already be present in the linked Supabase project.
...
COMMENT ON FUNCTION public.admin_mark_account_deletion_completed(uuid, text, text) IS
  'Queues governed, durable hard-delete completion work; review follow-up resets attempts when retryable, blocked, or permanently failed jobs are re-queued.';
...
VALUES (
  '20260526093000',
  'Review comments 1071-1081 follow-ups',
  'schema+policy',
  'Forward follow-up for merged PR review comments: re-queues blocked or failed account deletion completion jobs with attempts reset so worker claims can resume.',
  false
)
