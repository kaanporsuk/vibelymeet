-- Discover/home visibility: exclude cancelled; cap ended rows at effective_end + 6h.
-- Backfill ended_at for legacy status=ended rows where null (conservative).

-- ─── 1) Normalize ended_at for existing terminal ended rows ───────────────────
...
UPDATE public.events e
SET ended_at = e.event_date + COALESCE(e.duration_minutes, 60) * interval '1 minute'
WHERE e.status = 'ended'
  AND e.ended_at IS NULL;

-- ─── 2) get_visible_events ───────────────────────────────────────────────────
...
COMMENT ON FUNCTION public.get_visible_events IS
  'Returns discover/home-visible events for p_user_id. Excludes draft, cancelled, archived, recurring. '
  'Rows past effective end + 6h are hidden (effective_end = COALESCE(ended_at, scheduled end)). '
  'Location/scope/premium rules unchanged: no coords => global only; regional/local require coordinates.';
