-- Require a fully confirmed onboarding location across all completion paths.
--
-- A location is only "confirmed" when all three fields are coherent:
-- 1. `location` display text is non-blank
-- 2. `country` is non-blank
-- 3. `location_data` contains numeric lat/lng within valid ranges
--
-- This protects active finalize_onboarding callers plus the legacy
-- complete_onboarding() gate from ambiguous or half-resolved location payloads.
...
BEGIN
  ...
  BEGIN
    IF v_profile.location_data IS NOT NULL AND v_profile.location_data != 'null'::jsonb THEN
      v_location_lat := NULLIF(trim(COALESCE(v_profile.location_data->>'lat', '')), '')::double precision;
      v_location_lng := NULLIF(trim(COALESCE(v_profile.location_data->>'lng', '')), '')::double precision;
  ...
  IF v_profile.location IS NULL OR trim(v_profile.location) = ''
     OR v_profile.country IS NULL OR trim(v_profile.country) = ''
     OR NOT v_has_confirmed_location THEN
...
COMMENT ON FUNCTION public.complete_onboarding IS
  ...
  'Required: name, birth_date, age>=18, gender, 2+ photos, interested_in, relationship_intent, confirmed location (location, country, numeric location_data), community_agreed_at. '
...
COMMENT ON FUNCTION public.finalize_onboarding(uuid, jsonb) IS
  ...
  'Required fields: name, birthDate, age>=18, gender, 2+ photos, interestedIn, relationshipIntent, confirmed location (location, country, numeric locationData), communityAgreed. About me optional but >=10 chars when provided.';
