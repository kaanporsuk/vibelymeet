-- Add launch-latency checkpoints emitted by joined Daily prewarm, solo
-- prejoin, and route preloading. This preserves the existing sanitized payload
-- contract and only expands the checkpoint allowlist.
...
COMMENT ON FUNCTION public.record_video_date_launch_latency_checkpoint(uuid, text, jsonb, integer) IS
  ...
  'Allowlist includes Daily prewarm join, solo prejoin, and route preload checkpoints. '
