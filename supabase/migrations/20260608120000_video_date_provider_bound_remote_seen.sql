-- Provider-bound remote-seen recovery.
--
-- The 2026-06-08 production run reached a real Daily date, then accepted a
-- stale client remote-seen stamp after that actor's Daily provider session had
-- already emitted participant.left. Daily alive was already provider-bound; this
-- migration gives remote-seen the same proof envelope and adds a direct JSON
-- shield around daily-alive so stale provider paths cannot leak raw HTTP 500s.

BEGIN;

DO $$
BEGIN
  IF to_regprocedure('public.mark_video_date_daily_alive_20260608120000_provider_remote_seen_base(uuid, text, text, text, text, text)') IS NULL THEN
    IF to_regprocedure('public.mark_video_date_daily_alive(uuid, text, text, text, text, text)') IS NOT NULL THEN
      ALTER FUNCTION public.mark_video_date_daily_alive(uuid, text, text, text, text, text)
        RENAME TO mark_video_date_daily_alive_20260608120000_provider_remote_seen_base;
    END IF;
  END IF;

  IF to_regprocedure('public.mark_video_date_remote_seen_20260608120000_provider_base(uuid)') IS NULL THEN
    IF to_regprocedure('public.mark_video_date_remote_seen(uuid)') IS NOT NULL THEN
      ALTER FUNCTION public.mark_video_date_remote_seen(uuid)
        RENAME TO mark_video_date_remote_seen_20260608120000_provider_base;
    END IF;
  END IF;

  IF to_regprocedure('public.mark_video_date_daily_alive_20260608120000_provider_remote_seen_base(uuid, text, text, text, text, text)') IS NULL THEN
    RAISE EXCEPTION 'missing daily alive provider-bound base';
  END IF;

  IF to_regprocedure('public.mark_video_date_remote_seen_20260608120000_provider_base(uuid)') IS NULL THEN
    RAISE EXCEPTION 'missing remote-seen provider-bound base';
  END IF;
END
$$;

REVOKE ALL ON FUNCTION public.mark_video_date_daily_alive_20260608120000_provider_remote_seen_base(
  uuid, text, text, text, text, text
) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.mark_video_date_daily_alive_20260608120000_provider_remote_seen_base(
  uuid, text, text, text, text, text
) TO service_role;

REVOKE ALL ON FUNCTION public.mark_video_date_remote_seen_20260608120000_provider_base(uuid)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.mark_video_date_remote_seen_20260608120000_provider_base(uuid)
  TO service_role;
...
BEGIN
  ...
  BEGIN
    RETURN public.mark_video_date_daily_alive_20260608120000_provider_remote_seen_base(
      p_session_id,
      p_owner_id,
      p_call_instance_id,
