-- Discover: hide events as soon as actual end passes (no post-end grace window).
-- Preserves scope, premium browse, cancelled exclusion, and location semantics.
...
COMMENT ON FUNCTION public.get_visible_events IS
  ...
  'Rows at or past effective end are hidden (effective_end = COALESCE(ended_at, scheduled end)); no post-end grace. '
