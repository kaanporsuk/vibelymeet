-- Name polish for prepare-entry timing checkpoint wrapper.
--
-- The 20260506101000 wrapper is functional, but PostgreSQL truncates function
-- identifiers past 63 bytes. Rename the delegated base function to a deliberate
-- short internal name and repoint the public wrapper.

ALTER FUNCTION public.record_video_date_launch_latency_checkpoint_20260506101000_prep(uuid, text, jsonb, integer)
  RENAME TO record_vd_launch_latency_202605061020_base;
...
COMMENT ON FUNCTION public.record_vd_launch_latency_202605061020_base(uuid, text, jsonb, integer) IS
  'Internal base launch-latency checkpoint ingestion function retained for the prepare-entry timing wrapper.';
