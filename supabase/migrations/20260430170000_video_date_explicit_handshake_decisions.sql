-- Makes video-date handshake decisions explicit.
-- Canonical semantics:
--   participant_*_liked = NULL means undecided, TRUE means explicit Vibe, FALSE means explicit Pass.
--   participant_*_decided_at = non-null means that boolean is an explicit user decision.
-- Expiry must not coerce undecided NULL values to FALSE.
ALTER TABLE public.video_sessions
  ADD COLUMN IF NOT EXISTS participant_1_decided_at timestamptz,
  ADD COLUMN IF NOT EXISTS participant_2_decided_at timestamptz;

UPDATE public.video_sessions
SET participant_1_decided_at = COALESCE(participant_1_decided_at, state_updated_at, started_at)
WHERE participant_1_liked IS TRUE
  AND participant_1_decided_at IS NULL;

UPDATE public.video_sessions
SET participant_2_decided_at = COALESCE(participant_2_decided_at, state_updated_at, started_at)
WHERE participant_2_liked IS TRUE
  AND participant_2_decided_at IS NULL;

COMMENT ON COLUMN public.video_sessions.participant_1_decided_at IS
  'Explicit handshake decision timestamp. Non-null means participant_1_liked is an explicit Vibe/Pass; null means undecided.';

COMMENT ON COLUMN public.video_sessions.participant_2_decided_at IS
  'Explicit handshake decision timestamp. Non-null means participant_2_liked is an explicit Vibe/Pass; null means undecided.';

-- Extends complete_handshake post-deadline grace when both users have already joined the Daily room
-- (participant_*_joined_at set by mark_video_date_daily_joined), reducing premature
-- handshake_grace_expired while WebRTC/media is live but mutual vibe may still be settling.
--
-- expire_stale_video_date_phases: defers grace-based expiry briefly for the one-sided-vibe
-- + both-joined case, capped by handshake_started_at + 120s so sessions still terminate.
...
BEGIN
  ...
  IF p_action = 'complete_handshake' THEN
    ...
    IF v_session.handshake_grace_expires_at IS NULL THEN
      ...
      IF v_session.participant_1_joined_at IS NOT NULL AND v_session.participant_2_joined_at IS NOT NULL THEN
        v_grace_expires_at := v_now + interval '60 seconds';
      END IF;
...
COMMENT ON FUNCTION public.video_date_transition(uuid, text, text) IS
  'Canonical participant-owned video date state machine. Handshake completion uses a lazy grace window (15s default, 60s when both Daily joins are stamped) after the visible handshake timer so mutual vibe can settle while media is connected.';
  ...
  FOR r IN
    ...
    WHERE ended_at IS NULL
      ...
      AND NOT (
        ...
        AND (
          (participant_1_decided_at IS NOT NULL AND participant_2_decided_at IS NULL)
          OR (participant_1_decided_at IS NULL AND participant_2_decided_at IS NOT NULL)
        )
        AND handshake_started_at + interval '120 seconds' > v_now
      )
    ORDER BY id
...
COMMENT ON FUNCTION public.expire_stale_video_date_phases() IS
  'Ends stale handshake sessions (standard timeout or elapsed handshake grace), and stale date sessions. Skips active reconnect-grace windows. Defers grace-based handshake expiry briefly when both Daily joins are stamped and exactly one vibe is recorded, capped at handshake_started_at + 120s.';
