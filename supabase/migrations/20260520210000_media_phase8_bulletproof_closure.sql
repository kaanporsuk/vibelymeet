-- Phase 8 bulletproof closure: enforce private profile Vibe Video signed playback.
--
-- Non-discoverable profile viewers must not receive the raw Bunny Stream UID
-- when signed playback is required. The durable playback ref is the only client
-- handle for those views; get-chat-media-url re-checks access and signs HLS.
-- Ready public/self/admin profile videos also receive the ref so clients can use
-- signed playback when Bunny token authentication is enabled without breaking
-- the existing raw UID/status compatibility contract.
...
COMMENT ON FUNCTION public.get_profile_for_viewer(uuid) IS
  'Canonical safe other-user profile read. Direct public.profiles reads are self-only for normal clients; use this RPC for established/shared-event/admin profile display without private PII. When private profile Vibe Video signed playback is required, bunny_video_uid/status are masked and vibe_video_playback_ref is the only client playback handle. Ready public/self/admin profile videos may also include the ref so signed playback can be used when Bunny token authentication is enabled.';
