...
COMMENT ON FUNCTION public.get_profile_for_viewer(uuid) IS
  'Safe profile read for app surfaces. Allows self, admin, established relationships, and eligible shared-event discovery; masks events_attended and returns only safe profile display fields plus updated_at for media freshness, backend-computed coarse distance_label, and zodiac, never private PII, birth_date, location_data, or raw coordinates.';
