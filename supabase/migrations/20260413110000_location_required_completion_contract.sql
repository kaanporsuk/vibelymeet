-- Tighten the backend completion contract so location is required now that
-- the web/native location step UX has been hardened.
--
-- "Missing location" means the persisted/display location string is NULL or
-- trims to empty. This matches the current onboarding data shape and the
-- product flow, which already blocks progression until a city is selected.
--
-- Changes:
-- 1. complete_onboarding: rejects completion when profiles.location is blank
-- 2. finalize_onboarding: rejects completion when p_final_data.location is blank
...
BEGIN
  ...
  IF v_profile.location IS NULL OR trim(v_profile.location) = '' THEN
    v_errors := array_append(v_errors, 'Location is required');
  END IF;
...
COMMENT ON FUNCTION public.complete_onboarding IS
  ...
  'Required: name, birth_date, age>=18, gender, 2+ photos, interested_in, relationship_intent, location, community_agreed_at. '
...
COMMENT ON FUNCTION public.finalize_onboarding(uuid, jsonb) IS
  ...
  'Required fields: name, birthDate, age>=18, gender, 2+ photos, interestedIn, relationshipIntent, location, communityAgreed. About me optional but >=10 chars when provided.';
