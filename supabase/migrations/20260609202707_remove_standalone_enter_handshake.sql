...
ALTER FUNCTION public.video_date_transition(uuid, text, text)
  RENAME TO vd_transition_20260609202707_enter_hs_base;

REVOKE ALL ON FUNCTION public.vd_transition_20260609202707_enter_hs_base(uuid, text, text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.vd_transition_20260609202707_enter_hs_base(uuid, text, text)
  TO service_role;
...
COMMENT ON FUNCTION public.video_date_transition(uuid, text, text) IS
  'Video Date lifecycle transition with standalone enter_handshake removed. Clients must use prepare_date_entry/prepare_entry for date entry; prepare_entry, end, and other lifecycle actions delegate through the existing hot-path no-throw stack.';
