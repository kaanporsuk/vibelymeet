-- Vibely Video Date Sprint 7:
-- safety, privacy, RLS, and operator health surface.
--
-- Additive only:
-- - Creates a service-role-only JSON health RPC for safety/ops dashboards.
-- - Stores and returns only counts, enum-like reasons, IDs already present in
--   private operator tables, and timestamps. No Daily tokens, provider secrets,
...
CREATE INDEX IF NOT EXISTS idx_date_feedback_sprint7_session_user
  ON public.date_feedback(session_id, user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_user_reports_sprint7_pair_recent
  ON public.user_reports(reporter_id, reported_id, created_at DESC);
...
BEGIN
    ...
    SELECT
      ...
      count(*) FILTER (
        WHERE sw.ended_at IS NULL
          ...
          AND COALESCE(sw.ready_gate_expires_at, sw.state_updated_at, sw.started_at) < now() - interval '2 minutes'
      )::integer AS stuck_ready_gate_count,
      count(*) FILTER (
        WHERE sw.ended_at IS NULL
          AND sw.phase = 'handshake'
          AND COALESCE(sw.handshake_started_at, sw.state_updated_at, sw.started_at) < now() - interval '2 minutes'
  ...
  event_rollup AS (
    ...
    FROM windows w
    LEFT JOIN public.event_loop_observability_events eo
      ON eo.created_at >= now() - w.window_interval
  ...
  safety_rollup AS (
    SELECT
      w.window_id,
      count(DISTINCT ur.id)::integer AS report_count,
      count(DISTINCT ur.id) FILTER (WHERE ur.status = 'pending')::integer AS pending_report_count,
      count(DISTINCT ur.id) FILTER (WHERE COALESCE(ur.also_blocked, false))::integer AS report_with_block_count,
      count(DISTINCT bu.id)::integer AS block_count
    FROM windows w
    LEFT JOIN public.user_reports ur
      ON ur.created_at >= now() - w.window_interval
     AND (
       p_event_id IS NULL
       OR EXISTS (
         SELECT 1
    ...
    LEFT JOIN public.blocked_users bu
      ON bu.created_at >= now() - w.window_interval
     AND (
       p_event_id IS NULL
       OR EXISTS (
         SELECT 1
  ...
  orphan_rollup AS (
    SELECT
      w.window_id,
      count(o.id)::integer AS orphan_room_cleanup_rows,
      count(o.id) FILTER (WHERE o.action = 'delete_failed')::integer AS orphan_room_cleanup_failed_count,
      count(o.id) FILTER (WHERE o.action IN ('delete_candidate', 'deleted', 'dry_run_delete'))::integer AS orphan_room_destructive_candidate_count,
      count(o.id) FILTER (WHERE o.action = 'skipped_safety_review')::integer AS orphan_room_safety_interlock_skip_count
    FROM windows w
    LEFT JOIN public.video_date_orphan_room_cleanup_audit o
      ON o.created_at >= now() - w.window_interval
     AND (
       p_event_id IS NULL
       OR EXISTS (
         SELECT 1
         ...
         WHERE vs.event_id = p_event_id
           AND (
             ...
             OR (
               ...
               AND vs.daily_room_name IS NOT NULL
               AND vs.daily_room_name = o.room_name
             )
           )
       )
     )
  ...
  queue_rollup AS (
    SELECT
      w.window_id,
      count(*) FILTER (
        WHERE m.metric_stream = 'drain_rpc_outer'
          AND m.reason_code IN ('no_queued_session', 'session_not_promotable', 'queued_session_not_promotable')
      )::integer AS queue_drain_no_match_count,
      count(*) FILTER (
        WHERE m.metric_stream = 'drain_rpc_outer'
          AND m.outcome NOT IN ('success', 'no_op')
      )::integer AS queue_drain_failure_count
    FROM windows w
    LEFT JOIN public.v_event_loop_observability_metric_streams m
      ON m.created_at >= now() - w.window_interval
...
COMMENT ON FUNCTION public.get_video_date_sprint7_ops_health(uuid) IS
  'Service-role-only Sprint 7 Video Date safety/privacy/ops dashboard payload. Reports stuck states, safety action counts, queue drain misses, Daily join/prepare failures, webhook DLQ, and orphan-room cleanup health without exposing private participant content or provider secrets.';
...
VALUES (
  '20260525235000',
  'Video date Sprint 7 safety/privacy ops health',
  'schema+policy',
  'Additive service-role-only health RPC and supporting indexes for safety, privacy, RLS, and operator dashboards. No client grants; no private payload fields returned.',
  false
)
