-- RPC called by both web and native at the end of onboarding.
-- Validates required fields, sets onboarding_complete = true, sets onboarding_stage = 'complete'.
-- Uses SET LOCAL ROLE postgres for the UPDATE so protect_sensitive_profile_columns allows the change.
...
BEGIN
  ...
  IF v_profile.about_me IS NULL OR length(trim(v_profile.about_me)) < 10 THEN
...
COMMENT ON FUNCTION public.complete_onboarding IS 'Validates required onboarding fields and marks the profile as onboarding-complete. Called by both web and native at end of onboarding flow.';
