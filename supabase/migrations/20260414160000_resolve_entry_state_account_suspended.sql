-- Account suspension gate in resolve_entry_state().
--
-- Suspended profiles (moderation) must not receive route_hint "app" or complete state,
-- even when onboarding_complete = true. Clients route to entry recovery with state
-- account_suspended (same shell as other recovery paths).
--
-- Preserves phone-normalization and fragmentation logic from 20260414100000; adds one
-- early return before the onboarding_complete shortcut.
...
BEGIN
  ...
  IF v_has_profile AND COALESCE(v_profile.is_suspended, false) = true THEN
    RETURN jsonb_build_object(
...
COMMENT ON FUNCTION public.resolve_entry_state IS
  'Shared entry-state resolver for authenticated users. Routes to app, onboarding, or entry recovery; returns account_suspended + entry_recovery when profiles.is_suspended. Does not expose foreign account identifiers or raw contact data.';
