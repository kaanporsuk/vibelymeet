...
import { mapBunnyStatusToChatClipStatus, updateChatVibeClipStatusByProvider } from "../_shared/chat-vibe-clips.ts";
...
const SENTRY_FLUSH_TIMEOUT_MS = Math.max(0, Math.round(numericEnv("SENTRY_FLUSH_TIMEOUT_MS", 500)));
...
if (SENTRY_DSN) {
  Sentry.init({
    ...
    environment: Deno.env.get("SENTRY_ENVIRONMENT")?.trim() || Deno.env.get("SUPABASE_ENV")?.trim() || "supabase-edge",
  });
}

function sanitizeTraceFields(fields: SafeTraceFields = {}): Record<string, string | number | boolean | null> {
...
async function flushSentryQuietly(): Promise<void> {
  if (!SENTRY_DSN || SENTRY_FLUSH_TIMEOUT_MS <= 0) return;
  try {
    await Sentry.flush(SENTRY_FLUSH_TIMEOUT_MS);
  } catch {
    // Observability must never block the webhook response.
  }
}

async function withSentryWebhookTransaction<T>(fields: SafeTraceFields, callback: () => Promise<T>): Promise<T> {
  if (!SENTRY_DSN || typeof Sentry.startSpan !== "function") {
  ...
  return await Sentry.startSpan(
    ...
    async () => {
      try {
        ...
        await flushSentryQuietly();
      }
    },
  );
}

function captureSentryWebhookException(error: unknown, fields: SafeTraceFields): void {
  if (!SENTRY_DSN) return;
  Sentry.captureException(error instanceof Error ? error : new Error("video_webhook_non_error_throw"), {
    tags: {
      "supabase.function": "video-webhook",
      scope: "chat_vibe_clip_upload",
    },
    extra: sanitizeTraceFields(fields),
  });
}
...
async function authenticateWebhook(
  ...
  if (hasSignatureHeaders && signatureKeyConfigured) {
    const signatureResult = await verifyBunnyStreamWebhookSignature(req.headers, rawBody, webhookSigningKey);
  ...
  if (bearerToken && hasWebhookTokenConfigured && constantTimeCompare(bearerToken, webhookToken)) {
    return {
      ok: true,
      authMode: "bearer",
      signatureKeyConfigured,
  ...
  if (legacyToken && hasWebhookTokenConfigured && constantTimeCompare(legacyToken, webhookToken)) {
    return {
      ok: true,
...
serve(async (req) => {
  ...
  return await withSentryWebhookTransaction(
    {
      project_ref: projectRef,
      webhook_trace_id: webhookTraceId,
      method: req.method,
    ...
    async () => {
      ...
      const authResult = await authenticateWebhook(req, rawBody, webhookToken, webhookSigningKey);
      if (!authResult.ok) {
        logWebhook("warn", "video_webhook_rejected", {
      ...
      try {
        ...
        if (chatClipLookupError) {
          ...
          logWebhook("info", "video_webhook_chat_vibe_clip_matched", {
            ...
            client_request_id:
              typeof chatClipUploadForLog.client_request_id === "string"
            ...
            media_asset_id:
              typeof chatClipUploadForLog.media_asset_id === "string" ? chatClipUploadForLog.media_asset_id : null,
            previous_status: typeof chatClipUploadForLog.status === "string" ? chatClipUploadForLog.status : null,
            published_message_id:
              typeof chatClipUploadForLog.published_message_id === "string"
        ...
        if (chatClipResult.handled) {
          ...
          return new Response(chatClipResult.error ? "error" : "ok", {
            status: chatClipResult.error ? 500 : 200,
          });
        }
        ...
        const { data: sessionResult, error: sessionError } = await supabase.rpc("update_media_session_status", {
        ...
        captureSentryWebhookException(err, {
          project_ref: projectRef,
          webhook_trace_id: webhookTraceId,
          elapsed_ms: Date.now() - startedAt,
        });
