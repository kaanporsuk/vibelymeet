CREATE OR REPLACE FUNCTION private_video_date.vdt_pre_date_end_cleanup(p_session_id uuid, p_action text, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  IF p_action IS DISTINCT FROM 'end' THEN
    RETURN private_video_date.vdt_core_legacy_01(
      p_session_id,
      p_action,
      p_reason
    );
