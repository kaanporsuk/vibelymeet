CREATE OR REPLACE FUNCTION private_video_date.vdt_peer_missing_end(p_session_id uuid, p_action text, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  IF p_action IS DISTINCT FROM 'prepare_entry' THEN
    RETURN private_video_date.vdt_provider_atomic_entry(
      p_session_id,
      p_action,
      p_reason
    );
