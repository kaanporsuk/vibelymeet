CREATE OR REPLACE FUNCTION private_video_date.vdt_event_inactive(p_session_id uuid, p_action text, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_action IS DISTINCT FROM 'end'
    ...
    RETURN private_video_date.vdt_peer_missing_end(
      p_session_id,
      p_action,
      p_reason
    );
