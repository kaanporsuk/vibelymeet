CREATE OR REPLACE FUNCTION private_video_date.vdt_prepare_entry_prewarm(p_session_id uuid, p_action text, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  IF p_action IS DISTINCT FROM 'prepare_entry' THEN
    RETURN private_video_date.vdt_pre_date_end_cleanup(
      p_session_id,
      p_action,
      p_reason
    );
