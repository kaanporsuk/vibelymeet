...
Current Event Lobby leaner-path cleanup, 2026-06-09: direct legacy queue/session RPCs `find_video_date_match(uuid,uuid)` and `join_matching_queue(uuid,uuid)` are removed from the active backend contract by migration `20260609163130_remove_legacy_queue_session_rpcs.sql`, and the remaining legacy cleanup RPC `leave_matching_queue(uuid)` is removed by migration `20260609165218_remove_leave_matching_queue.sql`. Current clients must use `/event/:eventId/lobby` deck/swipe through `swipe-actions`, reciprocal swipe or supported queue promotion, Ready Gate, then Video Date. Generated Supabase types and active validation must assert all three deprecated RPCs are absent. Keep `drain_match_queue`, `promote_ready_gate_if_eligible`, Ready Gate, Video Date state-machine behavior, and `video_sessions.session_source` intact.
