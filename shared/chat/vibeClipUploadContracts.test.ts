...
test("video bubbles remain adaptive and full-width across web and native chat", () => {
  ...
  assert.match(webOutboxContext, /if \(syncedStatus === "ready"\) \{[\s\S]{0,120}selfHealedCount \+= 1;[\s\S]{0,80}continue;/);
  ...
  assert.match(nativeOutboxContext, /if \(syncedStatus === 'ready'\) \{[\s\S]{0,120}selfHealedCount \+= 1;[\s\S]{0,80}continue;/);
