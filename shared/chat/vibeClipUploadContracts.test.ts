...
test("video bubbles remain adaptive and full-width across web and native chat", () => {
  ...
  assert.match(uploadAttentionTargets, /attentionId: `local:\$\{item\.id\}`/);
  assert.match(uploadAttentionTargets, /attentionId: `server:\$\{upload\.id\}`/);
  ...
  assert.match(webApp, /uploadAttentionTargetIdentity\(primaryTarget\)/);
  assert.match(webApp, /setHiddenAttentionTarget/);
  assert.doesNotMatch(webApp, /setHiddenAttentionKey/);
  assert.match(webApp, /uploadAttention: primaryTarget\.attentionId/);
  assert.match(webApp, /uploadAttentionNonce: String\(Date\.now\(\)\)/);
  assert.match(webApp, /View the stuck upload in chat/);
  ...
  assert.match(webChat, /uploadAttentionClientRequestSelector\(localClientRequestId\)/);
  ...
  assert.match(nativeLayout, /uploadAttentionTargetIdentity\(primaryTarget\)/);
  assert.match(nativeLayout, /setHiddenAttentionTarget/);
  assert.doesNotMatch(nativeLayout, /setHiddenAttentionKey/);
  assert.match(nativeLayout, /uploadAttention: primaryTarget\.attentionId/);
  assert.match(nativeLayout, /uploadAttentionNonce: String\(Date\.now\(\)\)/);
  assert.match(nativeLayout, /View the stuck upload in chat/);
  ...
  assert.match(nativeChat, /router\.setParams\(\{[\s\S]{0,120}uploadAttention: undefined/);
  assert.match(nativeChat, /uploadAttentionNonce: undefined/);
  assert.match(nativeChat, /threadMessageClientRequestId\(row\.message\) === localClientRequestId/);
