...
test("admin events panel includes lightweight category manager", () => {
  ...
  const tableIndex = adminPanel.indexOf("{/* Table */}");
  const filterIndex = adminPanel.indexOf("{/* Filter bar */}");
  const categoryManagerRenderIndex = adminPanel.indexOf("<AdminEventCategoryManager />");
  assert.ok(tableIndex !== -1, "admin events table section should exist");
  assert.ok(filterIndex !== -1, "admin events filter section should exist");
  assert.ok(categoryManagerRenderIndex > tableIndex, "category manager should render after the events table");
  assert.ok(categoryManagerRenderIndex > filterIndex, "category manager should render after event filters");
  assert.match(adminPanel, /const \[isOpen, setIsOpen\] = useState\(false\)/);
  assert.match(adminPanel, /aria-expanded=\{isOpen\}/);
  assert.match(adminPanel, /Show categories/);
  assert.match(adminPanel, /Hide categories/);
