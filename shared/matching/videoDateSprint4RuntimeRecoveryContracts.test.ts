...
test("Sprint 4 web terminal paths cannot open post-date survey from local optimism", () => {
  ...
  assert.match(webDate, /fetchVideoDateSnapshot\(id, \{ includeToken: false \}\)/);
...
test("Sprint 4 native terminal paths cannot grant survey without terminal DB truth", () => {
  ...
  assert.match(nativeDate, /openNativePostDateSurveyFromTerminalTruth\(attemptSource, sessionRow\)/);
...
test("Sprint 4 Daily joined confirmation failures do not create or block survey eligibility", () => {
  const webJoinedBlock = sourceBlock(webCall, "void markDailyJoinedWithBackoff({");
  ...
  const nativeJoinedBlock = sourceBlock(nativeDate, "void markDailyJoinedWithBackoff({");
