import test from "node:test";
import assert from "node:assert/strict";
import {
  completeEntryExpectation,
  entryDecisionFailureIndicatesSessionEnded,
  entryRpcIndicatesTerminalDecisionRejection,
  entryTruthIndicatesEndedSession,
  persistEntryDecisionWithVerification,
  type VideoDateEntryTruth,
} from "./videoDateEntryPersistence";

const baseTruth: VideoDateEntryTruth = {
  id: "session-1",
  participant_1_id: "user-a",
  participant_2_id: "user-b",
  participant_1_joined_at: "2026-04-24T06:02:00.000Z",
  participant_2_joined_at: "2026-04-24T06:02:01.000Z",
  participant_1_liked: null,
  participant_2_liked: null,
  participant_1_decided_at: null,
  participant_2_decided_at: null,
  state: "entry",
  phase: "entry",
  ended_at: null,
  ended_reason: null,
  entry_grace_expires_at: null,
};

test("vibe persistence succeeds only when the actor decision is present in DB truth", async () => {
  const result = await persistEntryDecisionWithVerification({
    sessionId: "session-1",
    actorUserId: "user-a",
    action: "vibe",
    retryDelaysMs: [],
    rpc: async () => ({ data: { success: true, state: "entry" }, error: null }),
    fetchTruth: async () => ({
      truth: {
        ...baseTruth,
        participant_1_liked: true,
        participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      },
    }),
  });

  assert.equal(result.ok, true);
  assert.equal(result.actorDecisionPersisted, true);
  assert.equal(result.actorDecisionSlot, "participant_1_liked");
  assert.equal(result.actorDecisionTimestampSlot, "participant_1_decided_at");
  assert.equal(result.persistedDecision, true);
  assert.equal(result.persistedDecisionAt, "2026-04-24T06:02:02.000Z");
  assert.equal(result.attempts, 1);
});

test("RPC success with the actor decision still null retries and then surfaces failure", async () => {
  let rpcCalls = 0;
  const result = await persistEntryDecisionWithVerification({
    sessionId: "session-1",
    actorUserId: "user-a",
    action: "vibe",
    retryDelaysMs: [0],
    sleep: async () => undefined,
    rpc: async () => {
      rpcCalls += 1;
      return { data: { success: true, state: "entry" }, error: null };
    },
    fetchTruth: async () => ({ truth: baseTruth }),
  });

  assert.equal(rpcCalls, 2);
  assert.equal(result.ok, false);
  assert.equal(result.reason, "decision_not_persisted");
  assert.equal(result.actorDecisionPersisted, false);
  assert.equal(result.persistedDecision, null);
});

test("transient RPC failure is retried before acknowledging Vibe", async () => {
  let rpcCalls = 0;
  const result = await persistEntryDecisionWithVerification({
    sessionId: "session-1",
    actorUserId: "user-a",
    action: "vibe",
    retryDelaysMs: [0],
    sleep: async () => undefined,
    rpc: async () => {
      rpcCalls += 1;
      if (rpcCalls === 1) {
        return { data: null, error: { message: "Failed to fetch" } };
      }
      return { data: { success: true, state: "entry" }, error: null };
    },
    fetchTruth: async () => ({
      truth: {
        ...baseTruth,
        participant_1_liked: true,
        participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      },
    }),
  });

  assert.equal(rpcCalls, 2);
  assert.equal(result.ok, true);
  assert.equal(result.attempts, 2);
});

test("retryable fail-soft transition payload is retried before acknowledging Vibe", async () => {
  let rpcCalls = 0;
  let truthCalls = 0;
  const result = await persistEntryDecisionWithVerification({
    sessionId: "session-1",
    actorUserId: "user-a",
    action: "vibe",
    retryDelaysMs: [0],
    sleep: async () => undefined,
    rpc: async () => {
      rpcCalls += 1;
      if (rpcCalls === 1) {
        return {
          data: {
            success: false,
            code: "VIDEO_DATE_TRANSITION_FAILED",
            error: "video_date_transition_failed",
            retryable: true,
          },
          error: null,
        };
      }
      return { data: { success: true, state: "entry" }, error: null };
    },
    fetchTruth: async () => {
      truthCalls += 1;
      return {
        truth: truthCalls === 1
          ? baseTruth
          : {
              ...baseTruth,
              participant_1_liked: true,
              participant_1_decided_at: "2026-04-24T06:02:02.000Z",
            },
      };
    },
  });

  assert.equal(rpcCalls, 2);
  assert.equal(truthCalls, 2);
  assert.equal(result.ok, true);
  assert.equal(result.attempts, 2);
});

test("persisted truth wins when RPC payload reports failure after saving Vibe", async () => {
  const result = await persistEntryDecisionWithVerification({
    sessionId: "session-1",
    actorUserId: "user-a",
    action: "vibe",
    retryDelaysMs: [],
    rpc: async () => ({
      data: {
        success: false,
        code: "VIDEO_DATE_TRANSITION_FAILED",
        error: "video_date_transition_failed",
        retryable: true,
      },
      error: null,
    }),
    fetchTruth: async () => ({
      truth: {
        ...baseTruth,
        participant_1_liked: true,
        participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      },
    }),
  });

  assert.equal(result.ok, true);
  assert.equal(result.actorDecisionPersisted, true);
  assert.equal(result.persistedDecision, true);
  assert.equal(result.attempts, 1);
});

test("one vibe and partner undecided ends at the hard deadline", () => {
  assert.deepEqual(
    completeEntryExpectation({
      ...baseTruth,
      participant_1_liked: true,
      participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      participant_2_liked: null,
    }),
    {
      kind: "ended_timeout",
      waitingForParticipant1: false,
      waitingForParticipant2: true,
    },
  );
});

test("both liked true proceeds to date", () => {
  assert.deepEqual(
    completeEntryExpectation({
      ...baseTruth,
      participant_1_liked: true,
      participant_2_liked: true,
      participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      participant_2_decided_at: "2026-04-24T06:02:03.000Z",
    }),
    { kind: "date" },
  );
});

test("one explicit pass ends as non-mutual", () => {
  assert.deepEqual(
    completeEntryExpectation({
      ...baseTruth,
      participant_1_liked: true,
      participant_2_liked: false,
      participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      participant_2_decided_at: "2026-04-24T06:02:03.000Z",
    }),
    { kind: "ended_non_mutual" },
  );
});

test("boolean false without decided_at is still undecided → hard-deadline timeout", () => {
  assert.deepEqual(
    completeEntryExpectation({
      ...baseTruth,
      participant_1_liked: false,
      participant_2_liked: false,
    }),
    {
      kind: "ended_timeout",
      waitingForParticipant1: true,
      waitingForParticipant2: true,
    },
  );
});

test("both undecided ends at the hard deadline", () => {
  assert.deepEqual(completeEntryExpectation({ ...baseTruth }), {
    kind: "ended_timeout",
    waitingForParticipant1: true,
    waitingForParticipant2: true,
  });
});

test("explicit pass and partner undecided → terminal, no grace", () => {
  assert.deepEqual(
    completeEntryExpectation({
      ...baseTruth,
      participant_1_liked: false,
      participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      participant_2_liked: null,
    }),
    { kind: "ended_non_mutual" },
  );
});

test("explicit pass and partner vibe → terminal, no grace", () => {
  assert.deepEqual(
    completeEntryExpectation({
      ...baseTruth,
      participant_1_liked: false,
      participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      participant_2_liked: true,
      participant_2_decided_at: "2026-04-24T06:02:03.000Z",
    }),
    { kind: "ended_non_mutual" },
  );
});

test("both explicit pass → terminal, no grace", () => {
  assert.deepEqual(
    completeEntryExpectation({
      ...baseTruth,
      participant_1_liked: false,
      participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      participant_2_liked: false,
      participant_2_decided_at: "2026-04-24T06:02:03.000Z",
    }),
    { kind: "ended_non_mutual" },
  );
});

test("explicit pass persistence succeeds when false and decided_at are both stored", async () => {
  const result = await persistEntryDecisionWithVerification({
    sessionId: "session-1",
    actorUserId: "user-a",
    action: "pass",
    retryDelaysMs: [],
    rpc: async () => ({ data: { success: true, state: "entry" }, error: null }),
    fetchTruth: async () => ({
      truth: {
        ...baseTruth,
        participant_1_liked: false,
        participant_1_decided_at: "2026-04-24T06:02:02.000Z",
      },
    }),
  });

  assert.equal(result.ok, true);
  assert.equal(result.persistedDecision, false);
});

test("entryTruthIndicatesEndedSession", () => {
  assert.equal(entryTruthIndicatesEndedSession(null), false);
  assert.equal(entryTruthIndicatesEndedSession({ ...baseTruth }), false);
  assert.equal(
    entryTruthIndicatesEndedSession({ ...baseTruth, ended_at: "2026-04-24T06:10:00.000Z" }),
    true,
  );
  assert.equal(entryTruthIndicatesEndedSession({ ...baseTruth, state: "ended" }), true);
  assert.equal(entryTruthIndicatesEndedSession({ ...baseTruth, phase: "ended" }), true);
});

test("entryRpcIndicatesTerminalDecisionRejection", () => {
  assert.equal(entryRpcIndicatesTerminalDecisionRejection(null), false);
  assert.equal(entryRpcIndicatesTerminalDecisionRejection({ success: false }), false);
  assert.equal(
    entryRpcIndicatesTerminalDecisionRejection({ success: false, code: "GRACE_EXPIRED" }),
    true,
  );
  assert.equal(
    entryRpcIndicatesTerminalDecisionRejection({ success: false, code: "SESSION_ENDED" }),
    true,
  );
  assert.equal(entryRpcIndicatesTerminalDecisionRejection({ state: "ended" }), true);
});

test("entryDecisionFailureIndicatesSessionEnded combines truth and payload", () => {
  assert.equal(
    entryDecisionFailureIndicatesSessionEnded({
      truth: { ...baseTruth, ended_at: "2026-04-24T06:10:00.000Z" },
      rpcPayload: null,
    }),
    true,
  );
  assert.equal(
    entryDecisionFailureIndicatesSessionEnded({
      truth: baseTruth,
      rpcPayload: { success: false, code: "GRACE_EXPIRED" },
    }),
    true,
  );
  assert.equal(
    entryDecisionFailureIndicatesSessionEnded({
      truth: baseTruth,
      rpcPayload: { success: true, state: "entry" },
    }),
    false,
  );
});

test("persistEntryDecisionWithVerification: rpc_error returns fetched truth when prior truth was null", async () => {
  let fetchCount = 0;
  const result = await persistEntryDecisionWithVerification({
    sessionId: "session-1",
    actorUserId: "user-a",
    action: "vibe",
    retryDelaysMs: [],
    rpc: async () => ({
      data: null,
      error: { code: "PGRST301", message: "JSON object requested, multiple (or no) rows returned", name: "Error" },
    }),
    fetchTruth: async () => {
      fetchCount += 1;
      return {
        truth: {
          ...baseTruth,
          ended_at: "2026-04-24T06:10:00.000Z",
          state: "ended",
          phase: "ended",
        },
      };
    },
  });

  assert.equal(result.ok, false);
  assert.equal(result.reason, "rpc_error");
  assert.equal(fetchCount >= 1, true);
  assert.equal(entryTruthIndicatesEndedSession(result.truth), true);
});
