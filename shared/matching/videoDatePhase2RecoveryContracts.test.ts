...
test("PR 2.5 Daily webhook endpoint verifies signatures and writes an idempotent ledger", () => {
  ...
  assert.match(dailyWebhook, /function base64ToBytes/);
  assert.match(dailyWebhook, /function hmacSha256Base64/);
  assert.match(dailyWebhook, /const webhookSecretBytes = base64ToBytes\(webhookSecret\)/);
  assert.match(dailyWebhook, /`\$\{timestampHeader\}\.\$\{rawBody\}`/);
  assert.match(dailyWebhook, /req\.headers\.get\(SIGNATURE_HEADER\)\?\.trim\(\)/);
  assert.match(dailyWebhook, /safeEqual\(received, expected\)/);
  assert.doesNotMatch(dailyWebhook, /SIGNATURE_VERSION|v0=|`\$\{SIGNATURE_VERSION\}:\$\{timestampHeader\}:\$\{rawBody\}`|hmacSha256Hex|signatureFromHeader/);
  assert.match(dailyWebhook, /const rawBody = await req\.text\(\)/);
  assert.match(dailyWebhook, /JSON\.parse\(rawBody\)/);
  assert.match(dailyWebhook, /function isDailyVerificationProbe/);
  assert.match(dailyWebhook, /payload\.test === "test"/);
  assert.match(dailyWebhook, /return json\(\{ ok: true, test: true \}\)/);
