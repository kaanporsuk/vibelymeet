...
test("web and native reject cached prewarmed token after Daily join failure and retry prepare", () => {
  ...
  assert.match(
    ...
    /rejectPreparedVideoDateEntry\(\s*sessionId,\s*user\.id,\s*["']daily_join_failed["'],\s*eventId \|\| null/,
  );
...
test("native date entry reuses same-session Daily joins across remounts and rescue timers", () => {
  assert.match(
    nativeVideoDateRoute,
    /type SharedDailyCallEntryState =[\s\S]{0,160}\| ["']creating["'][\s\S]{0,80}\| ["']joining["'][\s\S]{0,80}\| ["']joined["'][\s\S]{0,80}\| ["']failed["'][\s\S]{0,80}\| ["']leaving["']/,
  );
  ...
  assert.match(
    nativeVideoDateRoute,
    /showJoiningOverlay =[\s\S]*\(joining \|\| isConnecting\)[\s\S]{0,120}!localInDailyRoom[\s\S]{0,120}!showFeedback/,
  );
...
test("native video date capture uses supported Daily defaults while web keeps explicit portrait constraints", () => {
  ...
  assert.match(webVideoCallHook, /releaseAppAcquiredMedia\("daily_room_failed_before_media_preflight"\)/);
  ...
  assert.match(nativeVideoDateRoute, /frame_issue_hint:[\s\S]{0,80}["']remote_layout_contains_sender_frame_without_receiver_crop["']/);
...
test("native date route opens recovered pending surveys after current_room_id is cleared", () => {
  ...
  assert.match(nativeVideoDateRoute, /surveyOpenedRef\.current = true;[\s\S]{0,700}setShowFeedback\(true\);/);
...
test("native terminal route guard keeps an already-open post-date survey mounted", () => {
  const indexOfAny = (source: string, needles: string[], fromIndex = 0) => {
    const hits = needles
...
test("web and native use server-owned leave, reconnect, and permission recovery paths", () => {
  ...
  assert.match(nativeVideoDateRoute, /supabase\.rpc\(\s*["']mark_video_date_daily_joined["']/);
...
test("video date button escape contracts keep web and native users routable", () => {
  ...
  assert.match(nativeVideoDateRoute, /endVideoDate\(\s*sessionId,\s*["']ended_from_client["']/);
  ...
  assert.match(nativeVideoDateRoute, /endVideoDate\(\s*sessionId,\s*["']partial_join_peer_timeout["']/);
...
test("peer-missing user exit preserves the canonical partial-join terminal reason", () => {
  ...
  assert.match(
    webVideoDatePage,
    /handlePreDateExit\(\{[\s\S]{0,80}reason: ["']partial_join_peer_timeout["'],[\s\S]{0,80}source: ["']peer_missing_back_to_lobby["'][\s\S]{0,80}\}\)/,
  );
  assert.match(nativeVideoDateRoute, /function shouldTerminalizeNativePeerMissingAbort/);
  assert.match(
    nativeVideoDateRoute,
    /Boolean\(truth\.participant_1_joined_at\)[\s\S]{0,80}!==[\s\S]{0,80}Boolean\(truth\.participant_2_joined_at\)/,
  );
  assert.match(
    nativeVideoDateRoute,
    /truthFetchFailed[\s\S]{0,80}\|\|[\s\S]{0,80}shouldTerminalizeNativePeerMissingAbort\(truth\)/,
...
test("web video date access recovery covers permission denial and playback-blocked CTAs", () => {
  ...
  assert.match(
    webVideoDatePage,
    /permissionBlock\.recoveryAction === ["']open_settings["'][\s\S]{0,80}\? ["']I updated settings["'][\s\S]{0,80}: ["']Try again["']/,
  );
...
test("web and native stamp bilateral remote-video evidence once remote media is mounted", () => {
  ...
  assert.doesNotMatch(webVideoCallHook, /\.rpc\("mark_video_date_remote_seen", \{ p_session_id: sessionId \}\)/);
  assert.doesNotMatch(webVideoCallHook, /!currentOptions\?\.userId\) return/);
  assert.match(webVideoCallHook, /const currentUserId = currentOptions\?\.userId/);
  ...
  assert.match(
    nativeVideoDateRoute,
    /const activeNativeDailyCallIdentityRef =[\s\S]{0,120}useRef<ActiveNativeDailyCallIdentity \| null>\(null\)/,
  );
  ...
  assert.match(
    nativeVideoDateRoute,
    /remoteSeenLastStampRef\.current = \{[\s\S]{0,80}sessionId,[\s\S]{0,80}stampedAtMs: Date\.now\(\)[\s\S]{0,80}\}/,
  );
  ...
  assert.match(
    nativeVideoDateRoute,
    /const markRemoteSeenOnServerRef = useRef<\(\(source: string\) => void\) \| null>\(\s*null,?\s*\)/,
  );
  assert.doesNotMatch(nativeVideoDateRoute, /markRemoteSeenOnServerRef\.current\?\.\(["']participant_joined["']\)/);
  assert.doesNotMatch(nativeVideoDateRoute, /markRemoteSeenOnServerRef\.current\?\.\(["']participant_updated["']\)/);
  assert.doesNotMatch(nativeVideoDateRoute, /markRemoteSeenOnServerRef\.current\?\.\(["']shared_call_snapshot["']\)/);
  assert.doesNotMatch(nativeVideoDateRoute, /markRemoteSeenOnServerRef\.current\?\.\(["']post_join_snapshot["']\)/);
...
test("video date trace id is propagated through prepare entry analytics and Daily-room payloads", () => {
  ...
  assert.match(
    nativeVideoDateRoute,
    /VIDEO_DATE_DAILY_JOIN_FAILURE[\s\S]*video_date_trace_id:\s*[\s\S]{0,120}preparedEntryAtFailure\?\.value\.video_date_trace_id/,
  );
});
...
test("native AppState background path remains statically covered for away, return, and timeout", () => {
  ...
  assert.match(
    nativeVideoDateRoute,
    /setTimeout\(\(\) => \{[\s\S]*endVideoDate\(\s*sessionId,\s*["']app_background_timeout["'][\s\S]*\}, NATIVE_BACKGROUND_GRACE_MS\)/,
  );
