...
export function sanitizeWebMediaTelemetryProperties(
  ...
  return sanitizeMediaEventProperties(properties, {
    ...options,
    defaults: { platform: "web", ...(options.defaults ?? {}) },
  });
}
...
export function trackMediaTelemetryEvent(
  ...
  const sanitized = sanitizeWebMediaTelemetryProperties(properties, options);
  try {
...
export function addMediaTelemetryBreadcrumb(
  ...
  try {
    Sentry.addBreadcrumb({
      ...
      message,
      level,
      data: sanitizeWebMediaTelemetryProperties(properties, options),
    });
  } catch {
    // Breadcrumb capture is best-effort.
  }
...
export function captureMediaTelemetryException(
  ...
  try {
    Sentry.captureException(error, {
      ...
      extra: sanitizeWebMediaTelemetryProperties(properties, options),
    });
  } catch {
