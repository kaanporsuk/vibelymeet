...
function shouldRetryReadyGateMarkReadyPayload(data: unknown): data is ReadyGateSessionTruth {
  ...
  if (payload.retryable !== true || payload.terminal === true || status === ReadyGateStatus.BothReady) {
    return false;
  }
...
export const useReadyGate = ({ sessionId, eventId, onBothReady, onForfeited }: UseReadyGateOptions) => {
  ...
  const fetchPartnerName = useCallback(async (partnerId: string | null): Promise<string | null> => {
    ...
    try {
      ...
      readyGateDebug("partner profile display lookup threw", {
        sessionId,
        eventId: eventId ?? null,
  ...
  const fetchSession = useCallback(async (): Promise<ReadyGateSyncResult | null> => {
    ...
    if (snapshot.terminal === true || snapshot.retryable === false) {
      return {
        ...
        status: normalizeReadyGateStatus(snapshot.readyGateStatus),
        terminal: snapshot.terminal,
      };
    }
    readyGateDebug("start snapshot unavailable; falling back to raw session truth", {
      sessionId,
      eventId: eventId ?? null,
  ...
  }, [sessionId, eventId, user?.id, applyReadyGateTruth, applyStartSnapshot, fetchPartnerName]);
  ...
  const fetchCanonicalReadyGateSnapshot = useCallback(async (
    ...
    if (snapshot.ok === true) {
      ...
      setState((prev) => ({
        ...
        countdownDegraded: snapshot.phaseDeadlineAt == null,
      }));
  ...
  }, [fetchSession, sessionId, setSequenceGapUnresolved, user?.id]);
  ...
  const runReadyGateTransition = useCallback(async (
    ...
    const callReadyGateTransitionRpc = () => (
      action === "mark_ready"
        ...
        : supabase.rpc("ready_gate_transition", {
            p_session_id: sessionId,
            p_action: action,
          })
    );
  ...
  }, [sessionId, eventId, user?.id, applyReadyGateTruth, applyStartSnapshot]);
