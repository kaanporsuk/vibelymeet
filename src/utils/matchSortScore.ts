 ...
 * Keep in sync with `apps/mobile/lib/matchSortScore.ts`.
 */
...
export type MatchScoreInput = {
  ...
  /** Match is tied to an event (e.g. speed dating). */
...
/** Count of shared vibe labels (case-insensitive, trimmed). */
...
/**
 * Sort key: larger = better. Stable for identical inputs (tie-break with list index in UI).
 * Formula: overlap×1e6 + intent×1e4 + event×1e2
 */
...
/**
 * Display 50–99 for row badge / drawer; derived from the same inputs as `bestMatchSortKey`
 * (monotonic in overlap, intent, event).
 */
...
/** Header pill: "1 match" / "N matches" (includes 0 → "0 matches"). */
