...
interface PushSetupFlowProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onRequestPermission: () => Promise<boolean>;
}

export function PushSetupFlow({
  open,
  onOpenChange,
  onRequestPermission,
}: PushSetupFlowProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogHeader className="sr-only">
          <DialogTitle>Set up push notifications</DialogTitle>
        </DialogHeader>
...
export function PushSetupButton({
  ...
  if (isGranted) {
    return (
      <motion.button
        ...
        aria-label="Notification setup complete"
      >
  ...
  return (
    <motion.button
      whileTap={{ scale: 0.95 }}
      onClick={onClick}
      className="relative p-2 rounded-full bg-amber-400/15 text-amber-300 hover:bg-amber-400/20 transition-colors"
      aria-label="Set up push notifications"
    >
      <Bell className="w-5 h-5" />
      <span className="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full border border-background bg-amber-400 shadow-[0_0_10px_rgba(251,191,36,0.85)]" />
    </motion.button>
  );
}
