...
type SortField = 'name' | 'created_at' | 'age' | 'location';
...
const AdminUsersPanel = () => {
  ...
  const { data: users, isLoading } = useQuery({
    queryKey: ['admin-users', searchQuery, genderFilter, sortField, sortDirection],
    queryFn: async () => {
      let query = supabase
        .from('profiles')
        .select(`
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 border-2 border-border">
                          <AvatarImage src={user.avatar_url || user.photos?.[0]} />
