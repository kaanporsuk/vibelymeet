...
Current recovery overlay: PR #1194 is merged with squash commit `0a160cd975d87cd756e9c399e748810508f005cb`; it builds on PR #1192 at `b2a4a10ce22c2f4950b94fa6b9e49aa235c6c7fa` and PR #1190 at `b72e487d65972566e63f508d023cf2e1e886734a`. Supabase migrations `20260604142017_video_date_active_presence_join_guard.sql`, `20260604170438_video_date_warmup_reconnect_stability.sql`, `20260604193140_video_date_latest_presence_grace_repair.sql`, and `20260604205645_video_date_remote_seen_latest_state.sql` are applied to project `schdyxcunwcvddlcshwd`. The latest stabilization work adds web/native Daily start ownership, lifecycle soft-signal handling, latest-state presence, canonical remote-seen latest-state repair, and terminal-survey hard-stop changes. Do not use this March snapshot to claim current Video Date health. Active Daily co-presence now requires latest joined presence for both users without a later `participant.left` / `participant_*_away_at`; stale `participant_*_joined_at` alone is not proof. Same-session nonterminal Daily calls should be reused or waited on, browser `visibilitychange` is soft telemetry during active Daily handoff/warm-up/date, Daily `participant-left` waits for local 12s transport grace before backend partner-away, return evidence clears reconnect grace, canonical `remote_seen` advances on every remote-media observation, and survey-required terminal truth must open `PostDateSurvey` on `/date/:sessionId` without Daily/surface churn. A fresh manual two-user match -> survey run remains the recovery acceptance proof.
