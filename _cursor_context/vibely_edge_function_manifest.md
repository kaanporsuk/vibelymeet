...
- **Rebuild notes:** defaults `DAILY_DOMAIN` to `vibelyapp.daily.co` if missing **and emits a `daily_domain_env_fallback` warning log at module init** so misconfiguration is observable; VideoDate beforeunload cleanup must send JWT via fetch keepalive; input guards reject malformed payloads early with **400** codes `MISSING_MATCH_ID`, `MISSING_CALL_ID`, `MISSING_ROOM_NAME` (room names validated against `^[a-zA-Z0-9_-]{1,128}$`, UUIDs against canonical pattern); `create_match_call` inserts `match_calls` via service role and enforces archived match, blocks, suspension/pause, and at most one `ringing`/`active` row per match (app precheck + DB partial unique index `uniq_match_calls_match_id_open`; insert **`23505`** → **409** `DUPLICATE_ACTIVE_CALL`, Daily room deleted); **token issuance during `create_match_call` is now wrapped in try/catch that deletes the just-created Daily room on failure**, closing the last orphan-room window (rows never reach `match_calls`, so room-cleanup cron cannot reclaim them); structured logs include `reject_layer` (`precheck` vs `db_unique`), `answer_match_call_not_found`, `create_match_call_token_failed`, token-failure context, and `daily_room_unhandled_exception` on unexpected errors; `answer_match_call` runs `match_call_transition('answer')` before issuing a Daily token (rollback `end` + **503** `TOKEN_ISSUE_FAILED` if token issuance fails); `match_call` push prefs use `notification_preferences.notify_match_calls` via `send-notification`
