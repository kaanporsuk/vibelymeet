...
**Status:** final policy recommendation (Phase 3b). **No** migrations, cron, Edge Functions, or product changes in this document.
...
| **Enforcement mechanism** | **Scheduled batched `DELETE`** on `event_loop_observability_events` by `created_at` (implementation later) |
