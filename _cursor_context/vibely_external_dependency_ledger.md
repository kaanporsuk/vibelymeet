...
- all 70 deployable functions are in `supabase/config.toml` (reconciled 2026-06-12 after `match-call-room-cleanup` removal); 40 JWT-at-gateway, 30 public-but-protected
...
- confirm all 71 Edge Functions are deployed with correct verify_jwt
...
**Retired historical functions**
- `email-drip` was removed from source/config and is not active in the linked Supabase function inventory.
- `unsubscribe` was removed from source/config and is not active in the linked Supabase function inventory.

### Secrets/config expected by code
- `RESEND_API_KEY`
- `EMAIL_VERIFICATION_OTP_SECRET` (preferred OTP HMAC pepper; falls back to service role for older deployments)

### Hardcoded runtime assumptions
- active outbound sender examples include `Vibely <hello@vibelymeet.com>`, `Vibely <notifications@vibelymeet.com>`, and `Vibely Support <support@vibelymeet.com>`
...
- product links depend on production-domain correctness, not just email delivery success
- if drip/unsubscribe are deliberately restored, the drip worker must be scheduler-gated by `CRON_SECRET` and unsubscribe links must be HMAC-gated by `UNSUB_HMAC_SECRET`

### Known unknowns to verify
- whether all active sender aliases are verified in Resend
- whether Resend suppression/bounce/webhook settings should feed application observability
...
- current posture: retired; no current source/config/live Edge Function endpoint
- historical direction: public browser/email click → Supabase Edge Function
- secret involved if restored: `UNSUB_HMAC_SECRET`
- must verify before restoration: token generation, link integrity across all email templates, and active deployment

### `email-drip`
- provider: scheduler/cron source
- current posture: retired; no current source/config/live Edge Function endpoint
- historical direction: scheduler → Supabase Edge Function
- secret involved if restored: `CRON_SECRET`
- must verify before restoration: scheduler source, cadence, unsubscribe-link HMAC, and active deployment
