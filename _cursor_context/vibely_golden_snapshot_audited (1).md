...
**Intent:** rebuild-oriented source-of-truth for the pre-native-hardening web codebase, with corrections where Claude's earlier snapshot was incomplete or stale.
...
### Function config audit (`supabase/config.toml`)

- Functions explicitly configured in `config.toml` have `verify_jwt` values recorded there.
- Two function directories exist in the repo but are **not listed** in `supabase/config.toml`: `forward-geocode`, `push-webhook`.
- This omission matters during rebuild because default JWT behavior can differ from intended deployment behavior.
...
- There is also a `push-webhook` function present in repo and missing from function config listing, which makes it a rebuild-sensitive surface.
...
- `forward-geocode` exists and must be carried in manifests.
- `push-webhook` exists and must be carried in manifests.
...
- JWT verification intent for functions, including explicit treatment of repo functions missing from `config.toml`
