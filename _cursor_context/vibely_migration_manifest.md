...
- Deployable Edge Functions: **67** (`supabase/functions/*/index.ts`); see `supabase/config.toml` and `_cursor_context/vibely_edge_function_manifest.md` for current config-backed inventory.
...
- **2026-05-22 (media derivatives / placeholders / realtime):** `20260522161000_media_derivatives_placeholders_realtime.sql` adds `media_assets.derivative_thumb_path`, `derivative_hero_path`, `placeholder_kind`, `placeholder_hash`, `dominant_color`, and `placeholder_updated_at`; creates `profile_photo_derivatives_for_paths`; wires `photo_derivatives` into `get_profile_for_viewer` / `get_my_profile_settings`; and adds owner-scoped realtime media asset broadcasts. Clients must only use server-confirmed derivative refs and fall back to original paths.
