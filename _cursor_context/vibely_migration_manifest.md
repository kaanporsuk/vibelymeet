...
- **Migrations:**
  ...
  - `supabase/migrations/20260409110000_expire_video_date_reconnect_grace_cron.sql` — **`public.expire_video_date_reconnect_graces()`** (same end semantics as the RPC branch); **pg_cron** job `expire-video-date-reconnect-graces` every minute so grace expiry does not depend on clients polling `sync_reconnect`
...
- **Parity note:** `20260408120000_waitlist_promotion_cron_vault.sql` was applied on the linked project before this branch; the file is kept in-repo so `supabase db push` / migration history stays aligned with remote
