...
0. **Video Date recovery command center:** `docs/video-date-success-command-center.md` — mandatory first read/update point for all Ready Gate, Video Date, Daily, notification, and post-date survey recovery work. It records PR #1190/#1192/#1194/#1196/#1199/#1200/#1212/#1213/#1216 plus the current terminal-survey lifecycle hardening, surface-owner recovery work, lifecycle false-away follow-up, 2026-06-06 single-owner runtime hardening, shared entry/Daily owner rollout, stable-copresence heartbeat guard, rollout documentation sync, and the provider-authoritative presence follow-up after failed session `c9dc7af1-1f40-431f-93ed-4435019126aa`. Current documented app baseline includes PR #1216 for provider-authoritative presence after PR #1213 commit `a3c34dd2b2400908c3cf529d8c3146a141b7ebb8`; Supabase recovery migrations are applied through `20260606203000_video_date_provider_authoritative_presence.sql` after `20260606092944_video_date_decisive_mark_ready_commit.sql`, `20260606100511_video_date_mark_ready_lint_cleanup.sql`, `20260606164737_event_registration_rpc_owned_dml_lockdown.sql`, and `20260606180000_video_date_stable_copresence_handshake_guard.sql`. The command center also preserves the older Video Date migration chain, Daily start ownership, latest-state presence, canonical remote-seen latest-state repair, immediate confirmed-encounter promotion, lifecycle-away suppression/clearing for active `web_beforeunload` and `web_pagehide`, current unexpired surface-claim evidence at reconnect expiry, current-peer-vs-historical-encounter separation, sticky survey status until feedback, pending-survey registration repair, terminal Daily room repair/backfill, cleanup/outbox provider-delete markers that preserve room forensics, cross-surface active video/survey route ownership with web/native forced survey guard bypass, native notification pending-survey deep-link recovery, exposed lifecycle RPC outer fail-soft wrappers, append-only surface-claim and presence audit events, widened client stuck observability, terminal-survey hard-stop, sync/deploy evidence, a fresh-session handoff prompt, and the still-unproven manual match -> survey acceptance run.
   ...
   - **Provider-authoritative follow-up:** applied migration `20260606203000_video_date_provider_authoritative_presence.sql` makes stable copresence and Daily alive join-stamping require current provider presence, while web/native/mobile send `owner_state='joined'` only when Daily is `joined-meeting` with a local provider session id. It is deployed to Supabase cloud but not acceptance-proven yet.
