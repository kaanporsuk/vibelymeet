...
| **Video Date review-comments 1322-1330 follow-up (2026-06-14)** | `docs/branch-deltas/review-comments-1322-1330-followups.md`; web pre-room media preflight now skips only for live app-acquired prewarm media; terminal `in_survey` fallback filters current room/current event before newest-row limit; `scripts/video-date-latency-forensics.mjs` UUID-validates provided session ids and budgets first provider joins from the webhook ledger; no migration |
| **Video Date review-comments 1322-1324 follow-up (2026-06-13)** | `docs/branch-deltas/review-comments-1322-1324-followups.md`; native `apps/mobile/lib/videoDateApi.ts` (fresh entry-truth bypasses decision-less snapshot); Edge Function `video-date-room-cleanup` (non-dry-run `markerRecorded===false` → reconciliation failure); contract pins `videoDateStartSnapshotContracts.test.ts` + `videoDateRoomCleanupReconciliationContracts.test.ts`; no migration |
| **Video Date review-comments 1314-1321 follow-up (2026-06-13)** | `docs/branch-deltas/review-comments-1314-1321-followups.md`; migration `20260613131415_vd_release_guard_passthrough_terminal_room_pointer.sql` (in-gate release guard falls through for terminal sessions); Edge Function `video-date-room-cleanup` (reconciliation-failure → `ok:false`/500); `scripts/video-date-load-probe.mjs` (deck-token guard + residue exit code); `shared/matching/videoDateValidationFollowupContracts.test.ts` listener-shape pin |
| **Video Date review-comments 1306-1313 follow-up (2026-06-13)** | `docs/branch-deltas/review-comments-1306-1313-followups.md`; date-route survey `current_room_id` scoping (web `src/pages/VideoDate.tsx` + native `apps/mobile/app/date/[id].tsx`); claim-surface contract pin repointed to consolidation v2; no migration |
| **Video Date review-comments 1298-1305 follow-up (2026-06-13)** | `docs/branch-deltas/review-comments-1298-1305-followups.md`; migration `20260613113508_review_comments_1298_1305_actionability_terminal_order.sql` (actionability terminal-first reorder); validation packs `supabase/validation/video_date_prepare_entry_lease.sql`, `supabase/validation/stale_ready_gate_room_blocker_repair.sql` retargeted to the folded `expire_stale_video_sessions()` |
...
| **Video Date queue_status entry vocab flip (`in_handshake`→`in_entry`)** | Migration `supabase/migrations/20260613015625_queue_status_vocab_flip_in_entry.sql` (flips writers + `valid_queue_status` CHECK, prunes dead queue-era `searching`/`matched`/`completed`, regenerates the 11 referencing functions vocab-only); web/native/shared read-sites flipped with one-release legacy `in_handshake` read tolerance; public-head fixture `supabase/contract-fixtures/2026-06/functions/public-heads/mark_video_date_daily_alive.sql` refreshed; contracts `shared/matching/videoSessionCreatedDefinitiveContracts.test.ts`, `shared/matching/videoDateLatestFailureSurfaceOwnerContracts.test.ts` |
