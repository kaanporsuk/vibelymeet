...
0. **Video Date recovery command center:** `docs/video-date-success-command-center.md` — mandatory first read/update point for all Ready Gate, Video Date, Daily, notification, and post-date survey recovery work. It records PR #1190/#1192/#1194/#1196/#1199/#1200/#1212/#1213/#1216/#1218 plus PR #1219 and later documentation/handoff syncs, the current terminal-survey lifecycle hardening, surface-owner recovery work, lifecycle false-away follow-up, 2026-06-06 single-owner runtime hardening, shared entry/Daily owner rollout, stable-copresence heartbeat guard, rollout documentation sync, provider-authoritative presence follow-up after failed session `c9dc7af1-1f40-431f-93ed-4435019126aa`, the 2026-06-07 review-comments follow-up for PR #1205 through #1216, and the provider-terminal recovery after failed session `98d50175-1c75-4966-a6e6-f444c4631289`. Current implementation baseline is PR #1218 merge commit `a7b8cb7dc05a47262a4c7c7dcd31e5972ed4d0c4`; docs-only PRs may advance source `main`, so verify exact source with `git rev-parse HEAD` and `git ls-remote origin refs/heads/main`. Supabase recovery migrations are applied through `20260606224200_video_date_provider_terminal_recovery.sql` after `20260606092944_video_date_decisive_mark_ready_commit.sql`, `20260606100511_video_date_mark_ready_lint_cleanup.sql`, `20260606164737_event_registration_rpc_owned_dml_lockdown.sql`, `20260606180000_video_date_stable_copresence_handshake_guard.sql`, `20260606203000_video_date_provider_authoritative_presence.sql`, `20260606205211_video_date_provider_participant_id_presence_repair.sql`, and `20260606212727_review_comments_1205_1216_followups.sql`. The command center also preserves the older Video Date migration chain, Daily start ownership, latest-state presence, canonical remote-seen latest-state repair, immediate confirmed-encounter promotion, caller-bound confirmed-encounter promotion wrapper authority, lifecycle-away suppression/clearing for active `web_beforeunload` and `web_pagehide`, current unexpired surface-claim evidence at reconnect expiry, current-peer-vs-historical-encounter separation, sticky survey status until feedback, pending-survey registration repair, terminal Daily room repair/backfill, cleanup/outbox provider-delete markers that preserve room forensics, cross-surface active video/survey route ownership with web/native forced survey guard bypass, native notification pending-survey deep-link recovery, exposed lifecycle RPC outer fail-soft wrappers, append-only surface-claim and presence audit events, widened client stuck observability, terminal-survey hard-stop, sync/deploy evidence, a fresh-session handoff prompt, and the still-unproven manual match -> survey acceptance run.
