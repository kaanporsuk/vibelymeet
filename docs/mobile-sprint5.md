...
Current 2026-06-09 note: this sprint note preserves the implementation shape at the time it was written. The legacy `leave_matching_queue(uuid)` RPC has since been removed from the active backend contract by `20260609165218_remove_leave_matching_queue.sql`, the legacy public `daily-room` actions `create_date_room` / `join_date_room` have since been removed from the active Edge Function action contract, standalone/client-visible `video_date_transition('enter_handshake')` has since been removed by `20260609202707_remove_standalone_enter_handshake.sql`, and active product countdown code now uses neutral entry/date timer aliases over legacy DB column names. Current web/native room and token acquisition uses `prepare_date_entry` / `prepare_entry`; end flows rely on `video_date_transition('end')`, Daily cleanup, and the supported Event Lobby path. See `docs/branch-deltas/neutral-entry-timer-aliases.md` for the current timer terminology boundary.
