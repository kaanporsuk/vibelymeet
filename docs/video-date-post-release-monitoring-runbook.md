...
- Current Video Date recovery migrations through `20260605203904_video_date_remote_seen_grace_payload_preserve.sql` are local and remote, and `video-date-outbox-drainer`, `video-date-room-cleanup`, and `video-date-orphan-room-cleanup` should be deployed with provider-delete markers instead of room-metadata nulling.
