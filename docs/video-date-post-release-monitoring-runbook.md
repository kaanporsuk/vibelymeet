...
Current recovery overlay (2026-06-07): for active Video Date recovery, start with `docs/video-date-success-command-center.md`. PR #1216 merged at `3ae7f196749f2229d66da6f0ef73ae2f76f30768` after failed session `c9dc7af1-1f40-431f-93ed-4435019126aa`; Supabase project `schdyxcunwcvddlcshwd` is applied/aligned through `20260606205211_video_date_provider_participant_id_presence_repair.sql`. The current monitoring focus is whether provider-authoritative copresence holds through the full user journey: stale or provider-null client heartbeats must remain telemetry only, Daily webhook `provider_participant_id` must be the primary provider session identity, stable copresence must require current provider proof, and `queue_status='in_survey'` with cleared `current_room_id` must still route to terminal-survey recovery. The prior owner/stable-copresence implementation adds shared date-entry and Daily owner contracts in code plus applied migration `20260606180000_video_date_stable_copresence_handshake_guard.sql`; PR #1216 adds `20260606203000_video_date_provider_authoritative_presence.sql` plus corrective `20260606205211_video_date_provider_participant_id_presence_repair.sql`. Active date/survey route ownership must suppress lobby/Ready Gate Daily prepare churn, transition/queue/surface RPCs should return structured retryable JSON instead of raw 500s, `video_date_surface_claim_events` should preserve claim history, and stuck-client observability should retain same-session continuity and singleton parking fields. Verify current Git and deployment state before assuming this doc baseline is still current. Static/CI/cloud checks can verify implementation only; the fresh manual two-user match -> survey acceptance run is still unproven.
...
Current 2026-06-07 recovery baseline:
...
- Current Video Date recovery migrations through `20260606205211_video_date_provider_participant_id_presence_repair.sql` are local and remote, and `video-date-outbox-drainer`, `video-date-room-cleanup`, and `video-date-orphan-room-cleanup` should be deployed with provider-delete markers instead of room-metadata nulling.
- Confirm web/native/mobile clients in the monitored environment send `mark_video_date_daily_alive` heartbeats after `20260606180000_video_date_stable_copresence_handshake_guard.sql`, and that post-`20260606205211` joined heartbeats include current provider session ids rather than provider-null telemetry.
...
- `video_date_presence_events` rows for `provider_daily_joined`, `provider_daily_left`, `client_daily_alive`, `owner_heartbeat`, `remote_seen`, and lifecycle decisions after `20260606205211` is applied
...
- After provider-authoritative stable-copresence rollout, both users keep fresh latest owner heartbeats within 15 seconds after the later provider-backed joined time, each joined heartbeat carries current provider proof, and either the first qualifying bilateral owner-heartbeat pair has been stable for at least 2 seconds or current canonical remote-seen exists before handshake starts.
