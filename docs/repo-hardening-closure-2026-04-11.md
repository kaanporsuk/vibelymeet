...
| No inbox-first gating on in-app email verification | **Confirmed:** flows use `resolveCanonicalAuthEmail` + OTP via `email-verification/send` & `verify`; “check your inbox” copy elsewhere refers to **Supabase account email confirmation** (linked email provider), not a prerequisite for the profile trust OTP step. **Edge runtime:** OTP hashing uses **Web Crypto HMAC-SHA256** (not bcrypt); see [`email-verification-settlement-2026-04-11.md`](./email-verification-settlement-2026-04-11.md). |
