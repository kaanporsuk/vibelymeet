...
| **notifications** | `send-notification` Edge Function, `push_subscriptions`, `notification_preferences`, OneSignal | OneSignal Web SDK; subscription in `push_subscriptions` mirrored to web prefs | OneSignal native SDK; subscription in `push_subscriptions` mirrored to mobile prefs; same categories | Single entrypoint; same prefs/quiet hours/mutes | Yes: store all subscribed IDs and include in send path |
...
- **Web:** The web app uses OneSignal Web SDK; on permission grant it obtains a subscription/player ID, registers it through `push_subscriptions`, and mirrors it to `notification_preferences` (`onesignal_player_id`, `onesignal_subscribed`). `send-notification` targets the stored subscription ID through `include_subscription_ids` and OneSignal delivers to the browser.
- **iOS / Android:** The native app uses the OneSignal iOS/Android SDK. On init and login, the app registers the device with OneSignal and associates the external user ID (Supabase user id). OneSignal assigns a player/subscription ID per device. The native client registers this identifier through `register_onesignal_push_subscription`, which stores it in `push_subscriptions` and mirrors the current device into `notification_preferences` for compatibility. `send-notification` includes all subscribed IDs so one user can receive on multiple native/web devices from a single send.

**Compatibility:** Existing web behavior is preserved by continuing to mirror the current browser into `notification_preferences.onesignal_player_id`, while `push_subscriptions` is now the durable multi-device target source for web and native. No mobile-only backend logic forks: the same `send-notification` function and category/preference rules apply to all platforms.
