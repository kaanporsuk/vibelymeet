...
| Video Date end-to-end acceptance | Manual proof pending | Engineering / Operator | Latest failed production run `c9dc7af1-1f40-431f-93ed-4435019126aa` for event `43d1614c-9b2d-45d6-be59-c56fa6cb852f` proved Ready Gate, same Daily room, visible local/remote media, and date UI could appear, but participant 2's Daily provider session left at `2026-06-06T19:22:52Z` with no later provider join while stale/missing-provider client heartbeats kept backend copresence alive. PR #1216 deployed provider-authoritative presence at merge `3ae7f196749f2229d66da6f0ef73ae2f76f30768`; Supabase is applied through `20260606205211_video_date_provider_participant_id_presence_repair.sql`, which prefers webhook `provider_participant_id` before payload fallbacks. Closes only when a fresh disposable two-user production run completes match -> Ready Gate -> same Daily room -> current provider-backed bilateral owner heartbeats and media/date -> date end -> post-date survey completion, plus short leave/rejoin and prolonged absence checks. | `docs/archive/video-date/video-date-success-command-center.md`, `docs/archive/video-date/branch-deltas/fix-video-date-ready-gate-decisive-mark-ready.md`, `docs/archive/video-date/branch-deltas/fix-video-date-surface-owner-failsoft.md`, `docs/archive/video-date/branch-deltas/fix-video-date-entry-daily-owner-stable-copresence.md`, `docs/archive/video-date/branch-deltas/fix-video-date-provider-authoritative-presence.md` |
