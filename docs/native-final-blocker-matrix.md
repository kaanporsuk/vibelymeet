...
| Video Date end-to-end acceptance | Manual proof pending | Engineering / Operator | Latest failed production web run `4082fe36-8480-4d30-9a1d-1de227b855e3` reached Ready Gate, Daily room, bilateral media evidence, and `date_started_at`, then churned across date/ready/lobby, ended `reconnect_grace_expired`, left both users `in_survey`, and created no `date_feedback`. The 2026-06-06 code/cloud follow-up deployed `20260605232304_video_date_single_owner_runtime_hardening.sql` and adds web/native/mobile single-owner route hardening, native terminal-survey hard-stop ownership, explicit native `forceSurvey` survey-intent routing parity, native notification pending-survey deep-link recovery, fail-soft transition/queue/surface RPCs, append-only surface-claim audit, and widened client stuck observability. Closes only when a fresh disposable two-user production run completes match -> Ready Gate -> same Daily room -> stable bilateral media/date -> date end -> post-date survey completion, plus short leave/rejoin and prolonged absence checks. | `docs/video-date-success-command-center.md`, `docs/branch-deltas/fix-video-date-surface-owner-failsoft.md` |
...
| Two-user video-date seeded runtime QA | Manual proof pending | Run `docs/qa/video-date-seeded-runtime-qa-pack.md` with admin, User A, User B, and a live/recent event. Include the post-`20260605232304` proof bar: match -> Ready Gate -> same Daily room -> stable bilateral media/date -> survey completion. |
