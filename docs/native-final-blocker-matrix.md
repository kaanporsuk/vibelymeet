...
| Video Date end-to-end acceptance | Manual proof pending | Engineering / Operator | Latest failed production run `cac485cd-da3b-475b-aa4c-27b70cd914d6` for event `21497965-394a-45fe-8700-5d91bf927f65` failed before Daily: one participant committed `ready_b`, the other hit SQLSTATE `57014` / `READY_GATE_TRANSITION_TIMEOUT`, no `both_ready` or Daily room metadata was created, and the session expired `ready_gate_expired`. The 2026-06-06 code/cloud follow-up deployed `20260606092944_video_date_decisive_mark_ready_commit.sql` and `20260606100511_video_date_mark_ready_lint_cleanup.sql`, plus web/native/mobile mark-ready RPC-error retry with the same deterministic idempotency key. The prior single-owner runtime hardening `20260605232304` remains the active date/survey route-owner baseline. The latest owner/stable-copresence code adds shared date-entry and Daily owners plus `mark_video_date_daily_alive` heartbeats; migration `20260606180000_video_date_stable_copresence_handshake_guard.sql` is dry-run validated and pending coordinated rollout after heartbeat-capable clients are live. Closes only when a fresh disposable two-user production run completes match -> Ready Gate -> same Daily room -> fresh latest owner heartbeats plus first qualifying bilateral heartbeat stability or remote-seen -> stable bilateral media/date -> date end -> post-date survey completion, plus short leave/rejoin and prolonged absence checks. | `docs/video-date-success-command-center.md`, `docs/branch-deltas/fix-video-date-ready-gate-decisive-mark-ready.md`, `docs/branch-deltas/fix-video-date-surface-owner-failsoft.md`, `docs/branch-deltas/fix-video-date-entry-daily-owner-stable-copresence.md` |
...
| Two-user video-date seeded runtime QA | Manual proof pending | Run `docs/qa/video-date-seeded-runtime-qa-pack.md` with admin, User A, User B, and a live/recent event. Include the post-`20260606092944` proof bar plus the owner/stable-copresence bar after coordinated rollout: match -> both ready without 57014/ready expiry -> same Daily room -> fresh `mark_video_date_daily_alive` heartbeats from both owners -> first qualifying bilateral heartbeat pair stable for at least 2 seconds or remote-seen before handshake/date -> stable bilateral media/date -> survey completion. |
| Daily real-device join/leave | Manual proof pending | Verify room join, media, refresh/rejoin under 12s, real prolonged absence terminalization, end/leave, and backend state. |
