...
| Video Date end-to-end acceptance | Manual proof pending | Engineering / Operator | Latest failed production run `cac485cd-da3b-475b-aa4c-27b70cd914d6` for event `21497965-394a-45fe-8700-5d91bf927f65` failed before Daily: one participant committed `ready_b`, the other hit SQLSTATE `57014` / `READY_GATE_TRANSITION_TIMEOUT`, no `both_ready` or Daily room metadata was created, and the session expired `ready_gate_expired`. The 2026-06-06 code/cloud follow-up deployed `20260606092944_video_date_decisive_mark_ready_commit.sql` and `20260606100511_video_date_mark_ready_lint_cleanup.sql`, plus web/native/mobile mark-ready RPC-error retry with the same deterministic idempotency key. The prior single-owner runtime hardening `20260605232304` remains the active date/survey route-owner baseline. A later code-only surface-owner follow-up stabilized web/native/mobile server-facing surface client ids and owner-tokened delayed release while preserving strict duplicate-device conflicts; no new Supabase migration was retained for that pass. Closes only when a fresh disposable two-user production run completes match -> Ready Gate -> same Daily room -> stable bilateral media/date -> date end -> post-date survey completion, plus short leave/rejoin and prolonged absence checks. | `docs/video-date-success-command-center.md`, `docs/branch-deltas/fix-video-date-ready-gate-decisive-mark-ready.md`, `docs/branch-deltas/fix-video-date-surface-owner-failsoft.md` |
