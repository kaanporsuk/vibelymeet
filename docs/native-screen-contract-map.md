...
| `/settings` | `Settings` | `app/settings` | useLogout, useDeleteAccount, useCredits, usePremium, endAccountBreak | profile pause fields / account-break helpers; delete-account (EF); notification_preferences; create-checkout-session (EF) | OneSignal | P0 |
| `/settings/referrals` | `Referrals` | `app/settings/referrals` | `buildInviteLandingUrl` (shared), `applyNativeReferralAttribution` (post-auth helper), profile status query | `profiles` read/update (`referred_by`) via existing client contract (no new RPC/EF) | Native `Share` + clipboard | P0 |
