...
- Repo inspection found 49 deployable function directories and 49 config entries.
...
- `_cursor_context/vibely_external_dependency_ledger.md` records 49 functions, Stream 19 posture, provider secret names by name only, and manual provider gates.

WARN: Rebuild runbook is partially contradicted by newer release artifacts.

- Current authoritative sources say 49 deployable functions and 49 config entries.
- The runbook retains an older historical deploy list and counts.
- Repair recommended: replace that section with the current 49-function inventory and point to the machine-readable inventory/manifest as source of truth.
...
1. Refresh `_cursor_context/vibely_rebuild_runbook.md` Section 13 so it uses the current 49-function inventory, current JWT counts, and no obsolete function names.
