...
Current recovery override (2026-06-07): this file is historical closure provenance, not current Video Date health evidence. For active recovery work, start with `docs/archive/video-date/video-date-success-command-center.md`. The latest synced baseline is PR #1216 at `3ae7f196749f2229d66da6f0ef73ae2f76f30768`, with Supabase project `schdyxcunwcvddlcshwd` aligned through `20260606205211_video_date_provider_participant_id_presence_repair.sql`. The active lesson is provider-authoritative Daily presence: `provider_participant_id` from the Daily webhook ledger is the primary provider identity, and provider-null or provider-stale client heartbeats are telemetry only. Verify current Git/Supabase state before assuming no docs-only follow-up sits on top. A fresh manual two-user match -> survey run is still required before production confidence can be reclaimed.
