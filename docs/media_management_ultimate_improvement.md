...
| 1.2 | `test/cvc-p1-smoke-matrix` | Playwright smoke (web) + Maestro smoke (iOS, Android) for Chat Vibe Clip happy path + 4G throttle + kill-mid-TUS + webhook-delayed + signed-URL refresh + app-launch stuck-row nudge cases. Runs in CI per PR; nightly against staging. | All matrix rows green on staging. |
| 1.3 | `fix/cvc-p1-targeted-server-fix` | Closure evidence points to aggregate hardening rather than one isolated suspect: PRs #917-#919 covered upload/publish/webhook/status-sync/orphan cleanup, signed-URL refresh, provider-status regressions, and recovery UX. This Phase 1 closure adds the server-row recovery sweep, local-source gating, credential reissue, and provider-unavailable observability. | 7-day rolling Chat Vibe Clip success ≥ 99 %. |
