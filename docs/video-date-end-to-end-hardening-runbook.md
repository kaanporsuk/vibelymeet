...
- Ready Gate mark-ready is a decisive hot path. `video_session_mark_ready_v2` starts/reuses the idempotent command, writes `ready_participant_*_at`, extends `ready_gate_expires_at` to at least `now() + 45 seconds`, and writes deterministic `both_ready` room metadata before auxiliary observability/event/outbox work. Legacy `ready_gate_transition('mark_ready')` callers bridge into the same RPC, so web/native/mobile should not diverge.
...
- Historical remote-seen/date-entry proof makes an ended session survey-eligible, but it is not current presence proof after later Daily leave/rejoin churn. It should suppress local client terminalization, emit `peer_missing_suppressed_remote_seen`, and leave provider/server absence reconciliation to decide terminal state.
- After `date_started_at`, automatic peer absence should terminalize through provider/server absence truth instead of pre-date `partial_join_peer_timeout`; explicit user exits may still use survey-eligible `partner_absent_after_confirmed_encounter`.
...
24. Simulate post-encounter peer disappearance after `date_started_at`; confirm client watchdog suppression (`daily_no_remote_watchdog_historical_truth_suppressed` / `peer_missing_suppressed_remote_seen`) while provider/server absence owns any automatic terminal path, or confirm explicit manual exit uses `partner_absent_after_confirmed_encounter`; then verify sticky `in_survey` and survey completion for both users.
