...
Operator runbook for executing launch closure with dashboards and real devices. Do phases in order. For each phase: the implementation agent can do repo/docs; Kaan does dashboard/device/store actions.
...
| Role | Actions |
|------|--------|
| **Implementation agent** | Confirm repo: bundle ID `com.vibelymeet.vibely`, package `com.vibelymeet.vibely`; app uses `Purchases.getOfferings()` and `offerings.current.availablePackages`. Ensure checklist §2 is accurate. No code deploy. |
...
| Role | Actions |
|------|--------|
| **Implementation agent** | No actions in this phase (validation is on device). |
...
| Role | Actions |
|------|--------|
| **Implementation agent** | Ensure `eas.json` has `preview` profile (distribution: internal, environment: preview). Ensure `app.config.js` sets OneSignal mode production for preview. Doc: list required EAS secrets for preview. No Supabase deploy. |
...
| Role | Actions |
|------|--------|
| **Implementation agent** | Ensure `production` profile exists (distribution: store, environment: production). OneSignal mode production for production profile (already in `app.config.js`). No code change. |
...
| Role | Actions |
|------|--------|
| **Implementation agent** | Provide checklist doc; no device actions. |
...
| Role | Actions |
|------|--------|
| **Implementation agent** | After Kaan reports results: update `docs/native-final-blocker-matrix.md` with: blockers solved by the implementation agent (none in this sprint beyond docs); blockers requiring Kaan (dashboard/store/device) with status; blockers requiring build/test evidence (mark resolved or still open with reason). Add "Sprint 6 test results" section if Kaan provides pass/fail per phase. |
| **Kaan** | Report pass/fail for Phases 2, 4, 7 (and build success for 5, 6). Share any blocker (e.g. "RevenueCat webhook 401") so the implementation agent can doc the fix. |

**Expected result:** Blocker matrix reflects actual test results and remaining blockers.

**Pass/fail:** Matrix updated; remaining blockers clearly listed with owner (Kaan vs implementation agent vs needs evidence).
