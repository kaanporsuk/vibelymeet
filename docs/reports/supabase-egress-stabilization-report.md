...
Scope: frontend/client stabilization only. No schema, migration, bucket, cron, or RLS changes.
...
| Edge Functions | Production traces showed `/functions/v1/health` twice on anonymous and authenticated boot. Patched local: one network health call per boot, second call served from the in-page cap. | Normal browser boot no longer repeats Supabase health probes. |
...
- `/functions/v1/health`: 2 -> 1 per boot.
...
- Dashboard matches query now has `limit=20`; dashboard rendering still only fetches profile rows for the first 5 visible matches.
...
- Added one-per-boot health control for `/functions/v1/health`; duplicate/in-flight browser calls receive the cached response and emit `browser.health_check_capped`.
...
- Reduced dashboard boot traffic:
  ...
  - Limited dashboard matches to 20 before archive/profile fanout.
- Reduced write churn:
...
- Anonymous and authenticated `/home`: max one `/functions/v1/health` network call per boot.
