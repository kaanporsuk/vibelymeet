...
Canonical rollout flags for this closure are the v2 flags: Ready Gate uses `video_date.timeline_v2` and `video_date.broadcast_v2`; push dedupe uses `video_date.multi_device_dedup_v2`; verdict confirmation uses `video_date.verdict_confirm_v2`; deck polish uses `video_date.deck_prefetch_polish_v2`. **Update 2026-06-10:** the v1 compatibility aliases (`video_date.ready_gate_resilient_clock_v1`, `video_date.push_open_dedupe_v1`, `video_date.verdict_confirm_v1`, `video_date.deck_optimistic_v1`) and the shared alias dual-read helper (`shared/featureFlags/featureFlagAliasResolution.ts`, `VIDEO_DATE_FEATURE_FLAG_ALIAS_GROUPS`) were retired with the Top-5 simplification pass (see `docs/branch-deltas/video-date-simplification-top5.md`); clients now read the canonical v2 flag's `.enabled` directly. The alias rows may remain in the `client_feature_flags` table but are no longer read by any client.
