...
| 3 | Daily configuration fallback | `daily-room` now fails closed for staging/production when `DAILY_DOMAIN` is missing; explicit local/dev/test fallback remains only for developer resilience. | Release certification must keep `npm run certify:video-date:required` green and record Daily env readiness. |
