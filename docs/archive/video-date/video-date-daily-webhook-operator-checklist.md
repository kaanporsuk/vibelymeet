...
Current local overlay (2026-06-08): start with `docs/video-date-success-command-center.md` and `docs/branch-deltas/fix-video-date-active-owner-terminal-truth.md`. Migration `20260608171837_video_date_active_owner_terminal_truth.sql` preserves delayed provider join/left truth by webhook `occurred_at` even when the session is already terminal and row mutation is blocked. That preservation must also run when the base Daily recorder returns `state = ignored` / `result = ignored_terminal_session`; the response means no active state mutation, not "discard historical provider truth." A late Daily event must not be allowed to incorrectly restart an ended session, but it must still leave historical evidence that a participant actually joined or left. This overlay is not yet cloud proof until the migration is applied remotely.
...
8. Confirm `video_sessions` reflects active presence correctly:
   ...
   - pre-`20260606180000`, `entry_started_at` starts only after both participants' latest provider presence is active.
...
13. After `20260608171837`, if Daily provider events arrive after terminalization, confirm they are ordered by `occurred_at`, preserve `participant_*_provider_joined_at` / `participant_*_provider_left_at`, and append `video_date_presence_events.source = 'daily_webhook_historical_truth'` without reviving active date state. This remains required when the recorder result is `ignored_terminal_session`.
