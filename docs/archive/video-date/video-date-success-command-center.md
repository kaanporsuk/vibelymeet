...
## 2026-06-12 Implementation Update: Backend Ops-Surface Purge + Ops Posture (Rebuild PR 9 of 10)

- **What changed (branch `claude/vd-rebuild-09-ops-purge`, migration `20260612210000_video_date_ops_surface_purge.sql`, cloud-applied):** the duplicate alert/reminder/certification/debug ops surfaces are dropped. Removed live: 31 functions (phase-8 certification + feedback-exception family, worker-run mutex trio, launch-latency + client-stuck recording RPCs and their sanitizer helpers except `video_date_client_stuck_safe_text` which `claim_video_date_surface` uses, `get_video_date_sprint7_ops_health`, `get_video_date_phase2_recovery_health`, zero-feedback reminder RPC quartet, `detect_post_date_half_verdict_timeouts`, daily-performance decision/emission RPC pair, circuit-breaker decision pair), 16 views (phase-8 family, daily-performance chain, legacy-deck-cleanup readiness, extension certification/mutual health, multi-device health, provider-room reconciliation, v4 schema inventory, orphan-cleanup health, phase-5 circuit-breaker decision), 4 tables (`video_date_phase8_certification_runs`, `video_date_certification_feedback_exceptions`, `video_date_worker_runs`, `post_date_zero_feedback_reminders`), and the hourly `post-date-half-verdict-timeout-detection` cron job.
- **Kept posture (user-decided):** ONE synthetic monitor (`synthetic-video-date-monitor`, reads `vw_synthetic_video_date_health` + `vw_video_date_flag_rollout` — both kept), ONE recovery-alert path (`video-date-recovery-alert-dispatcher` v35 now reads `vw_video_date_recovery_alerts` directly; `vw_video_date_lease_recovery_health` kept because the alert view is built on it), `post-date-verdict-reminders` v307 (pending-verdict lane only), and `admin-video-date-ops` v353 as READ-ONLY diagnostics with two actions: `metrics` (Ready Gate→join latency, swipe recovery, notification-outbox health, PostHog timer-drift pointer) and `get_session_timeline`.
- **Worker concurrency truth:** the outbox drainer (v48) and deadline finalizer (v37) used `video_date_worker_runs` only as a whole-run mutex; the real safety is the per-row claim/lease RPCs (`claim_video_date_provider_outbox_v2` / `claim_video_session_deadlines_v2` + `refresh_*_claim_v1`), which are untouched. Worker-run writes were removed from both functions and the shared helper.
- **Client emitters removed with the RPCs:** the fail-soft client-stuck (`record_video_date_client_stuck_observability`) and launch-latency (`record_video_date_launch_latency_checkpoint(+s_v1)`) DB mirrors are gone, so their fire-and-forget emitters were stripped from 11 web/native runtime files plus `src/lib/analytics.ts` / `apps/mobile/lib/analytics.ts`; PostHog product/ops analytics checkpoints (`ready_gate_to_date_latency_checkpoint` vocabulary in `videoDateOperatorMetrics.ts`) are unchanged.
- **Deviations from the PR-9 brief, with evidence:** (1) rate-limit twins both stay — `video-date-token-refresh` calls `take_video_date_token_refresh_provider_rate_limit_v1` per session/bucket (room_lookup/meeting_token) AND caller-scope `take_video_date_token_refresh_rate_limit_v1`, so there is nothing to collapse; (2) room-cleanup/orphan-room-cleanup cron consolidation DEFERRED — both delete via direct Daily API (presence-grace vs reconciliation+interlock), neither uses the outbox `daily.delete_video_date_room` path, so a merge is a behavior change, not mechanical; (3) the `_20260505214500`/`_20260610000100` generation functions named in the brief no longer existed live (folded by earlier passes); (4) `admin-video-date-ops` had already been made mutation-free by earlier PRs — this PR removed its dead read surfaces (sprint7 health, launch-latency first-frame metrics, daily-performance views).
- **Live verification:** post-apply dry-run `Remote database is up to date`; DB lint 0 errors (5 pre-existing shadowed-variable warnings); all 31 functions + 20 relations verified absent and the 4 kept views + 3 kept functions present; final video-date cron set = minutely `expire-stale-video-sessions`, `expire-video-date-reconnect-graces`, `video-date-deadline-finalizer`, `video-date-outbox-drainer`, `video-date-room-cleanup`; */5 `daily-room-keepwarm`, `synthetic-video-date-monitor`, `video-date-recovery-alert-dispatcher`, `post-date-verdict-reminders`; */10 `video-date-orphan-room-cleanup`. Exactly one minutely tick (17:26:02Z) failed for drainer+finalizer in the window between migration apply and Edge redeploy; zero worker failures since, and 200-OK payloads were captured for all four kept paths post-deploy.
- **Pre-existing condition surfaced by the kept alert path (not caused by this PR):** one page-severity alert — 26 `notification.send` outbox rows in `state='failed'` accumulated since 2026-05-27 with `notification_no_preferences` (disposable test users without notification preferences). Follow-up: either backfill prefs for smoke users or teach the alert view to classify `notification_no_preferences` as non-paging.
- **Not acceptance:** static batteries (typecheck, lint, red-flags, full v4, event-lobby regression) are green, but the program bar remains a fresh two-user run through persisted `date_feedback`.

## 2026-06-12 Implementation Update: Native Port onto Shared Controller Layer + Frozen v2 Transition RPC Drops (Rebuild PR 8 of 10)

Eighth PR of the staged re-foundation (branch `claude/vd-rebuild-08-native-port-db-drops`). Native now consumes the shared `shared/videoDate/` route-ownership layer (the same surface web adopted in PR 7), the native date-screen monolith's module scope is decomposed into its family, and the frozen v2 transition RPCs are dropped from the linked project.

- **Native navigation-intents binding:** `apps/mobile/lib/videoDateNavigationIntents.ts` binds the shared store (in-memory like the deleted owners; RC breadcrumb stream preserved; expo-router `navigateToDateSessionGuarded` side effect hosted in the binding). `apps/mobile/lib/dateEntryTransitionLatch.ts` and `apps/mobile/lib/dateNavigationGuard.ts` are deleted; 7 consumers re-pointed. The shared factory gained an injectable `duplicateNavigationMs` so native keeps its 30s burst window (web 15s default unchanged).
- **Native route surfaces consume `decideVideoDateSurfaceRoute`:** `NativeSessionRouteHydration` delegates the ready-gate-bounce decision tree (keeps its no-fetch entry-latch fast path; native still only navigates for the Ready Gate bounce — never same-route re-navigations, the native remount-storm class). `ready/[id].tsx`'s standalone reconcile takes ownership/latch suppression from the decision (surface `ready_redirect`), mirroring web ReadyRedirect. Deliberate unification deltas mirror PR 7's web deltas (hydration honors route-ownership suppression; latch cleared on not-routable truth).
- **Module-scope decomposition (verbatim):** `app/date/[id].tsx` 13,444 → ~12,050 LOC. New family: `lib/daily/nativeDailyCallSingleton.ts` (RN Daily adapter core — call/prejoin singleton state behind `nativeDailyCallSingletonState.{sharedDailyCallEntry,sharedNativePrejoinPipelineEntry}` with substring-preserving names for existing pins; park-before-leave invariants documented in-file), `lib/daily/nativeDailyMediaHelpers.ts` (camera/track/render-recovery helpers), `lib/videoDate/nativeVideoDateSurfaceClient.ts` (surface-claim client identity + backoff), `lib/videoDate/videoDateScreenShared.tsx` (constants/types/diagnostics/terminal-survey helpers/warmup banner), `lib/videoDate/videoDateScreenStyles.ts`. First body concern hook extracted with the PR 7.5 verbatim technique: `lib/videoDate/useNativeDailyAliveHeartbeat.ts` (provider-proofed daily_alive heartbeat; heartbeat refs hook-owned; `callRef` + terminal-survey recovery threaded as typed deps); the evidence-RPC ownership pin now names it as the native daily_alive owner.
- **Pin/audit migration:** new `shared/testUtils/nativeVideoDateFlowSources.ts` group readers (`NATIVE_VIDEO_DATE_SCREEN_FLOW_FILES`, `NATIVE_VIDEO_DATE_NAVIGATION_INTENT_FILES`, `NATIVE_READY_SCREEN_FLOW_FILES`); 48 contract tests codemodded from single-file reads to the screen family + 9 manual sites (consumer scan lists extended with the new modules, tuple labels, multiline reads); `audit-video-date-{buttons,remote-frame,ultimate-design}.mjs` read the family. ~10 pins retargeted intent-preserved (native hydration decideVideoDateSurfaceRoute delegation ×4, latch/guard family reads, lastDateNavigation rename, keysToClear ownership-clear, single-line destructure kept in the binding). Fixed pre-existing main breaks found in passing in `videoDateSurfaceContinuityHardening.test.ts` (stale `terminalSurveyOwner`/`onVideoDateReady`/`date_route_ownership_active` pins vs the PR 7/7.5 web decomposition and the instant-next removal; suite is wired into `run_hardening_contract_tests.sh`, not the v4 battery, which is why PR 7.5's green battery missed them).
- **DB drops (cloud-applied):** migration `20260612134101_vd_rebuild_pr8_drop_frozen_v2_transition_rpcs.sql` drops `video_session_continue_entry_v2`, `video_session_date_timeout_v2`, `video_session_forfeit_v2`, `video_session_entry_auto_promote_v2`, and chain-private `vd_auto_promote_eligible_base` / `vd_auto_promote_stable_media_base` (live evidence: only self/chain-internal prosrc references; zero view/policy/trigger dependents; zero repo references; the live promotion path is the evidence single bodies). Generated types regenerated — zero references to dropped names; 2 generated-type presence pins flipped to absence. `video_session_continue_handshake_v2*` / `vs_handshake_auto_promote_*_base` were already absent (PR 5).
- **Verified-live and KEPT (deferred with evidence, not dropped):** `video_date_session_is_post_date_survey_eligible` (v1) has 6 live callers and is not a dead twin — v1 gates on encounter *exposure*, v2 on *confirmed* (remote-seen-proofed) encounters; re-pointing callers would tighten verdict/surface-claim/survey-continuity behavior and needs an explicit decision. `video_sessions.snooze_expires_at`/`snoozed_by` (Ready Gate snooze machine + `enforce_one_active_video_session`), `refund_*` (refund-on-end trigger), and `started_at`/`entry_started_at` (webhook terminal truth, drift validators) all carry live references.
- **Honest scope (PR 8.5 follow-up proposed, mirroring PR 7 → 7.5):** the remaining `VideoDateScreen` body split (~11,700 LOC: call listeners, remote-seen, start-call pipeline ≈3.2k-LOC effect, cleanup/end, camera switch, AppState, surface-claim loop, render), the `ready/[id].tsx` body split, and the native date-guard cluster port onto `decideVideoDateSurfaceRoute` are NOT in this PR. The date-guard port is behavior-affecting (advisor → canonical decision owner, the same deliberate delta web took in PR 7) and deserves its own focused change with a live two-user gate.

Static tests are not product acceptance; the program bar remains a fresh two-user run through persisted `date_feedback`, and PR 8's native acceptance additionally needs an on-device cross-platform pair (native + web peer) through both surveys — pending manual validation.

## 2026-06-12 Implementation Update: Orchestration-Body Split (Rebuild PR 7.5, follow-up to PR 7)

The dedicated orchestration-body split promised in PR 7, landed before the PR 8 native port (client-only; no DB migrations, no Edge deploys). Both web flow giants are now thin orchestrators over concern sub-hooks with verbatim bodies; every shared ref/setter keeps its original name (threaded via typed deps), so contract pins keep matching and closure capture is unchanged.

- **`useVideoCall.ts` 6,963 → 481 LOC.** Seven concern files under `src/hooks/videoCall/`: `videoCallRuntime.ts` (shared `VideoCallSharedRuntime` deps type + moved option/identity types; sub-hook call order documented as part of the behavior contract), `useDailyAliveHeartbeat.ts` (provider-proofed `mark_video_date_daily_alive` heartbeat, listener/token-timer teardown, receive-settings adaptation, SDK-unresponsive watchdog), `useVideoDateRemoteSeen.ts` (provider-bound remote-seen stamping with bounded retries/restamp throttle, first-remote-frame proof, live-remount identity preservation), `useRemoteRenderPipeline.ts` (attach/reattach, same-track render validation, bounded recovery, maintenance reattach interval; the pinned synchronous `scheduleRemoteRenderValidationRef` assignment stays render-phase), `useWebCameraSwitch.ts` (commit-before-hint, deterministic switch + cycleCamera fallback, flipCamera), `useVideoDateMediaPreflight.ts` (progressive portrait capture preflight + release), `useDailyCallCleanup.ts` (reconnect-grace teardown + `cleanupCallObject` incl. live same-session remount parking), `useVideoDateStartCall.ts` (start gate, truth fetch, room acquisition, token refresh/rejoin, call-object creation/singleton reuse, per-attempt Daily event wiring, join + joined proof). `startCall` deliberately stays one ~3.4k-LOC per-attempt closure: its event handlers capture attempt-locals, and splitting that capture is the regression class this rebuild avoids. Parent keeps state, shared refs, the continuity-latch trio, wiring, and the public API tail.
- **`VideoDate.tsx` 6,130 → 5,036 LOC.** Three page concern hooks under `src/pages/videoDate/`: `useTerminalSurveyRecovery.ts` (terminal hard-stop, survey open/dedupe, terminal context hydration, registration/in_survey + ended-truth recovery), `useVideoDateBroadcastReconcile.ts` (seq-aware channel reconcile, gap snapshot recovery with bounded retries, extension broadcasts, channel subscription lifecycle), `useVideoDateLifecycleLeave.ts` (pre-date server-end signal + background retry, pre-date exits, leave flow, end-date confirmation, peer-missing terminal handling). JSX render tree and remaining effects stay in the parent; hooks are called at the original declaration positions so effect order is unchanged.
- **Pin/audit migration:** `WEB_VIDEO_CALL_FLOW_FILES`/`WEB_VIDEO_DATE_PAGE_FLOW_FILES` group readers extended with the new family members (original source order), so the 50 PR-7 re-pointed suites needed no further edits. 4 pins retargeted intent-preserved: evidence-RPC ownership tightened to per-concern owners (`daily_alive`→heartbeat, `daily_joined`→start-call, `remote_seen`→remote-seen), session-row-reader list follows the start-call pipeline, and two access-guard slice pins anchored on the `if (dupBlocked) return` line because the broadcast hook hosts the same access guard earlier in the concatenated family. `audit-video-date-remote-frame.mjs` + `audit-video-date-ultimate-design.mjs` read the families (ultimate-design parity vs main verified: 25 PASS / 2 pre-existing GAP / 2 BLOCKED). Repaired in passing: `audit-video-date-buttons.mjs` had been crashing at startup since PR 7 deleted `dateNavigationGuard.ts` (script is unwired in CI so it failed silently); re-pointed to the navigation-intents family + page family and fixed 5 stale pins it had been masking (2 web formatting, 3 native quote-style).
- **Process lesson (important):** `npx tsc --noEmit -p tsconfig.json` validates NOTHING in this repo — root `tsconfig.json` is solution-style with `files: []`. The real gate is `npm run typecheck` (core-strict + mobile + `tsconfig.app.json`). The first live-gate attempt failed with a runtime `ReferenceError` (`peerMissingTruthRefreshCountRef`) precisely because of this; the real typecheck then enumerated every wiring slip (stale sharedRuntime entries for moved refs, missing module imports in 4 sub-hooks, 2 setters missing from a destructure). All repaired; failed-attempt fixtures cleaned to zero residue.
- **Accepted debt:** 49 `react-hooks/exhaustive-deps` warnings (0 errors) — the rule cannot prove destructured deps refs are stable; adding ref names to dep arrays or inserting disable comments would edit pinned verbatim bodies for zero behavior change. Revisit only with a typed lint exemption.
- **Battery (all green):** `npm run typecheck` (core-strict + native + app), eslint (0 errors), full `test:video-date-v4` (91 files), `test:video-date:red-flags` (31 files), `test:event-lobby-regression`, remote-frame + buttons + ultimate-design audits, web production build, `db push --dry-run` "Remote database is up to date". No cloud artifacts deployed.

**Live gate result (2026-06-12, tag `vd-pr75-smoke-20260612-66669`): PASSED.** Same PR-6/7 technique (fresh disposable tagged pair, production Supabase + Daily, local Vite 5173, two headless Chromium with fake media): mutual vibe → both ready → both on `/date/<id>` → bilateral provider-proofed remote-seen + `date_started_at` promotion → manual end → both surveys → 2 persisted `date_feedback` rows; tag-scoped cleanup, zero residue. A first attempt (tag `…-65453`) failed at remote-seen with the `ReferenceError` above — the live gate caught a real runtime wiring bug that the vacuous tsc invocation and eslint both missed; its fixtures were cleaned to zero residue.

## 2026-06-12 Implementation Update: Shared Session Controller + Web Decomposition (Rebuild PR 7 of 10)

Seventh PR of the staged re-foundation (client-only; no DB migrations, no Edge deploys). One platform-agnostic Video Date session state machine now lives in `shared/videoDate/`, and the web client's route-ownership layer is ported onto it. Native is NOT ported here (PR 8); `shared/videoDate/` compiles for both platforms (no DOM globals, no React, no supabase-js).

- **New `shared/videoDate/` module family:** `types.ts` (controller states/inputs/commands/route decision), `navigationIntents.ts` (route ownership + entry-pipeline latch + manual-exit suppression + duplicate-navigation claims with injected clock/storage ports — exact TTL/storage-key semantics of the deleted web modules), `routeDecision.ts` (`decideVideoDateSurfaceRoute`, the single surface-route decision composing `decideCanonicalVideoDateRoute` with intent suppression and survey pinning, applying intent mutations itself), `sessionController.ts` (explicit state machine: hydrate → ready_gate → preparing_entry → joining → entry → date → ending → survey_required → done, with reconnecting/parked_remount as states and terminal-survey recovery reachable from every non-terminal state; emits explicit commands with idempotency keys; performs no IO). 36 unit tests across three suites cover every transition incl. remount/reconnect/terminal-survey recovery, own-row survey confirmation before advance, seq-gap broadcast recovery, and `pre_stable_media_failed` survey-ineligibility; wired into `test:video-date-v4` and `test:video-date:red-flags`.
- **Deleted web ad-hoc owners → controller replacements:** `src/lib/dateEntryTransitionLatch.ts` and `src/lib/dateNavigationGuard.ts` are deleted; semantics live in `shared/videoDate/navigationIntents.ts` behind the web binding `src/lib/videoDateNavigationIntents.ts` (sessionStorage ports, same persisted keys — in-flight user state survives the deploy). `SessionRouteHydration` keeps the active-video app-shell section but delegates the ready-gate-bounce decision tree to `decideVideoDateSurfaceRoute`; the `VideoDate` date-guard's in_ready_gate/canonical-bounce/ownership-suppression branch cluster and `ReadyRedirect`'s canonical ready-gate ownership check consume the same single decision. `EventLobby`/`useActiveSession` consume the intents binding. `useVideoDateDupTabGuard` stays a web-adapter concern, unchanged. Deliberate unification deltas: hydration now also honors route-ownership suppression (previously only the date guard did), and the date guard's bounce targets come from the canonical decision (both_ready truth now stays date-owned rather than advisor-dependent ready-bounce) — both reduce route-bounce storms and align all owners on one decision.
- **Web decomposition (partial — honest scope):** `useVideoCall.ts` 8,029 → 6,963 LOC by extracting all module-scope code verbatim: `src/lib/daily/webDailyCallSingleton.ts` (live same-session remount parking with heartbeat transfer + per-session/user start gate; invariants documented in-file) and `src/lib/daily/webDailyMediaHelpers.ts` (pure media/camera/render helpers). `VideoDate.tsx` 6,494 → 6,129 LOC via `src/pages/videoDate/videoDatePageShared.tsx` (module-scope helpers/constants/types). The ~800-LOC-per-flow-file target was NOT reached: the remaining mass is the two orchestration bodies (the `useVideoCall` hook body and the `VideoDate` component effects/render), whose internal split requires stale-closure-safe sub-hook factoring of the most incident-hardened code in the repo (call-start protection, heartbeat-transfer parking, terminal-survey hard-stop ordering) plus re-pointing the order-dependent shape pins. Decision: land the controller + port + verbatim seams with a green battery and a passed live run, and do the orchestration-body split as a dedicated follow-up before the PR 8 native port. `shared/testUtils/webVideoDateFlowSources.ts` provides file-group source readers so the follow-up split does not need another mass pin migration.
- **Contract-pin migration:** 50 test files re-pointed from single-file reads to group readers with every regex pin preserved; 4 pins retargeted intent-preserved to the new decision owner (bothReadyCanonical hydration consumption, Sprint-1 canonical-contract consumption, 1232-1242 ready-gate ownership release, route-lifecycle hydration bounce structure). Fixed pre-existing main breaks found in passing: `profileDirectPrivacyContracts.test.ts` pinned `get_profile_for_viewer` directly in VideoDate/ReadyGateOverlay/native videoDateApi, which had long since moved behind `fetchVideoDatePartnerProfile`/lib owners (re-pointed through the call chain).
- **Battery (all green):** typecheck (web+native), eslint, full `test:video-date-v4` (88 files + 3 new controller suites), `test:video-date:red-flags` (31 files), `test:event-lobby-regression`, web production build (`vite build`), linked migration list aligned, `db push --dry-run` "Remote database is up to date", linked DB lint zero error-level findings. No cloud artifacts deployed (no migrations, no Edge changes).

**Live gate result (2026-06-12, tag `vd-pr7-smoke-20260612-60076`): PASSED.** Fresh disposable pair (SQL-created confirmed auth users, fake `…@vibely.test` emails) ran the full golden flow through this branch's web client (local Vite on port 5173, production Supabase + Daily, two headless Chromium contexts with fake media): lobby deck mutual vibe swipe → Ready Gate both `mark ready` → both on `/date/<id>` → real Daily room join with bilateral fake-camera media → evidence-gated promotion to `date` → manual end (`ended_from_client`) → both post-date surveys (verdict + own-row confirmation) → 2 persisted `date_feedback` rows + mutual `matches` row. Server truth read back: `ready_gate_status=both_ready`, `entry_started_at`/`date_started_at`/`ended_at` set, Daily room stamped, bilateral `participant_*_remote_seen_at` provider-proofed. Tag-scoped cleanup deleted 2 feedback, 1 match, 1 session, 2 swipes, 2 impressions, 2 reservations, 2 registrations, 1 event, 2 profiles, 2 identities, 2 auth users; zero-residue verification returned 0 everywhere. New operational gotchas for future smokes: a DB trigger auto-creates `public.profiles` on `auth.users` insert (shape it with UPDATE, not INSERT) and `protect_sensitive_profile_columns()` requires `set_config('vibely.onboarding_server_update','1',true)` (+ `vibely.verification_server_update` for `email_verified`) in the same transaction; `events.visibility` allows only `all|premium|vip`; dev server must run with `npx vite --port 5173 --strictPort` because the repo config uses port 8080 while Edge CORS admits 5173.

## 2026-06-12 Implementation Update: Client Single-Path Freeze, Web + Native Together (Rebuild PR 6 of 10)

Sixth PR of the staged re-foundation (client-only; no DB migrations, no Edge deploys; PR 5 vocab flip landed separately as #1305). Exactly one client path per concern now exists on the golden flow, with zero `useFeatureFlag("video_date.*")` reads left in `src/` or `apps/mobile/`.

- **One transition API:** the four legacy v2 transition RPC client call paths are gone — `video_session_continue_entry_v2`, `video_session_entry_auto_promote_v2`, `video_session_date_timeout_v2`, `video_session_forfeit_v2` have zero references in `src/` + `apps/mobile/` (generated types still export them; PR 8 drops the DB functions after zero-reference proof). Lifecycle calls go through `video_date_transition` (`vibe`/`pass`, `complete_entry`, `end` incl. `date_timeout` reason, reconnect family) and Ready Gate stage transitions through `ready_gate_transition` (forfeit/snooze/sync). KEPT canonical: `video_session_mark_ready_v2` (sole mark-ready, both platforms), `video_session_request_extension_v2` (sole extension spend now that `extension_mutual_v2` is frozen on — the `video_session_extend_date_v2`/`spend_video_date_credit_extension` client branches are deleted as dead losers).
- **One realtime transport:** the `postgres_changes` lifecycle subscriptions are removed from `src/hooks/useReadyGate.ts`, `src/pages/VideoDate.tsx`, `apps/mobile/lib/readyGateApi.ts`, and `apps/mobile/lib/videoDateApi.ts`; the per-session private broadcast channel (`videoDateSessionChannel` + seq-gap snapshot recovery) is now unconditional, with the existing poll/reconcile fallbacks unchanged. Deliberately retained `postgres_changes` (not lifecycle, not in the freeze list): the ReadyGateOverlay reconcile channel (event_registrations queue-status continuity + video_sessions reconcile, web + native) and the vibe-questions/ice-breaker sync (web `IceBreakerCard.tsx`, native `date/[id].tsx`) — broadcast carries no vibe-question events, so removing them would break in-call sync with no replacement.
- **Flag freeze:** all 23 client-read `video_date.*` keys were checked against live `client_feature_flags` (every row `enabled=true`, `rollout_bps=10000`, `kill_switch_active=false`) and hard-coded to the enabled winner; every branch deleted; `VIDEO_DATE_V4_CLIENT_FEATURE_FLAGS` is now empty (server-read DB rows untouched). Dead code deleted with the branches: web timer-drift telemetry (no-op under timeline_v2), the lobby legacy first-3-card prefetch warm path, native `useCountdown`, `markEventParticipantHeartbeat` (legacy heartbeat RPC client path; readiness_v2 heartbeat is the only client heartbeat now), `isReadyGateResilientClockEnabled`/`isReadyGateResilientBroadcastEnabled`, and the flag-plumbing props/options (`useVideoCall` `resilienceV2`/`dailyTokenRefreshV2`, `InCallSafetyModal`/`InCallSafetySheet` `safetyV2`, `KeepTheVibe` `mutualMode`, `ReconnectionOverlay` `resilienceV2`, native `videoDateApi` per-call v2 option types, `useReadyGate` `readyGateClockEnabled`/`readyGateBroadcastEnabled` return fields).
- **Tests:** 19 parity/contract test files updated to pin the frozen single path (presence assertions flipped to absence for dropped RPCs/flags/branches); `clientFeatureFlagsContracts.test.ts` also had a pre-existing main break (PR #1305 vocab flip vs. seed assertions) fixed in passing. Battery green: typecheck (web+native), eslint, full `test:video-date-v4`, `test:video-date:red-flags`, `test:event-lobby-regression`, linked migration list + dry-run (`Remote database is up to date`), linked DB lint (zero errors, legacy warnings only).

**Live gate result (2026-06-12, tag `vd-pr6-smoke-20260612-025214`):** PASSED. A fresh disposable pair (SQL-created confirmed auth users, fake `…@vibely.test` emails) ran the full golden flow through this branch's web client (local Vite dev server on the PR-6 build, production Supabase + Daily, two headless Chromium contexts with fake media): lobby deck mutual vibe swipe → Ready Gate both `I'm Ready` (`video_session_mark_ready_v2`) → both on `/date/<id>` → real Daily room join with bilateral fake-camera media → evidence-gated auto-promotion to `date` (no explicit entry vibes needed; stable-bilateral-media gate promoted) → manual end (`video_date_transition('end')`) → both post-date surveys → mutual verdict. Server truth read back for session `70973454-58ea-44ca-ae12-200b3e394dc6`: `ready_gate_status=both_ready`, `entry_started_at`/`date_started_at`/`ended_at` set, `ended_reason=ended_from_client`, bilateral `participant_*_remote_seen_at` provider-proofed, Daily room stamped; two persisted `date_feedback` rows (both `liked=true`) and the mutual chat `matches` row created by the verdict chain. Tag-scoped cleanup deleted 2 feedback rows, 1 match, 1 session, 2 swipes, 2 impressions, 2 reservations, 2 registrations, 1 event, 2 profiles, 2 identities, 2 auth users; zero-residue verification returned 0 across auth users, profiles, events, sessions, feedback, registrations, and matches. Operational gotchas recorded for future smokes: Edge CORS allowlist only admits dev port 5173; `is_test_event=true` events are RLS-hidden from non-admin browsers; `event_profile_impressions` (not just swipes/reservations) must be reset to re-deal a previously-vibed pair; SQL-created auth users need empty-string GoTrue token columns; the Ready Gate CTA's accessible name is `Mark ready, NN seconds left`, not "I'm Ready"; the analytics consent banner intercepts bottom-sheet clicks until dismissed.

## 2026-06-11 Implementation Update: Ready Gate Family Single Bodies, Historical Generations Dropped (Rebuild PR 4 of 10)

Fourth PR of the staged re-foundation. Migration `20260611201927_video_date_ready_gate_single_bodies.sql` (applied to linked project `schdyxcunwcvddlcshwd`) rewrites `ready_gate_transition`, `video_session_mark_ready_v2`, and `video_date_ready_gate_actionability_v1` as single self-contained bodies (names/signatures/grants unchanged: authenticated + service_role only) and drops the 23 historical Ready Gate family functions. Corrective migration `20260611210512_ready_gate_single_body_lint_cleanup.sql` (also applied) recreates the identical `ready_gate_transition` body without two unused declarations left by the dropped legacy mark_ready branch; DB lint shows zero issues on all three rewritten functions (legacy backlog untouched). Live markers verified post-apply: `ready_gate_transition_20%` / `video_session_mark_ready_v2_20%` / `rgt_%` / `vd_mark_ready_%` / `vd_ready_gate_actionability_owner_eligibility_base` return zero rows; DB-wide `prosrc` scan shows zero executable references to any dropped name; all 7 already-single-body helpers (`terminalize_event_ready_gates`, `terminalize_stale_pre_date_ready_gate_blockers`, `recover_ready_gate_missing_rooms_v1`, `video_date_terminalize_ready_gate_session_v1`, `video_session_mark_ready_grace_extend_v1`, `handle_event_ready_gate_terminalization`, `persist_ready_gate_suppression_v2`) are intact and untouched.

- **Inventory truth corrections:** the live chains were 9 layers deep on both heads, including six chain-private layers missing from the rebuild plan's enumeration (`rgt_preserve_warmup_base_v1`, `rgt_pre_ready_room_meta_base_v1`, `vd_mark_ready_20260609130139_hot_base`, `vd_mark_ready_both_ready_owner_base`, `vd_mark_ready_terminal_truth_base`, `vd_mark_ready_partial_base`) — all inlined and dropped (referencers were family-internal only). Two detached dead sub-chains (3 `ready_gate_transition` generations ending at `observability_base`, 3 `video_session_mark_ready_v2` generations ending at `grace_base`/`start_snapshot_base`) had no live referencers and were dropped without replacement. The convoy NOWAIT pin died earlier with `record_video_date_ready_gate_entered_v1` (PR #1293); the surviving convoy contract is the role-level `statement_timeout='15s'` cloud config, untouched and still pinned by `readyGateEntryProofRemovalContracts.test.ts`.
- **Behavior preserved (pinned):** the mark_ready bridge onto the idempotent v2 command; both sync fast paths; pre-ready room-metadata repair; event-inactive ownership under the locked row; the sync/snooze/forfeit core machine (expire-stale sweep, expiry re-check under lock, terminal echo idempotency, 2-minute snooze, forfeit + registration cleanup, full before/after observability); canonical both_ready room re-derivation (deterministic `date-<id>`, GUC → latest canonical host → `vibelyapp.daily.co`); the registration-desync forfeit post-check; result-status echo + dual server clock keys; the decisive mark-ready command core exactly (command begin/finish idempotency with replay/reclaim, event-inactive terminalization, mutability guard, expiry under lock, decisive commit with 45s both-ready grace + deterministic room stamping + ensure-room outbox, observability + session event append, auxiliary-error accounting); both-ready entry protection; partner_ready/date_starting notification enqueues with degraded flags; enrichment + both-ready route payload owner; `hot_path_no_throw_shell`/`active_entry_failsoft_shell` markers; the full actionability matrix incl. the both_ready prepare-entry-lease expiry exception (signature unchanged — daily-room Edge and `video_date_transition.prepare_entry` callers untouched).
- **Intentional deltas (documented in the PR):** raw `sqlstate`/`message`/`detail`/`hint` no longer enter authenticated client payloads anywhere in the family, including `auxiliary_errors` entries and `command_finish_*` keys (diagnostics route to `video_date_lifecycle_observe_exception_v2`); the legacy in-chain mark_ready machine (45s grace + `both_ready_provider_prepare_grace_extended` observability + after-ready warmup restore), unreachable behind the bridge, is not carried over; the review-comments duplicate precheck layer (auth/participant/queued/partner-snooze/safety re-checks already performed under the same locks by the actionability precheck) is not carried over.
- **New pinned contracts:** `shared/matching/videoDateReadyGateSingleBodyContracts.test.ts` (10 tests) wired into `test:video-date-v4` and `test:video-date:red-flags`; truth-pin delegation pins re-pointed to `*.single_body_core` markers; identifier-hygiene generated-types pins flipped to absence for the dropped `rgt_*` names; both head fixtures re-dumped from live post-apply (README notes the re-dump). The four `supabase/validation/ready_gate_*` packs were retargeted to single-body live truth and now return all-`ok` against the linked project (26/26), including three checks that had been stale since earlier refactors (terminalize sweep delegation, confirm-entry base delegation, head grant posture).

Latency intent: removes a 9-deep nested SECURITY DEFINER call stack from every mark-ready and every Ready Gate sync/snooze/forfeit call at both-ready time — the exact hot path of the 2026-06-10 lock convoy. Static tests are not product acceptance; the bar remains a fresh two-user run through persisted `date_feedback`.

## 2026-06-11 Implementation Update: Evidence-Family Single Bodies, Historical Generations Dropped (Rebuild PR 3 of 10)

Third PR of the staged re-foundation. Migration `20260611190852_video_date_evidence_single_bodies.sql` (applied to linked project `schdyxcunwcvddlcshwd`) rewrites `claim_video_date_surface`, `mark_video_date_daily_alive`, `mark_video_date_daily_joined`, and `mark_video_date_remote_seen` as single self-contained bodies (names/signatures/grants unchanged) and drops the 26 historical public generations of these families. `release_video_date_surface_claim` was already a single body and is untouched. Live markers verified post-apply: the family patterns (`claim_video_date_surface_20%`, `mark_video_date_daily%_20%`, `mark_video_date_remote_seen_20%`, `vd_claim_surface_%`, `vd_daily_%`, `vd_remote_seen_%`, `vd_alive_strict_provider_base`) return exactly one row — `vd_daily_webhook_terminal_truth_base`, intentionally retained as the live base of `record_video_date_daily_webhook_event_v2` (Daily webhook ledger family, preserved surface). DB-wide `prosrc` scan confirms no executable references to any dropped name remain (two comment-only provenance mentions inside the new bodies).

- **Inventory truth correction:** the alive chain had a 7th live layer not in the rebuild plan's enumeration — `vd_alive_strict_provider_base` sat between `vd_daily_alive_20260609130139_hot_base` and `vd_daily_alive_remote_seen_base`. It was inlined and dropped with the family (only referencer was the hot base). The "orphaned" old alive chain was therefore actually live; the joined chain tail-calls canonical `mark_video_date_daily_alive`, which the new joined single body preserves by design (one heartbeat owner).
- **Behavior preserved (pinned):** surface-claim single-active-owner semantics (per-profile upsert, stale expiry, takeover-gated `SURFACE_CLAIM_CONFLICT`, surface state gating, `video_date_surface_claim_events` ledger + `claim_terminal_audit` terminal-truth stamping); daily_alive heartbeat semantics (lifecycle eligibility precheck, `video_date_current_provider_session_proof_v1` precheck with structured ok:true no-ops, webhook-backed provider-presence gating, 6s/30s presence-event throttle, joined-stamp/away-clear/grace-clear, stable-copresence handshake start via `video_date_stable_copresence_v1`, `event_registrations` in_handshake/in_date continuity); remote_seen provider-proof requirements (owner_id + call_instance_id + provider_session_id + entry_attempt_id + owner_state='joined' + current provider join + fresh 15s owner heartbeat; full 11-code rejection matrix returns structured no-op JSON and never mutates encounter truth; render-evidence source allow-list unchanged); the stable-bilateral-media gate + auto-promote interplay (shared `video_date_promote_confirmed_encounter_v1` / `video_date_promote_provider_overlap_v1` owners and the `video_session_handshake_auto_promote_v2` path untouched, called in the same order); survey continuity (`in_survey` with date_feedback/is_blocked/user_reports guards); the full enrichment pipeline and shell markers; authenticated + service_role-only grants.
- **Intentional deltas (documented in the PR):** raw `sqlstate`/`sql_message`/`message`/`detail`/`hint` no longer enter authenticated client failure payloads (same rule as PR 2) — core exceptions route into `video_date_lifecycle_observe_exception_v2`; pure fail-soft wrapper layers collapse into the single bodies' exception structure (codes preserved: `SURFACE_CLAIM_FAILED`/`SURFACE_CLAIM_WRAPPER_FAILED`, `DAILY_ALIVE_STAMP_FAILED`/`DAILY_ALIVE_FAILED`, `DAILY_JOIN_STAMP_FAILED`, `REMOTE_SEEN_FAILED`/`REMOTE_SEEN_STAMP_FAILED`); dead duplicate auth/participant re-checks on already-locked rows are not carried over; the two detached uuid-only joined generations are dropped without replacement.
- **New pinned contracts:** `shared/matching/videoDateEvidenceSingleBodyContracts.test.ts` (10 tests: signatures/grants, all 26 drops + no dropped-name calls, no-raw-SQL rule, per-family semantics pins, promotion ordering, generated-types absence, webhook-base retention) wired into `test:video-date-v4` and `test:video-date:red-flags`; truth-pin head pins re-pointed from dropped hot-base delegation lines to single-body markers; the 4 changed head fixtures re-dumped from live post-apply (README notes the re-dump); generated types regenerated — 230 lines of dead RPC type surface removed, all 5 canonical heads kept.

Latency intent: removes a 7–8-deep nested SECURITY DEFINER call/enrichment stack from every heartbeat, joined-stamp, remote-seen, and surface-claim call on the date hot path. Static tests are not product acceptance; the bar remains a fresh two-user run through persisted `date_feedback`.

## 2026-06-11 Implementation Update: `video_date_transition` Single Body, Private Chain Dropped (Rebuild PR 2 of 10)

Second PR of the staged re-foundation. Migration `20260611175511_video_date_transition_single_body.sql` (applied to linked project `schdyxcunwcvddlcshwd`) rewrites `public.video_date_transition(uuid,text,text)` as one self-contained body implementing the effective composition of the former 25-generation `private_video_date.vdt_*` chain, then drops all 25 private functions and the `private_video_date` schema. Live markers verified post-apply: `private_video_date` function count = 0, schema absent, `pg_get_functiondef(public.video_date_transition)` contains no `private_video_date` reference and carries the new `single_body_rpc` marker; grants remain authenticated + service_role only.

- **Behavior preserved (pinned):** entry aliases (`complete_entry`/`continue_entry` onto handshake vocabulary until PR 5), exact `ENTER_HANDSHAKE_REMOVED` rejection payload, preflight-only `prepare_entry` (actionability precheck → `video_date_protect_both_ready_entry_v1` → 90s virgin-gate lease grant/refresh → event-inactive block → preflight checks; daily-room Edge stays the only room/token minter), `complete_handshake` delegation to `finalize_video_date_handshake_deadline`, 60s late vibe/pass interception, self/partner-away suppression evidence (presence, remote-seen, surface claims, transport-grace channel), core reconnect/vibe/pass/end machine incl. partial-join peer timeout and pre-date cleanup, the unconfirmed-date guard (`end_unconfirmed_video_date_start`), terminal v2 survey continuity, and the full enrichment pipeline (`enrich_lifecycle_payload_v1` ×2 → `sanitize_client_failsoft_payload_v1` → `enrich_and_sanitize_payload_v2` → `both_ready_route_payload_v1` → shell markers).
- **Intentional deltas (documented in the PR):** raw `sqlstate`/`sql_message`/`message`/`detail`/`hint` no longer enter authenticated client failure payloads — diagnostics route into `video_date_lifecycle_observe_exception_v2`; the superseded v1 survey-continuity pass is dropped (v2 was already the final authority, and v1 could stomp `event_registrations.current_room_id` for re-matched users); the shadowed `vdt_lifecycle_presence` self-away branch and four dead handlers (core `enter_handshake`/`complete_handshake`/`end`, older mutating `prepare_entry`) are not carried over.
- **New pinned contracts:** `shared/matching/videoDateTransitionSingleBodyContracts.test.ts` (12 tests) wired into `test:video-date-v4` and `test:video-date:red-flags`; the truth-pin head test now asserts no `private_video_date` reference plus the `single_body_rpc` marker, and `supabase/contract-fixtures/2026-06/functions/public-heads/video_date_transition.sql` was re-dumped from live post-apply (the 25 private fixtures remain as dropped-chain history; README notes the re-dump).
- **Verification:** transactional ROLLBACK dry-run of the full migration against the remote before apply; post-apply live probes returned the pinned `enter_handshake` rejection and a fully enriched UNAUTHORIZED `sync_reconnect` payload; typecheck, lint, `test:video-date-v4`, red-flags, event-lobby regression, daily-room contract 13/13, migration list aligned, push dry-run "up to date", DB lint 0 errors with no findings on the new body.

Latency intent: this removes the 25-deep nested SECURITY DEFINER call/enrichment stack on the Ready Gate hot path (the 57014-convoy maintainability/latency sink) while keeping the observable contract. Static tests are not product acceptance; the bar remains a fresh two-user run through persisted `date_feedback`.
...
- **#1296 startup snapshot compatibility:** `videoDateStartSnapshotToDateEntryTruth` now falls back from legacy `entry_started_at` / `entry_grace_expires_at` payload keys and normalizes legacy `handshake` phase to canonical `entry` until the startup snapshot RPC is physically updated.
...
- **Edge contract:** web/native post-date outbox calls still send `transition_version: "v3"` and an idempotency key through `post-date-verdict`. The deployed `post-date-verdict` Edge Function is active version `601` (2026-06-11 09:58:23 UTC) and calls only `submit_post_date_verdict_v3`; verdict requests with stale/non-v3 `transition_version` now return `unsupported_transition_version`, and keyless verdict requests return `missing_idempotency_key`. The old `deprecated_version_coerced_to_v3` / deterministic keyless compatibility path is gone.
- **Database contract:** forward migration `20260611094913_remove_legacy_post_date_verdict_rpcs.sql` was applied to linked project `schdyxcunwcvddlcshwd`. It recreates `submit_post_date_verdict_v3` with the full persistence path inline (command idempotency, `post_date_client_submissions`, immutable `date_feedback`, pending-verdict bookkeeping, safety-report handling, session events, and `next_surface` enrichment), then drops `submit_post_date_verdict`, `submit_post_date_verdict_v2`, and `submit_post_date_verdict_20260603090000_remote_seen_base`. Follow-up migration `20260611101241_remove_v3_verdict_unused_lint_variable.sql` is also applied and lint-cleans the live v3 function body without changing its signature.
- **Type/catalog proof:** `npm run regen:supabase-types` regenerated `src/integrations/supabase/types.ts`; generated types expose `submit_post_date_verdict_v3` only. Live catalog markers confirm v1, v2, and the 20260603 base are absent, v3 is present, and the v3 function body contains no v1/v2 delegation.
- **Tests updated:** stale Sprint 5 / Phase 3 contracts now assert the v3-only Edge path, legacy RPC absence, and `date_feedback` persistence semantics instead of preserving v2->v1 behavior.

This is a backend/Edge simplification pass, not product acceptance. The acceptance bar remains a fresh disposable two-user production run through both persisted `date_feedback` rows.
...
- **Verdict path prepared for v3-only.** Web/native `PostDateSurvey`, `apps/mobile/lib/videoDateApi.ts`, and both post-date outbox executors no longer carry `backendVersion`/`submitVerdictV3`; `transition_version: "v3"` is hard-coded. This pass originally left Edge-side stale/keyless coercion and the v1/v2 RPC drop deferred until a release boundary; the 2026-06-11 legacy-verdict removal above supersedes that deferred state and removes the compatibility chain.
...
- `video_date_transition` accepts `complete_entry` / `continue_entry` as aliases for `complete_entry` / `continue_entry` (old actions and the failsoft body unchanged).
- Types regenerated.
...
Phase C Edge-Function migration **deferred deliberately**: it would be behavior-neutral (`entry_started_at` === `entry_started_at`; `phase`/`state === 'entry'` can't change until Phase D), would risk the output payload keys consumers depend on, and would require deploying `daily-room` and the other critical functions to production with no two-user verification — real risk for zero behavior benefit, and the handshake column is not dropped for several phases. Recommend doing it in lockstep with the column-drop/enum-rename phase behind a real e2e window.
...
Subsequent phases (separate sign-off + real e2e window): DB additive compat columns/RPC wrappers/actions, Edge Function migration, `ALTER TYPE video_date_state RENAME VALUE 'entry' → 'entry'`, flag-key rename, types regen.
...
- New shared alias boundary `shared/matching/videoDateEntryTiming.ts` maps legacy DB fields to neutral names: `entry_started_at` -> `entryStartedAtIso`, `entry_grace_expires_at` -> `entryGraceExpiresAtIso`, and `date_started_at` -> `dateStartedAtIso`.
...
- Warm-up timer telemetry source action is now `server_entry_started_at`; active payloads include neutral `entry_started_at` while preserving legacy `entry_started_at` for compatibility.
- Branch delta: `docs/branch-deltas/neutral-entry-timer-aliases.md`.

Preserved:

- `entry_started_at` and `entry_grace_expires_at` remain server timing columns.
- Runtime phase values remain `handshake`, `date`, and `ended`.
- `complete_entry`, Vibe/Pass, `prepare_date_entry`, `video_date_transition('prepare_entry')`, and `video_date_transition('end')` remain active.
...
- `entry_started_at` and `entry_grace_expires_at` remain server timing columns, now aliased in active product countdown code as entry/date timer semantics.
...
- Migration `20260609014410_video_date_stable_bilateral_media_gate.sql` adds `video_date_stable_bilateral_media_gate_v1(...)` and wraps `video_date_promote_provider_overlap_v1(...)`, `video_date_promote_confirmed_encounter_v1(...)`, and `video_session_entry_auto_promote_v2(...)`.
- Deep audit correction migration `20260609022729_video_date_auto_promote_stable_bilateral_media_gate.sql` hardens `video_session_entry_auto_promote_v2(...)` so it checks lifecycle eligibility and stable bilateral media before delegating to the preserved legacy auto-promoter base. This closes the gap where auto-promote previously only tagged `stable_bilateral_media_gate_checked` after delegation.
...
- `SUPABASE_CLI_TELEMETRY_OPTOUT=1 supabase db lint --linked --schema public --fail-on error` exited 0 with only legacy warning/notice output.
- `SUPABASE_CLI_TELEMETRY_OPTOUT=1 supabase db advisors --linked --level error --fail-on error` returned `No issues found`.
- Live catalog query verified deployed markers for `video_date_stable_bilateral_media_gate_v1`, `video_date_promote_provider_overlap_v1`, `video_date_promote_confirmed_encounter_v1`, `video_session_entry_auto_promote_v2`, and the preserved short base functions.
...
- Direct live function verification confirmed `finalize_video_date_entry_deadline(...)` has `has_confirmed_encounter_rescue=true`, `has_positive_extension_v2=true`, `wraps_20260605085010_base=true`, and `old_least_pattern_position=0`.
...
- Console evidence included opaque 500s for `mark_video_date_daily_joined`, `record_video_date_launch_latency_checkpoint`, and `video_session_entry_auto_promote_v2`; those were amplifiers/observability gaps, not the first boundary failure.
...
- `10:36:00.217291Z`: the session ended as `entry_timeout`, `survey_required=true`, `date_started_at=null`, registrations moved to `in_survey`, and no `date_feedback` rows were present.
...
- Deadline rescue: expected the confirmed-encounter rescue to prevent false `entry_timeout`. Actual rescue ran too late; by deadline cleanup, provider left events had already emptied the room, so the server extended once and later ended survey-required.
...
- Primary server root cause: promotion is too delayed. `video_session_entry_auto_promote_v2` still waits for the 60-second handshake deadline before promoting, even when `video_date_session_has_confirmed_encounter(...)` is already true. This leaves the product in a long fragile handshake window after bilateral media evidence exists.
- Deadline-rescue limitation: `finalize_video_date_entry_deadline` can promote an active confirmed encounter at the deadline, but this test shows that deadline is not early enough. By the time cleanup ran, provider leave rows had already been processed, so the active room had been lost.
...
- Server: add an early confirmed-encounter promotion path from `mark_video_date_remote_seen` and/or `video_session_entry_auto_promote_v2`. Once both users have joined and both have canonical remote-seen evidence, and neither user has explicitly passed, promote to `date` immediately rather than waiting for the 60-second deadline.
...
- `video_session_entry_auto_promote_v2` now checks the same helper before delegating to the older deadline-gated command wrapper, so confirmed bilateral media bypasses `entry_auto_promote_not_due`.
- `finalize_video_date_entry_deadline` still delegates to the PR #1199 deadline rescue as fallback, but now checks the same early promotion helper first and restores canonical room metadata after the fallback path.
...
- Live function marker verification confirmed the migration row, early-promotion helper, `mark_video_date_remote_seen` wrapper, `video_session_entry_auto_promote_v2` wrapper, deadline finalizer fallback wrapper, post-base room repair, and `confirmed_encounter_promoted_to_date` / `state = date` helper markers are installed.
...
19. Confirm `mark_video_date_daily_joined` logged `entry_started_after_active_daily_copresence` only after both latest Daily presences were active, and `daily_join_waiting_for_active_partner` only when the partner's latest presence was absent or away.
...
- Whether `participant_1_joined_at`, `participant_2_joined_at`, `participant_1_away_at`, `participant_2_away_at`, `participant_1_remote_seen_at`, `participant_2_remote_seen_at`, `entry_started_at`, and `date_started_at` support active co-presence.
...
- Failure boundary: warm-up/date-start ownership diverged. No `continue_entry` command committed. Two `entry_auto_promote` commands raced at `07:53:00Z`: the first returned `state=handshake`, `extended=true`, `reason=entry_launch_evidence_extension`, but `seconds_remaining=0`; the second immediately ended the session with `ended_reason=entry_timeout`, `survey_required=true`, `date_started_at=null`, and no persisted user decisions. One user remained `in_survey`, the other later went `offline`; `date_feedback` had no rows.
- Root cause identified in deployed SQL: `finalize_video_date_entry_deadline` still uses `entry_started_at = LEAST(v_now, v_latest_launch_evidence_at)` for launch-evidence extension and does not consult `video_date_session_has_confirmed_encounter`. In this session, latest launch evidence was already more than 60 seconds old by the time the finalizer ran, so the extension returned zero seconds and allowed the peer client to terminalize immediately. Existing contract `shared/matching/videoDateDefinitiveHandoffRecovery.test.ts` currently asserts this flawed `LEAST(...)` pattern.
...
- Implemented local migration `20260605085010_video_date_confirmed_encounter_deadline_rescue.sql`. It wraps `finalize_video_date_entry_deadline`, restores canonical Daily metadata before deadline decisions, promotes active confirmed-encounter/no-pass/no-both-decided handshakes to `date`, updates both registrations to `in_date`, records `confirmed_encounter_deadline_promoted_to_date`, and replaces the zero-second `LEAST(...)` launch-evidence extension with `entry_started_at = v_now` plus a positive `seconds_remaining` response.
- Hardened web/native/mobile clients for the repaired deadline contract. Web `VideoDate.tsx` and native `apps/mobile/app/date/[id].tsx` now treat positive `state=handshake, extended=true` responses as real deadline extensions, clear stale retry keys, refresh local countdowns, and avoid immediately retrying an already-repaired deadline. Warm-up `warmup_timer_started` telemetry is session-deduped so server-side `entry_started_at` repairs do not create repeated timer-start metrics.
...
- Latest session timeline: match/session created at `2026-06-05T22:14:44.285514Z`; both users marked Ready Gate ready at `22:14:47.530303Z` and `22:14:47.659992Z`; canonical Daily room `date-4082fe3684804d309a1d1de227b855e3` was verified; Daily joins arrived at `22:14:52.071Z` and `22:14:53.163Z`; `entry_started_after_active_daily_copresence` fired at `22:14:53.504822Z`; `confirmed_encounter_promoted_to_date` fired at `22:14:53.687453Z`; `date_started_at` became `22:14:53.753531Z`.
...
- **Stale Sprint 5 assertion fixed:** the safety-report forced-pass contract now pins the v3-only reality and asserts the retired v2/legacy RPC branches stay removed. The 2026-06-11 verdict cleanup later changes stale/keyless Edge verdict callers from coercion to explicit `unsupported_transition_version` / `missing_idempotency_key` failures.
