...
Current recovery overlay (2026-06-05): start active Video Date work from `docs/video-date-success-command-center.md`. Functional Video Date code landed in PR #1200 at merge commit `fbca4996a096273914ee650b556ba7994477aa5e`; verify current Git state before assuming no docs-only follow-up sits on top. Supabase migrations through `20260605115657_video_date_early_confirmed_encounter_promotion.sql` make active Daily co-presence, canonical remote-seen, immediate confirmed-encounter promotion, and confirmed-encounter deadline fallback recovery latest-state models. Metrics that use `participant_*_joined_at` measure route/Daily join latency only unless joined evidence is newer than away/left evidence. For co-presence and terminal recovery, correlate with `participant_*_away_at`, `participant_*_remote_seen_at`, `reconnect_grace_ends_at`, `date_started_at`, `video_date_daily_webhook_events`, and observability reasons such as `reconnect_grace_cleared_by_provider_join`, `reconnect_grace_expiry_suppressed_latest_presence`, `confirmed_encounter_promoted_to_date`, `confirmed_encounter_deadline_promoted_to_date`, `handshake_deadline_extended_for_launch_evidence_v2`, `daily_call_cleanup`, and `daily_call_reuse`.
