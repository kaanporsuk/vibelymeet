...
- Replaced the historical 30-function baseline and stale JWT counts with the current 49-function repo inventory.
...
- Updated Section 13 of the rebuild runbook to record:
  - 49 deployable function directories excluding `_shared`
  - 49 `[functions.<slug>]` entries in `supabase/config.toml`
  - 31 `verify_jwt = true` functions
  - 18 `verify_jwt = false` functions
...
- runbook Section 13 uses the current 49-function inventory
