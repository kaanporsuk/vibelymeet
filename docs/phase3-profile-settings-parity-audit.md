...
| **Verification** | VerificationSteps (email, photo, phone) with step labels and actions (e.g. open photo verification) | Card, single paragraph: "Verify your email, photo, and phone on www.vibelymeet.com..." | **Functional gap:** web has actionable steps; native is text-only CTA to web |
...
| **Privacy** | glass-card row → opens Privacy drawer (toggles: Online Status, Last Seen, Read Receipts, Location Discovery, Show Age) | SettingsRow → Linking.openURL(www.vibelymeet.com/settings) | Web: drawer with toggles; native: external link only. |
