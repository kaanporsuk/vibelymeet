...
- The same migration preserves delayed Daily provider join/left truth by webhook `occurred_at` into participant provider-proof columns and an append-only `daily_webhook_historical_truth` presence event, even when the session is already terminal and state mutation is no longer allowed.
...
- Delayed Daily webhook facts are historical evidence. They must be preserved by provider `occurred_at` even if terminal state correctly blocks further lifecycle mutation.
