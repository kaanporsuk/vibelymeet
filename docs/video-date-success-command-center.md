...
Static tests are not product acceptance; the required gate for this PR is a fresh two-user disposable-tagged production smoke through two persisted `date_feedback` rows with tag-scoped cleanup (recorded below once run).
