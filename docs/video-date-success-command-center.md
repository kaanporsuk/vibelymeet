...
- Linked Supabase read-only verification passed: migration `20260609152000` exists on remote, post-apply dry-run returned `Remote database is up to date`, linked public-schema lint exited 0 with only legacy warning-level notices, error-level advisors returned `No issues found`, and marker SQL showed zero `find_mystery_match%` routines, zero `session_source = 'mystery_match'` sessions, default `'reciprocal_swipe'::text`, and validated constraint `video_sessions_session_source_rec_swipe_only`.
...
- PR split guidance remains: client/product removal, backend function/data cleanup, tests/validation cleanup, optional `session_source` constraint/default cleanup, and current docs cleanup. Do not merge this as one giant PR unless explicitly directed.
