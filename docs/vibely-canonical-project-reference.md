...
Current recovery status: the 2026-04-30 handshake release was deployed, but active Video Date recovery supersedes that closure. Functional Video Date code landed in PR #1200 at merge commit `fbca4996a096273914ee650b556ba7994477aa5e`. Current terminal-survey lifecycle hardening adds `20260605135616_video_date_terminal_survey_lifecycle_hardening.sql`, `20260605143637_video_date_terminal_room_metadata_backfill.sql`, `20260605144003_video_date_terminal_room_metadata_corrective_backfill.sql`, `20260605145306_video_date_terminal_room_cleanup_preserve_metadata.sql`, `20260605145926_video_date_terminal_room_metadata_final_repair.sql`, `20260605150130_video_date_terminal_room_metadata_historical_delete_marker.sql`, and `20260605152058_video_date_pending_survey_registration_repair.sql` for sticky survey status, current-peer-vs-historical-encounter separation, pending-survey registration repair, terminal Daily room repair/backfill, and provider-delete marker tracking without erasing forensics. The 2026-06-06 single-owner runtime hardening adds `20260605232304_video_date_single_owner_runtime_hardening.sql` for active date/survey route ownership, fail-soft transition/queue/surface RPCs, append-only surface-claim audit, and widened client stuck observability. The third-pass native audit adds notification pending-survey deep-link recovery: native notification taps to `/date/:sessionId` must obey the same date/survey route-owner contract and must not send pending-survey terminal truth to lobby/tabs. The cleanup/outbox Edge Functions were redeployed with the same preservation invariant. Verify current Git/Supabase state before assuming deployment, and fresh manual match -> survey acceptance proof is still required.
...
- `video_date_transition`, `get_video_date_queue_hint_v1`, `drain_match_queue_v2`, and `claim_video_date_surface` should fail soft with structured retryable JSON under stale/duplicate/terminal churn.
