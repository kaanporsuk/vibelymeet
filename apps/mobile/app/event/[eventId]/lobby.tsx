...
export default function EventLobbyScreen() {
  ...
  const eventEndTime = useMemo(
    () =>
      eventDateValue
        ? getEventEndTime(eventDateValue, eventDurationMinutes)
        : null,
  ...
  const sameEventActiveSession = useMemo(() => {
    if (
      ...
      !id ||
      !scopedSession ||
      scopedSession.eventId !== id
    )
      return null;
    return scopedSession;
  }, [sessionHydrated, id, scopedSession]);
  const sameEventActiveSessionQueueStatus =
    (sameEventActiveSession as { queueStatus?: unknown } | null)?.queueStatus ??
    null;
  ...
  const {
    ...
    deckState,
    refetch: refetchDeck,
  } = useEventDeck(id, user?.id ?? null, deckQueryEnabled, {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      void queryClient.invalidateQueries({
        queryKey: ["event-deck", id, user.id],
      });
  ...
  const [readyGateLobbyToast, setReadyGateLobbyToast] = useState<{
    text: string;
  ...
  const onReadyGateLobbyMessage = useCallback(
    (text: string, variant?: "info" | "success") => {
  ...
  useEffect(() => {
    if (readyGatePressureActive) return;
  ...
  const [swipeRateLimitUntilMs, setSwipeRateLimitUntilMs] = useState<
    number | null
  >(null);
  const [activeSessionPartnerName, setActiveSessionPartnerName] = useState<
    string | null
  >(null);
  ...
  const deckRefreshBurstTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  const lobbyRefreshBurstTimerRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
    ...
    (
      ...
      if (
        ...
        rcBreadcrumb(
          ...
          "date_nav_suppressed_before_prepare",
          {
      ...
      void (async () => {
        ...
        const routeOwned = isVideoDateRouteOwned(
          sessionIdToOpen,
          user?.id ?? null,
        );
        ...
        if (!startable.ok) {
          if (
            routeOwned &&
            (startable.recommend === "ready" || startable.recommend === "lobby")
          ) {
            rcBreadcrumb(
              RC_CATEGORY.lobbyDateEntry,
              "date_route_decision_suppressed_by_ownership",
              {
                session_id: sessionIdToOpen,
                event_id: id,
                trigger,
        ...
        if (trigger === "ready_gate_overlay") {
          ...
          setActiveSessionId((current) =>
            current === sessionIdToOpen ? null : current,
          );
          ...
          setTimeout(() => {
            ...
            void (async () => {
              ...
              if (onDate) {
                rcBreadcrumb(
                  RC_CATEGORY.lobbyDateEntry,
                  "date_navigation_rescue_skipped",
                  {
              ...
              if (
                ...
                rcBreadcrumb(
                  ...
                  "date_navigation_rescue_skipped",
                  {
                    ...
                    pathname: p,
                    entry_transition_active:
                      isDateEntryTransitionActive(rescueSid),
                    date_route_owned: isVideoDateRouteOwned(
                      rescueSid,
                      user?.id ?? null,
                    ),
                  },
              ...
              rcBreadcrumb(
                RC_CATEGORY.lobbyDateEntry,
                "date_navigation_rescue_attempt",
                {
                  session_id: rescueSid,
                  event_id: id,
                  pathname: p,
              ...
              const rescueStartable =
                await ensureVideoDateStartableBeforeNavigation({
                  sessionId: rescueSid,
              ...
              rcBreadcrumb(
                RC_CATEGORY.lobbyDateEntry,
                "date_navigation_rescue_canonical_recheck",
                {
                  session_id: rescueSid,
                  event_id: id,
              ...
              rcBreadcrumb(
                RC_CATEGORY.lobbyDateEntry,
                rescueNavigated
                  ? "date_navigation_rescue_success"
                  : "date_navigation_rescue_failed",
                {
                  session_id: rescueSid,
                  event_id: id,
  ...
  const readyGateStallFallbackTimerRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  useEffect(() => {
    ...
    readyGateStallFallbackTimerRef.current = setTimeout(() => {
      ...
      if (
        g.serverKind !== "ready_gate" ||
        g.serverSessionId !== sid ||
        g.overlaySessionId !== sid
      ) {
        return;
      }
  ...
  const scheduleDeckRefresh = useCallback(
    (source: string, delayMs = 180) => {
      ...
      if (readyGatePressureActive) {
        vdbg("lobby_deck_refresh_suppressed_ready_gate", {
          eventId: id,
          source,
        });
        return;
  ...
  const startOptimisticSwipe = useCallback(
    (profile: DeckProfile, swipeType: "vibe" | "pass" | "super_vibe") => {
      ...
      return { remainingVisible, swipeSequence };
    },
    [
  ...
  const openReadyGateWithSession = useCallback(
    (sessionId: string, trigger = "unknown") => {
      ...
      if (
        sameEventActiveSession?.kind === "video" &&
        sameEventActiveSession.sessionId === sessionId
      ) {
        rcBreadcrumb(
      ...
      if (isReadyGateManualExitSuppressed(sessionId)) {
        rcBreadcrumb(
          RC_CATEGORY.readyGate,
          "ready_gate_open_suppressed_after_manual_exit",
          {
            session_id: sessionId,
            event_id: id,
            trigger,
      ...
      void (async () => {
        try {
          ...
          const canonicalLog = canonicalVideoDateRouteLogDetail(
            canonicalRoute,
            {
              sourceSurface: "event_lobby",
          ...
          if (canonicalRoute.target === "survey") {
            ...
            navigateToDateSession(
              sessionId,
              `ready_gate_open_${trigger}_pending_survey`,
              "replace",
              {
                force: true,
                forceSurvey: true,
          ...
          if (canonicalRoute.target === "date") {
            if (lastOpenedSessionRef.current !== sessionId) {
              vdbg(
                "lobby_date_route_decision_suppressed_stale_ready_gate_open",
                {
            ...
            rcBreadcrumb(RC_CATEGORY.lobbyDateEntry, "date_route_decision", {
              ...
              ready_gate_expires_at:
                session.ready_gate_expires_at == null
                  ? null
                  : String(session.ready_gate_expires_at),
  ...
  const suppressReadyGateAfterManualExit = useCallback(
    (sessionId: string) => {
  ...
  useEffect(() => {
    if (id && user?.id && lobbySideEffectsEnabled)
      scheduleLobbyRefreshBurst("lobby_side_effects_enabled", 0);
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          const currentRoomId = newData.current_room_id as
            | string
            | null
            | undefined;
          ...
          if (
            ...
            if (
              latestReg?.queue_status === "in_ready_gate" &&
              latestReg.current_room_id
            ) {
              openReadyGateWithSession(
                latestReg.current_room_id,
                "registration_realtime_refetch",
              );
            }
          }
        },
  ...
  useEffect(() => {
    ...
    const handleVideoSessionUpdate = async (payload: {
      ...
      if (routeDecision.target === "survey") {
        ...
        navigateToDateSession(
          ...
          "video_session_update_pending_survey",
          "replace",
          {
            force: true,
            forceSurvey: true,
    ...
    const handleVideoSessionInsert = async (payload: {
      new: Record<string, unknown>;
    }) => {
      const session = payload.new as Record<string, unknown>;
      if (session.event_id !== id) return;
      ...
      if (routeDecision.target === "survey") {
        ...
        navigateToDateSession(
          sid,
          "video_session_insert_pending_survey",
          "replace",
          {
            force: true,
            forceSurvey: true,
  ...
  useEffect(() => {
    if (!id || !user?.id || !lobbyBroadcastSessionId) {
      lobbyBroadcastSessionSeqRef.current = null;
  ...
  const timeRemaining = formatEventCountdown(eventEndTimeMs, lobbyClockMs);
      ...
      switch (result) {
        ...
        case "already_matched":
          if (options?.openingReadyGate) {
            ...
            show({
              ...
              message:
                "We'll bring you to Ready Gate when this match is ready.",
              variant: "info",
              primaryAction: { label: "OK", onPress: () => {} },
            });
          }
        ...
        case "event_not_active":
          show({
            ...
            message:
              "This event was cancelled or is no longer available for swipes.",
            variant: "warning",
            primaryAction: { label: "OK", onPress: () => {} },
          });
          break;
  ...
  const currentSwipePending = current
  ...
  const currentCardRetryState = swipeRateLimited
    ...
    : null;
  const swipeActionsDisabled =
    currentSwipePending || !currentIsSwipeable || swipeRateLimited;
  ...
  useEffect(() => {
    ...
    if (!deckQueryEnabled || deckLoading || deckError || pauseStatus.isPaused)
      return;
    if (
      !isLobbyFocused ||
      appState !== "active" ||
    ...
    function markVisible() {
      ...
      void (async () => {
        try {
          ...
          if (__DEV__)
            console.warn("[lobby] top-card visible mark failed:", err);
          scheduleRetry();
        }
      })();
    }
    ...
    return (
      <>
        <View style={[styles.centered, { backgroundColor: theme.background }]}>
          <LoadingState
            title="Loading lobby..."
            message="Getting the lobby ready..."
          />
        </View>
        {dialog}
      </>
  ...
  const discoverSectionIntro = (
    <View style={styles.sectionIntro}>
      <Text style={[styles.sectionKicker, { color: theme.textSecondary }]}>
        Discover
      </Text>
  ...
  const handleSwipe = async (swipeType: "vibe" | "pass" | "super_vibe") => {
    if (
      !current ||
      pendingSwipeTargetIds.has(current.id) ||
      pendingSwipeTargetIdsRef.current.has(current.id) ||
      !lobbySideEffectsEnabled
    )
      return;
    ...
    try {
      ...
      if (envelope.success === false) {
        ...
        if (shouldAdvanceLobbyDeckAfterSwipe(failureOutcome)) {
          ...
          shouldRestoreVideoDateDeckCardAfterSwipeFailure(failureOutcome)
        ) {
          restoreDeckProfileAfterOptimisticSwipe(targetProfile, swipeSequence);
        }
        return;
      ...
      trackEvent(
        EventLobbyObservabilityEvents.LOBBY_SWIPE_RESULT,
        lobbySwipeResultPayload,
      );
      if (isDuplicateSwipeResult(normalizedEnvelope)) {
        trackEvent(
          EventLobbyObservabilityEvents.LOBBY_SWIPE_DUPLICATE_SUPPRESSED,
          {
            platform: "native",
            event_id: id,
            swipe_type: swipeType,
            outcome,
            reason:
              getSwipeNotificationSuppressionReason(normalizedEnvelope) ??
              "already_swiped",
            source_surface: "event_lobby",
          },
        );
      ...
      const openingReadyGate =
        shouldOpenReadyGateFromSwipePayload(normalizedEnvelope);
      ...
      const readyGateOpenSuppressed = Boolean(
        openingReadyGate &&
        videoSessionId &&
        isReadyGateManualExitSuppressed(videoSessionId),
      );
      ...
      if (conflictDetected) {
        ...
        if (videoSessionId && openingReadyGate) {
          trackEvent(
            LobbyPostDateEvents.SIMULTANEOUS_SWIPE_RECOVERY_ATTEMPTED,
            {
              platform: "native",
              event_id: id,
          ...
          const recoveryDurationMs =
            recoveryStartedAtMs == null
              ? null
              : Math.max(0, Date.now() - recoveryStartedAtMs);
      ...
      if (openingReadyGate && videoSessionId) {
        if (readyGateOpenSuppressed) {
          rcBreadcrumb(
            RC_CATEGORY.readyGate,
            "swipe_ready_gate_open_suppressed_after_manual_exit",
            {
              session_id: videoSessionId,
              event_id: id,
            },
          ...
          if (outcome === "already_matched") {
            const recoveryDurationMs =
              recoveryStartedAtMs == null
                ...
                : Math.max(0, Date.now() - recoveryStartedAtMs);
            trackEvent(
              LobbyPostDateEvents.SIMULTANEOUS_SWIPE_RECOVERY_SUCCEEDED,
              {
                platform: "native",
                event_id: id,
                session_id: videoSessionId,
      ...
      if (!shouldAdvanceDeck) {
        if (shouldRestoreVideoDateDeckCardAfterSwipeFailure(outcome)) {
          restoreDeckProfileAfterOptimisticSwipe(targetProfile, swipeSequence);
        }
        return;
      }
  ...
  return (
    <>
            ...
            <View style={styles.headerRow}>
              ...
              <View style={styles.headerCenter}>
                ...
                <View style={styles.headerLiveRow}>
                  <View
                    style={[
                      styles.livePillStrong,
                      {
                        backgroundColor: withAlpha(theme.success, 0.18),
                        borderColor: withAlpha(theme.success, 0.45),
                      },
                    ]}
                  >
                    <View
                    ...
                    />
                    <Text style={[styles.liveTextStrong, { color: "#86efac" }]}>
                      Live now
                    </Text>
                  </View>
                </View>
            ...
            {hasCards && !pauseStatus.isPaused ? (
              <View style={styles.deckProgressSection}>
                <View style={styles.deckProgressLabels}>
                  ...
                  <Text
                    style={[styles.deckProgressCount, { color: theme.text }]}
                  >
          ...
          <View style={styles.body}>
            {pauseStatus.isPaused ? (
              <View style={styles.onBreakWrap}>
                ...
                />
                <Text style={[styles.onBreakTitle, { color: theme.text }]}>
                  {"You're on a break"}
                </Text>
                <Text
                  style={[
                    styles.onBreakSubtitle,
                    { color: theme.textSecondary },
                  ]}
                >
                  {pauseStatus.isTimedBreak && pauseStatus.pausedUntil
                    ? `Discovery is paused until ${pauseStatus.pausedUntil.toLocaleString(
                        "en-US",
                        {
                          month: "short",
                          day: "numeric",
                ...
                <Text
                  style={[styles.onBreakBody, { color: theme.textSecondary }]}
                >
                  {
              ...
              <>
                {yieldingToVideoDateUi || yieldingToReadyGateUi ? (
                  ...
                  <>
                    {discoverSectionIntro}
                    {postSurveyReturnContext ? (
                      <PostSurveyLobbyBridge
                        decision={postSurveyContinuityDecision}
                        theme={theme}
                      />
                    ) : null}
                  ...
                  <>
                    ...
                    <View style={styles.deckContainer}>
                      {thirdProfile && (
                        <View
                          style={[styles.stackCard, styles.stackCardBack3]}
                          pointerEvents="none"
                        >
                          <LobbyProfileCard
                      ...
                      {nextProfile && (
                        <View
                          style={[styles.stackCard, styles.stackCardBack2]}
                          pointerEvents="none"
                        >
                          <LobbyProfileCard
                            profile={nextProfile}
                    ...
                    <View style={styles.actions}>
                      ...
                      <Pressable
                        style={[
                          ...
                          {
                            backgroundColor: withAlpha(theme.neonYellow, 0.14),
                            borderColor: withAlpha(theme.neonYellow, 0.55),
                          },
                          swipeActionsDisabled && styles.actionDisabled,
                        ...
                        disabled={
                          swipeActionsDisabled || superVibeRemaining <= 0
                        }
                        accessibilityRole="button"
                      ...
                      >
                        ...
                        {superVibeRemaining > 0 && (
                          <View
                            style={[
                              styles.superVibeBadgeCount,
                              { backgroundColor: theme.neonYellow },
                            ]}
                          >
                            <Text style={styles.superVibeBadgeCountText}>
                              {superVibeRemaining}
                            </Text>
                          </View>
                        )}
            ...
            <ReadyGateOverlay
              ...
              onNavigateToDate={(sessionIdToOpen) => {
                rcBreadcrumb(
                  ...
                  { event_id: id, session_id: sessionIdToOpen },
                );
                lastOpenedSessionRef.current = null;
                setActiveSessionId(null);
                setActiveSessionPartnerName(null);
      ...
      {readyGateLobbyToast ? (
        ...
        >
          <Text style={[styles.lobbyTransientToastText, { color: theme.text }]}>
            {readyGateLobbyToast.text}
          </Text>
        </View>
      ) : null}
      {dialog}
  ...
  return (
    ...
    >
      <View style={styles.postSurveyBridgeTitleRow}>
        <View
          style={[styles.postSurveyBridgeDot, { backgroundColor: accent }]}
        />
        <Text style={[styles.postSurveyBridgeTitle, { color: theme.text }]}>
          {decision.title}
        </Text>
      </View>
      <Text
        style={[styles.postSurveyBridgeMessage, { color: theme.textSecondary }]}
      >
        {decision.message}
      </Text>
    </View>
...
function LobbyProfileCard({
  ...
  const showQueueBadge =
    ...
    (profile.queue_status &&
      !["browsing", "idle"].includes(profile.queue_status));
  ...
  const intentDisplay = intentRaw
    ? getRelationshipIntentDisplaySafe(intentRaw)
    : null;
  const prewarmFullProfile = useCallback(
    (trigger: "press_in" | "press" = "press_in") => {
  ...
  return (
    ...
    >
      ...
      {!isBehind && retryState && retryState.remainingSeconds > 0 ? (
        <View
          style={[
            styles.cardRetryState,
            {
              backgroundColor: "rgba(0,0,0,0.68)",
              borderColor: withAlpha(theme.neonYellow, 0.42),
            },
          ]}
          accessibilityLiveRegion="polite"
        >
          <Ionicons name="timer-outline" size={15} color={theme.neonYellow} />
          <Text style={[styles.cardRetryStateText, { color: "#fde68a" }]}>
            Retry in {retryState.remainingSeconds}s
          </Text>
        </View>
      ) : null}
      {profile.has_super_vibed && (
        <View
          style={[
            styles.superVibeBadge,
            {
              backgroundColor: withAlpha(theme.neonYellow, 0.18),
              borderColor: withAlpha(theme.neonYellow, 0.48),
            },
          ]}
        >
          <Ionicons name="sparkles" size={14} color={theme.neonYellow} />
          <Text
            style={[styles.superVibeText, { color: theme.neonYellow }]}
            numberOfLines={1}
          >
            Wants to meet you
          </Text>
      ...
      <View style={styles.cardBody}>
            ...
            {photoVerified ? (
              <View
                style={[
                  styles.trustChip,
                  {
                    backgroundColor: withAlpha(theme.neonCyan, 0.22),
                    borderColor: withAlpha(theme.neonCyan, 0.35),
                  },
                ]}
              >
                <Ionicons name="shield-checkmark" size={12} color="#a5f3fc" />
                <Text style={[styles.trustChipText, { color: "#cffafe" }]}>
                  Verified
                </Text>
              </View>
            ) : null}
              ...
              <View
                style={[
                  styles.trustChip,
                  {
                    ...
                    borderColor: withAlpha(theme.tint, 0.35),
                  },
                ]}
              >
                <Ionicons name="people-outline" size={12} color={theme.tint} />
                <Text style={[styles.trustChipText, { color: theme.tint }]}>
                  Connected
                </Text>
              </View>
            ) : null}
        ...
        {intentDisplay ? (
          <Text
            style={[
              styles.cardLookingFor,
              { borderLeftColor: withAlpha(theme.tint, 0.65) },
            ]}
            numberOfLines={1}
          >
        ...
        {profile.job || profile.location || heightLabel ? (
          <View style={styles.jobLocationRow}>
            ...
            {heightLabel ? (
              <Text style={styles.heightMeta}>{heightLabel}</Text>
            ) : null}
          </View>
        ) : null}
        {profile.about_me ? (
