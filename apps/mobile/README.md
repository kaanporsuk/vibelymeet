...
- **Sprint 5:** Live video date: join Daily.co room via `daily-room` (`prepare_date_entry`), preserve entry/date timer truth through prepare-entry state, use `video_date_transition('end')` for backend end, local/remote video, end/leave with `delete_room` + backend end. See `docs/mobile-sprint5.md`.
