...
export function useReadyGate(sessionId: string | null | undefined, userId: string | null | undefined) {
  ...
  const fetchSession = useCallback(async () => {
    ...
    const { data: session, error } = await supabase
      .from('video_sessions')
      .select('participant_1_id, participant_2_id, ready_gate_status, ready_participant_1_at, ready_participant_2_at, snoozed_by, snooze_expires_at')
      .eq('id', sessionId)
      .maybeSingle();
    ...
    setState({
      status: session.ready_gate_status ?? 'queued',
      iAmReady: !!myReadyAt,
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'video_sessions', filter: `id=eq.${sessionId}` }, (payload) => {
        const s = payload.new as Record<string, unknown>;
        const isP1 = s.participant_1_id === userId;
        setState((prev) => ({
          ...prev,
          status: (s.ready_gate_status as string) ?? prev.status,
          iAmReady: !!(isP1 ? s.ready_participant_1_at : s.ready_participant_2_at),
          partnerReady: !!(isP1 ? s.ready_participant_2_at : s.ready_participant_1_at),
