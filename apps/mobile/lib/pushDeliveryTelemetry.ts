...
function sanitizePushTelemetryProps(properties: PushTelemetryProperties): Record<string, string | number | boolean> {
  const clean: Record<string, string | number | boolean> = {};
  for (const [key, value] of Object.entries(properties)) {
    if (!ALLOWED_PUSH_TELEMETRY_PROPS.has(key) || value === undefined || value === null) continue;
    clean[key] = value;
  }
  return clean;
}
...
export function recordPushDeliveryTelemetry(
  ...
  rcBreadcrumb(RC_CATEGORY.pushDelivery, eventName, safeProps);
}
...
function normalizePushAppPath(rawPath: string): string | null {
  return normalizeNotificationAppPath(rawPath, 'native');
}
