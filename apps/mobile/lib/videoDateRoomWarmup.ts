...
const roomWarmupInflight = new Map<
  string,
  Promise<VideoDateRoomWarmupResult>
>();

function roomWarmupKey(sessionId: string, userId?: string | null): string {
  return `${sessionId}:${userId ?? 'unknown'}`;
}

function coalesceRoomWarmupResult(
  result: VideoDateRoomWarmupResult,
): VideoDateRoomWarmupResult {
...
export function videoDateRoomWarmupAfterReadyEnabled(): boolean {
  return (
    String(
      process.env.EXPO_PUBLIC_VIDEO_DATE_ROOM_WARMUP_AFTER_READY ?? 'true',
    ).toLowerCase() === 'true'
  );
}
...
export async function ensureVideoDateRoomWarmup(
  ...
  const key = roomWarmupKey(sessionId, options.userId);
  const existing = roomWarmupInflight.get(key);
  if (existing) return existing.then(coalesceRoomWarmupResult);
  ...
  const task = (async (): Promise<VideoDateRoomWarmupResult> => {
    try {
      ...
      if (!error && hasVideoDateRoomWarmupPayload(data)) {
          ...
          buildReadyGateToDateLatencyPayload({
            ...
            extra: {
              ...
              room_create_or_verify_ms:
                data.timings?.room_create_or_verify_ms ?? null,
            },
          }),
        );
        ...
        return {
          ...
          data,
          coalesced: false,
          ownerEntryAttemptId: data.entry_attempt_id ?? entryAttemptId,
        };
      }
      ...
      return {
        ok: false,
        code,
        message: readVideoDateRoomWarmupFailureMessage(
          data,
  ...
  roomWarmupInflight.set(key, task);
  try {
    return await task;
  } finally {
    roomWarmupInflight.delete(key);
  }
}
