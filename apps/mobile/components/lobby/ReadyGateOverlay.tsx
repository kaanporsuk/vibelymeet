...
function prepareEntryFailureMessage(code: string): string {
  return resolveReadyGatePrepareEntryFailureCopy({ code, platform: 'native' })
    .message;
}
...
export function ReadyGateOverlay({
  ...
  const [prepareEntryFailure, setPrepareEntryFailure] =
    useState<PrepareEntryFailure | null>(null);
  ...
  const suppressDuplicateNav = useCallback(
    (source: string) => {
      ...
      trackNativeReadyGateEvent(
        NativeReadyGateEvents.DUPLICATE_NAV_SUPPRESSED,
        {
          source,
  ...
  const suppressDuplicateTerminal = useCallback(
    (source: string, recoveryInput?: ReadyGateTerminalRecoveryInput) => {
      ...
      trackNativeReadyGateEvent(
        NativeReadyGateEvents.DUPLICATE_TERMINAL_SUPPRESSED,
        {
          source,
          source_action: source,
          ...
          for (
            ...
            if (inactivePrepareBlocker) {
              ...
              if (nonRetryablePrepareFailureRef.current !== inactiveKey) {
                ...
                trackNativeReadyGateEvent(
                  NativeReadyGateEvents.PREPARE_ENTRY_EVENT_INACTIVE,
                  {
                    source,
                ...
                rcBreadcrumb(
                  RC_CATEGORY.readyGate,
                  'native_prepare_entry_event_inactive',
                  {
                    event_id: eventId,
                    session_id: sessionId,
                    source,
            ...
            rcBreadcrumb(
              RC_CATEGORY.readyGate,
              'prepare_date_entry_failed_before_nav',
              {
                event_id: eventId,
                session_id: sessionId,
                source,
            ...
            if (retryable && isTerminalReadyGateTruth(latestTruth)) {
              ...
              rcBreadcrumb(
                RC_CATEGORY.readyGate,
                'prepare_date_entry_retry_cancelled_terminal',
                {
                  event_id: eventId,
                  session_id: sessionId,
                  source,
  ...
  const handleBothReady = useCallback(
    (
      ...
      rcBreadcrumb(RC_CATEGORY.readyGate, 'lobby_overlay_both_ready', {
        eventId,
        source_action: sourceAction,
      });
  ...
  const handleForfeited = useCallback(
    async (_reason: 'timeout' | 'skip', detail?: ReadyGateTerminalDetail) => {
      const recoveryInput: ReadyGateTerminalRecoveryInput = {
        status:
          detail?.status ?? (_reason === 'timeout' ? 'expired' : 'forfeited'),
        reason:
          detail?.reason ??
          (_reason === 'timeout' ? 'ready_gate_expired' : 'ready_gate_forfeit'),
      ...
      rcBreadcrumb(RC_CATEGORY.readyGate, 'lobby_overlay_forfeited', {
        reason,
        eventId,
      });
  ...
  const guardedSyncSession = useCallback(
    async (
      ...
      if (!options.allowWhileMarking && readyActionInFlightRef.current) {
        rcBreadcrumb(RC_CATEGORY.readyGate, 'sync_suppressed_mark_ready_in_flight', {
          session_id: sessionId,
          event_id: eventId,
          source,
        });
      ...
      if (guardedSyncCooldownUntilMsRef.current > nowMs) {
        rcBreadcrumb(RC_CATEGORY.readyGate, 'sync_suppressed_cooldown', {
          session_id: sessionId,
          event_id: eventId,
          source,
      ...
      if (guardedSyncInFlightRef.current) {
        rcBreadcrumb(RC_CATEGORY.readyGate, 'sync_coalesced', {
          session_id: sessionId,
          event_id: eventId,
          source,
        });
  ...
  useEffect(() => {
    const sync = () => {
      void guardedSyncSession('app_state_or_mount');
    };
    sync();
  ...
  useEffect(() => {
    if (
      ...
      return;
    if (!iAmReady || !partnerReadyKnown || partnerReady || snoozedByPartner)
      return;
    ...
    void (async () => {
      ...
      if (recovery.action === 'go_date') {
        await reconcileFromCanonicalTruth(
          'overlay_initial_daily_startable_check',
        );
        return;
      }
      if (recovery.action === 'go_survey') {
      ...
      if (reg?.queue_status !== 'in_ready_gate') {
        rcBreadcrumb(
          RC_CATEGORY.readyGate,
          'overlay_registration_stale_but_truth_ready',
          {
            session_id: sessionId,
            event_id: eventId,
            queue_status: reg?.queue_status ?? null,
  ...
  const handleSkip = useCallback(
    async (reason: 'skip' = 'skip') => {
      ...
      try {
        ...
        await handleForfeited(
          result.status === 'expired' ? 'timeout' : reason,
          {
        ...
        rcBreadcrumb(
          RC_CATEGORY.readyGate,
          'lobby_overlay_forfeit_failed_kept_open',
          {
            session_id: sessionId,
            event_id: eventId,
            message_snippet:
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (next <= 0) {
        ...
        void guardedSyncSession('countdown_expired')
          .then((result) => {
            if (result?.ok === false) {
              rcBreadcrumb(
                RC_CATEGORY.readyGate,
                'countdown_expiry_sync_deferred',
                {
                  session_id: sessionId,
                  event_id: eventId,
                  error: result.error,
        ...
        >
          {permissionsResolved && hasMediaPermission === false ? (
            ...
            >
              ...
              />
              {terminalActionError ? (
                <Text style={[styles.terminalError, { color: theme.danger }]}>
                  {terminalActionError}
                </Text>
              ) : null}
              <VibelyButton
            ...
            >
              ...
              {partnerReady && !iAmReady ? (
                <View style={styles.partnerReadyRow}>
                  ...
                  />
                  <Text
                    style={[styles.partnerReadyText, { color: theme.success }]}
                  >
              ...
              {snoozedByPartner ? (
                <View style={styles.partnerReadyRow}>
                  ...
                  />
                  <Text
                    style={[styles.subtleLine, { color: theme.textSecondary }]}
                  >
              ...
              {showReadyActionControls ? (
                <>
                  <View style={styles.ringWrap}>
                    ...
                    <Text style={[styles.countdownNum, { color: theme.text }]}>
                      {Math.max(0, timeLeft)}
                    </Text>
                  </View>
                      ...
                      void (async () => {
                        ...
                        try {
                          ...
                          if (!permissionReady) {
                            ...
                            trackNativeReadyGateEvent(
                              NativeReadyGateEvents.TRANSITION_FAILURE,
                              {
                                action: 'mark_ready',
                                source_action: 'ready_tap',
                          ...
                          if (result.ok === false) {
                            if (isReadyGateTransitionTimeoutSignal(result)) {
                              ...
                              if (
                                syncResult?.ok === true &&
                                isReadyGateReadyProgressStatus(
                                  syncResult.status,
                                )
                              ) {
                                setTerminalActionError(null);
                          ...
                          rcBreadcrumb(
                            RC_CATEGORY.readyGate,
                            'lobby_overlay_mark_ready_exception',
                            {
                              message_snippet:
                                e instanceof Error
                      ...
                      onPress={() => {
                        ...
                        void (async () => {
                          ...
                          try {
                            ...
                            rcBreadcrumb(
                              RC_CATEGORY.readyGate,
                              'lobby_overlay_snooze_exception',
                              {
                                message_snippet:
                                  e instanceof Error
                                    ? e.message.slice(0, 120)
                            ...
                            trackNativeReadyGateEvent(
                              NativeReadyGateEvents.TRANSITION_FAILURE,
                              {
                                ...
                                reason: fallback.reasonCode,
                                error_code:
                                  fallback.code ?? fallback.reasonCode,
                                terminal: false,
                                multi_device_conflict: fallback.staleOrConflict,
                              },
                            );
                ...
                <>
                  ...
                  >
                    ...
                    />
                    <Text
                      style={[styles.waitingPillText, { color: theme.text }]}
                    >
